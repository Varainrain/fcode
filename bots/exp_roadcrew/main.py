...
# exp_relay put the launcher road on the SIEGERS and scored 33%: laying it
# delayed the first gun t35 -> t58, and the tempo loss swamped the fact that it
# doubled guns-on-core (5.0 -> 10.4). That conflated two things - whether the
# road helps, and who pays for it. Here ONE dedicated builder lays it and the
# siegers never stop. Road cost comes out of a builder that was farming, not out
# of siege tempo. Measured launch geometry: pickup 1-2 tiles, throw up to 7,
# 1-turn cooldown, so ~6 spacing gives one hop per turn, and the road then
# serves every builder that follows for the rest of the game.
RELAY_SPACING = 6
RELAY_MIN_GAIN = 3
RELAY_MAX = 4
ROAD_RESERVE = 200
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if self.fiveDirections and len(self.fiveDirections) > 0: # only the first 5 bots spawned should be there
            ...
            if ct.can_spawn(spawnPos):
                ...
                elif self.numSpawned == 3:
                    ct.write_store(SLOT_ROADCREW_ID, spawned_id + 1)
                self.numSpawned += 1
    ...
    def builderBot(self, ct: Controller):
        ...
        if self._is_roadcrew(ct) and self._run_roadcrew(ct, myLoc):
            return
    ...
    def _is_roadcrew(self, ct: Controller) -> bool:
        assigned = ct.read_store(SLOT_ROADCREW_ID)
        return assigned != 0 and assigned == ct.get_id() + 1

    def _run_roadcrew(self, ct: Controller, myLoc: Position) -> bool:
        """Walk the route to their core, dropping launchers ~6 apart."""
        target = self._enemy_core_target()
        if target is None:
            return False
        if ct.get_global_resources() <= ROAD_RESERVE:
            return False
        dist = abs(myLoc.x - target.x) + abs(myLoc.y - target.y)
        if dist <= RELAY_SPACING:
            return False              # close enough; stop laying road
        built = 0
        for uid in ct.get_nearby_buildings():
            if (ct.get_team(uid) == ct.get_team()
                ...
                if abs(lp.x - myLoc.x) + abs(lp.y - myLoc.y) < RELAY_SPACING:
                    ...
                    return True      # keep walking, spacing not reached
        if built >= RELAY_MAX:
            return False             # road finished; rejoin normal life
        if ct.can_act():
            for d in CARDINALS:
                spot = myLoc.add(d)
    ...
    def runLauncher(self, ct: Controller) -> None:
        """Throw any friendly builder walking past, toward their core."""
        target = self._enemy_core_target()
        if target is None:
            return
        me = ct.get_position()
