...
# Measured 2026-08-01 from live replays, gunner placement by distance to the
# ENEMY core: Pantheon (94% form) puts 70% of its guns in the 4-6 firing band,
# 19% adjacent to the core, and ONE PERCENT in its own half. We put 41% in our
# own half and only 29% within 6. A gun that cannot reach their core is 10 Ti
# and a builder action spent on nothing - the discretionary attack state builds
# wherever the builder happens to be standing, aimed at whatever wandered past.
# 12 is the outer edge of Pantheon's distribution: 99% of their guns are inside
# it. Dedicated home defence is NOT affected - that runs through
# _run_home_defense and its own planner, which this does not touch.
CORE_PRESSURE_RADIUS = 12
...
class Player:
    ...
    def _pressures_enemy_core(self, seat: Position) -> bool:
        """Is a gun on this tile close enough to their core to ever matter?

        Unknown core (early rounds, no symmetry lock yet) returns True - with no
        information we keep the parent's behaviour rather than guess.
        """
        target = self._enemy_core_target()
        if target is None:
            return True
        return (abs(seat.x - target.x)
                + abs(seat.y - target.y)) <= CORE_PRESSURE_RADIUS
    ...
    def attack(self, ct: Controller, attackPos: Position):
        ...
        for d in CARDINALS:
            ...
            if dist < 10 and dist != 5 and 0 <= gunnerSpot.x < mapW and 0 <= gunnerSpot.y < mapH:
                ...
                if spotId is None:
                    if not self._pressures_enemy_core(gunnerSpot):
                        continue   # not worth 10 Ti out here; try another seat
