...
def should_raise_recall(core_hp: int, threat_count: int) -> bool:
    return threat_count > 0 and (
        core_hp < RECALL_CORE_HP or threat_count >= 2)
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        # One shooter still needs confirmed damage. Multiple real core-firing
        # turrets raise the existing soft recall before the first volley lands.
        under_attack = should_raise_recall(
            ct.get_hp(), len(self._home_threats(ct)))
