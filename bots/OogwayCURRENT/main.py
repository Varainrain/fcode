                ...
                if self.mapPf.checkPassable(ct, endTile):
                    ...
                    if endId is not None:
                        ...
                        if endType not in [EntityType.BARRIER] and endTeam != myTeam:
                            continue
                    bScore = math.exp(-endTile.distance_squared(teamCore) / 120) * math.exp(-myLoc.distance_squared(bPos) / 40)
                    if bScore > bestScore:
                        bestScore = bScore
                        bestEnd = endTile
    ...
    def routeHarvesterTask(self, ct: Controller, myLoc, myTeam) -> bool:
        ...
        for b in ct.get_nearby_buildings():
            ...
            if noTeamConv and len(workingSpots) > 0:
                ...
                bScore = math.exp(-workingSpots[0].distance_squared(teamCore) / 220) * math.exp(-myLoc.distance_squared(bPos) / 60)
                if bScore > bestScore:
                    bestScore = bScore
            ...
            for y in range(self.mapH):
                if self.mapPf.fullMap[x][y] == 1:
                    ...
                    if self.mapPf.inMoveZone(tile) and (x, y) not in enemyThreatened:
                        ...
                        tileScore = math.exp(-dist / 80) * math.exp(-myLoc.distance_squared(tile) / 220)
