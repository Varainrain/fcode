...
class Player:
    ...
    def _canonicalFor(self, ct: Controller, myLoc: Position, target: Position,
                      nearbyUnits, myTeam) -> bool:
        """True if I am the ally builder that should act on `target` — the
        closest, ties broken by lower id. Needs no messaging: every bot
        computes the same answer from its own vision."""
        myD = myLoc.distance_squared(target)
        myId = ct.get_id()
        for b in nearbyUnits:
            try:
                if b == myId or ct.get_team(b) != myTeam:
                    continue
                if ct.get_entity_type(b) != EntityType.BUILDER_BOT:
                    continue
                d = ct.get_position(b).distance_squared(target)
                if d < myD or (d == myD and b < myId):
                    return False
            except Exception:
                continue
        return True
    ...
    def runBestState(self, ct: Controller, myLoc: Position):
        ...
        if self.attackBan == 0:
            if ct.get_global_resources() > 120 and ct.get_id() > 4: # dont attack if u r broke and leave one bot for defense
                ...
                # ANTI-CLUMPING ("me_is_canonical_ally", from the 2nd-place
                # Cambridge team's postmortem). This bot has no claim
                # system, so every builder that sees a target independently
                # goes for it and several pile onto the same thing;
                # attackBan is the crude stand-in (seeing ONE friendly
                # gunner stops that builder attacking ANYTHING for 4-12
                # turns). Instead: only the builder CLOSEST to a target
                # acts on it, ties by id, computed from each bot's own
                # vision with zero comms. Measured on the oogerebus line:
                # 54% vs that champion, 64% vs krb, both over 168 games.
                if attackPos is not None and not self._canonicalFor(
                        ct, myLoc, attackPos, nearbyUnits, myTeam):
                    attackScore = 0
                    attackPos = None
