...
HEALER_TRACE_PATH = (
    ...
    "fcode-gate-artifacts/v89-healer-traces/risk_parity_trace.log"
)
...
class Player:
    ...
    def _risk_parity_healer_plan(
            self, ct: Controller, myLoc: Position,
            healer_target: Position, unsafe, direct_plans):
        if not direct_plans:
            return None
        direct_exposure = min(
            int(seat in unsafe) + int(stand in unsafe)
            for _, seat, stand, _ in direct_plans
        )
        best = None
        for seat in ct.get_nearby_tiles():
            ...
            for facing_index, facing in enumerate(DIRECTIONS):
                ...
                for approach_index, approach in enumerate(CARDINALS):
                    ...
                    key = (
                        exposure,
                        abs(myLoc.x - stand.x) + abs(myLoc.y - stand.y),
                        seat.distance_squared(healer_target),
                        seat.x,
                        seat.y,
                        facing_index,
                        approach_index,
                    ...
                    if best is None or key < best[0]:
                        best = (key, seat, stand, facing)
        if best is None or best[0][0] > direct_exposure:
            return None
        return best[1:]
    ...
    def _run_core_siege(self, ct: Controller, myLoc: Position) -> None:
        ...
        if healer_target is not None:
            suppression_plan = self._risk_parity_healer_plan(
                ct, myLoc, healer_target, unsafe, plans)
            if suppression_plan is not None:
                if self._execute_siege_plan(
                        ct, myLoc, suppression_plan):
                    healer_trace(
                        "HEALER_BUILD",
