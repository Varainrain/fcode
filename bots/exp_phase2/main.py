...
SLOT_SIEGER_2_ID = 10
SLOT_ENEMY_CORE = 11
SLOT_SIEGER_CUTOFF = 12
SLOT_SIEGER_2_COUNTER_TARGET = 13
SLOT_HOME_DEFENDER_ID = 14
SLOT_HOME_THREAT = 15
SIEGE_START = 45
SIEGE_TITANIUM_FLOOR = 120
# Traced turn by turn from a Pantheon replay, attributing each building to the
# builder standing next to it:
#   builder 3:  t3 harvester, t5-t33 nine conveyors     (pure economy)
#   builder 5:  t9 harvester, t11-t25 eight conveyors   (pure economy)
#   builder 88: t34,37,39,41 four gunners               (pure delivery)
#   builder 97: t44,50 two gunners                      (pure delivery)
# They specialise exactly as we do. The difference is ORDER: their economy
# builders are the EARLY ids and their gun builders are LATE ones, spawned once
# income exists. We designate siegers as spawns #1 and #2 and send them off at
# ~t5, committing our first builders to delivery before there is an economy to
# sustain it.
# So: the first CADRE_SIZE builders farm, everything spawned later delivers.
# exp_cadre tested this shape but also tripled the spawn rate and burned the
# bank on 30-Ti builders; the spawn rule here is the parent's, untouched.
CADRE_SIZE = 4
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if self.fiveDirections and len(self.fiveDirections) > 0: # only the first 5 bots spawned should be there
            ...
            if ct.can_spawn(spawnPos):
                ...
                if self.numSpawned + 1 == CADRE_SIZE:
                    # ids only increase, so a threshold is a stable role test.
                    # Everyone after the cadre is a sieger; the cadre never is.
                    ct.write_store(SLOT_SIEGER_CUTOFF, spawned_id + 1)
                self.numSpawned += 1
        ...
        # Pantheon's gun builders are ids 88 and 97 - by t34 they have MANY
        # units, and every one spawned after the economy phase goes straight to
        # delivery. exp_phase got the ORDER right and still landed 1.5 guns on
        # the core because our spawn rule caps the population: at 5 builders the
        # threshold is already 380 Ti, so "everyone after the cadre" is nearly
        # nobody. exp_cadre's mistake was cutting the base to 40 AND shrinking
        # the cadre; this keeps the cadre at 4 and only flattens the growth
        # term, so builders stay affordable once income exists.
        if globalTitanium > 80 + 25 * self.numSpawned:
    ...
    def _is_sieger(self, ct: Controller) -> bool:
        cutoff = ct.read_store(SLOT_SIEGER_CUTOFF)
        return cutoff != 0 and ct.get_id() + 1 > cutoff
