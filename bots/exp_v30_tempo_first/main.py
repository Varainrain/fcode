...
# TEMPO FIRST — from the Phase 0 diagnostics on 70 replays of v29's own losing
# matches (2026-08-04):
#   D1  52.7% of our guns sit in our OWN half; opponents put 29.1% there.
#   D3  our guns are NOT inefficient - they live 9 turns to their 5 and deal
#       37.9 core damage each to their 25.7 (489 a game against 358).
#   D2  but we win the race to the first gun on the enemy core in only 15 of 64
#       games (23%), median t20 to their t14, and t29 to t15 in longer games.
# So the opening gun budget goes to home defence and the offensive gun lands
# 6-14 turns late. 23 of 26 losses are core_destroyed at a median of t62 with
# 12 inside t60 - we are killed before our own kill lands.
# This does NOT restrict placement (exp_core_pressure did and gated 46%,
# losing 9 points to rushers). Defence is only DEFERRED: until the team has put
# its first gun on the enemy core, a defensive counter-gun must clear a real
# threat bar, and the defender heals instead. After that, v29 behaviour exactly.
DEFENSE_HOLD_SLOT = 11        # 0-8 taken; 9/10 free, 11 unused here
DEFENSE_HOLD_MIN_THREATS = 2  # before first blood, answer only a real push
...
class Player:
    ...
    def _firstBloodDone(self, ct: Controller) -> bool:
        return ct.read_store(DEFENSE_HOLD_SLOT) == 1

    def _markFirstBlood(self, ct: Controller) -> None:
        if ct.read_store(DEFENSE_HOLD_SLOT) != 1:
            ct.write_store(DEFENSE_HOLD_SLOT, 1)
    ...
    def runDefend(self, ct: Controller):
        ...
        if uncovered:
            # TEMPO FIRST: before the team has landed a gun on their core, a
            # single uncovered turret is not worth the opening gun budget - that
            # spend is what puts our own first gun 6-14 turns late. Answer only a
            # real push (2+), otherwise hold and heal. After first blood this is
            # exactly v29.
            if (not self._firstBloodDone(ct)
                    and len(uncovered) < DEFENSE_HOLD_MIN_THREATS):
                self.healCore(ct)
                return
