...
def gunnerSeatScore(buildableNow, seatCoverage, builderDistance, coreDistance):
    """Prefer an immediate legal shot before walking toward equivalent plans."""
    return (0 if buildableNow else 1, seatCoverage,
            builderDistance, coreDistance)
...
class Player:
    ...
    def findGunnerSpot(self, ct):
        ...
        for spotPos, spotDir in coreAttackers:
            ...
            if seatCov > 1:
                continue
            score = gunnerSeatScore(
                ct.can_build_gunner(spotPos, spotDir), seatCov,
                myLoc.distance_squared(spotPos),
                spotPos.distance_squared(enemyCore))
