...
class Player:
    ...
    def _launcher(self, ct):
        """THE RELAY. A throw is free and instant; walking is one tile per
        turn. HTTP 418 build a launcher at t3 and throw their builder six
        tiles at t5 (match 779ef2a1 g3). Our spear mirrors have been lost
        by five to ten turns, so transport is worth real titanium."""
        pos = ct.get_position()
        my = ct.get_team()
        if self.foe is None:
            self.foe = Position(self.w - 2 - pos.x, self.h - 2 - pos.y)
        rider = None
        for u in ct.get_nearby_units():
            ...
            if ct.get_entity_type(u) != EntityType.BUILDER_BOT:
                continue
            p = ct.get_position(u)
            if max(abs(p.x - pos.x), abs(p.y - pos.y)) <= 1:
                rider = p
                break
        if rider is None:
            return
        # take the legal landing that gains the most ground; scan widely,
        # since hand-picked directions found no legal tile at all
        best, bestGain = None, 0
        ride_d = cheb(rider, self.foe)
        for ax in range(-6, 7):
            for ay in range(-6, 7):
                if ax == 0 and ay == 0:
                    continue
                cand = Position(pos.x + ax, pos.y + ay)
                if not (0 <= cand.x < self.w and 0 <= cand.y < self.h):
                    continue
                gain = ride_d - cheb(cand, self.foe)
                if gain <= bestGain:
                    continue
                if ct.can_launch(rider, cand):
                    best, bestGain = cand, gain
        if best is not None:
            ct.launch(rider, best)
    ...
    def _core_turn(self, ct):
        ...
        # PRE-BANK. Their spec: start converting once the builder is
        # ~8 tiles out, so the first sentinel can fire the turn after it is
        # placed. Ours waited for a sentinel to exist and then converted
        # 20/turn into a volley that BURNS 20/turn - four sentinels firing
        # is 2 shots a turn at 10 ammo each - so the opening volley fired
        # at a fraction of its rate.
        near = ct.read_store(5) == 1
        if (built > 0 or near) and want > 0 and ct.can_convert_ammo(want):
    ...
    def _spear(self, ct):
        ...
        # RELAY: Jython just 5-0'd us by throwing their builder in and
        # arriving first. A throw is free and instant; walking is one tile
        # a turn. Capped at four stones, each laid only after we have moved
        # on from the last, and never at the cost of the four sentinels.
        lastl = getattr(self, 'last_lau', None)
        # ONE stone only. Four launchers cost ~80 Ti and three extra
        # stand-still turns for hops that walking nearly matches; the first
        # throw is the one that pays.
        if (getattr(self, 'lau', 0) < 1 and can_act and far > 12
                and self.built == 0
                and (lastl is None or cheb(pos, lastl) >= 4)
                and ct.get_global_resources()
                >= ct.get_launcher_cost() + 4 * ct.get_sentinel_cost()):
            for d in CARDINALS:
                sp = step(pos, d)
                if ct.can_build_launcher(sp):
                    ct.build_launcher(sp)
                    self.lau = getattr(self, 'lau', 0) + 1
                    self.last_lau = pos
                    self.wait_throw = 3
                    return
        if getattr(self, 'wait_throw', 0) > 0:
            self.wait_throw -= 1
            return
        if far <= 12:
            ct.write_store(5, 1)    # core: start banking ammo NOW
        ...
        if want_build and can_act \
            ...
            # THE ANVIL. not adgato's four sentinels land on CONSECUTIVE
            # turns - t25,26,27,28 vs Bean; t31,32,33,35 vs Clankers -
            # because their builder barely moves between placements. Ours
            # walked a tile per seat and took 8-11 turns, firing at a
            # fraction of full strength through the whole opening.
            # So: find ONE tile whose orthogonal neighbours are all valid
            # seats, stand on it, and build without moving again.
            def seat_ok(s):
                if not (0 <= s.x < self.w and 0 <= s.y < self.h):
                    return None
                if (s.x, s.y) in self.graves:
                    return None
                if ct.is_in_vision(s):
                    ...
                    if ct.get_tile_builder_bot_id(s) is not None:
                        return None
                for t2 in self._foe_tiles():
                    dx = t2.x - s.x
                    dy = t2.y - s.y
                    if dx == 0 or dy == 0 or abs(dx) == abs(dy):
                        reach = 5 if (dx == 0 or dy == 0) else 4
                        if 2 <= max(abs(dx), abs(dy)) <= reach:
                            return t2
                return None

            anvil = getattr(self, 'anvil', None)
            if getattr(self, 'bad_anvils', None) is None:
                self.bad_anvils = set()
            if anvil is None:
                best, bestN = None, 0
                for ax in range(-7, 8):
                    for ay in range(-7, 8):
                        stand = Position(self.foe.x + ax, self.foe.y + ay)
                        if not (0 <= stand.x < self.w
                                and 0 <= stand.y < self.h):
                            continue
                        if (stand.x, stand.y) in self.bad_anvils:
                            continue
                        if ct.is_in_vision(stand)                                 and not ct.is_tile_passable(stand):
                            continue
                        n = 0
                        for d in CARDINALS:
                            if seat_ok(step(stand, d)) is not None:
                                n += 1
                        if n > bestN:
                            best, bestN = stand, n
                if bestN >= 2:
            ...
            if anvil is not None:
                if (pos.x, pos.y) != (anvil.x, anvil.y):
                    # give up on an anvil we cannot reach, and never pick
                    # it again - auroraveil placed ZERO sentinels because
                    # we re-chose the same unreachable tile every turn
                    self.anvil_wait = getattr(self, 'anvil_wait', 0) + 1
                    if self.anvil_wait > 10 or self.stuck >= 3:
                        self.bad_anvils.add((anvil.x, anvil.y))
                        self.anvil = None
                    else:
                        self._go(ct, pos, anvil)
                        return
                for d in CARDINALS:
                    s = step(anvil, d)
                    tgt = seat_ok(s)
                    if tgt is None:
                        continue
                    back = dir8_toward(s, tgt)
                    if back is not None and ct.can_build_sentinel(s, back):
                        ct.build_sentinel(s, back)
                        self.built += 1
                        self.placed[(s.x, s.y)] = ct.get_current_round()
                        if getattr(self, 'mine', None) is None:
                            self.mine = []
                        self.mine.append(s)
                        ct.write_store(SLOT_BUILT, self.built)
                        return
                self.bad_anvils.add((anvil.x, anvil.y))
                self.anvil = None       # exhausted: fall back to scanning
            ...
            if cands:
                ...
                cands.sort(key=key)
                # LATCH THE WALK TARGET. Measured (match d9727ca9 g1): the
                # builder reached (17,1) at t21 and then bounced
                # (17,1)-(17,2)-(17,3)-(17,2) until it died, because the
                # candidate list is re-sorted every turn and two seats kept
                # swapping places as we moved. Commit to one seat until we
                # arrive, it gets taken, or we give it up.
                latched = getattr(self, 'seat_goal', None)
                if latched is not None:
                    still_ok = True
                    if (latched.x, latched.y) in self.graves:
                        still_ok = False
                    elif ct.is_in_vision(latched)                             and ct.get_tile_building_id(latched) is not None:
                        still_ok = False
                    if still_ok:
                        self.seat_wait = getattr(self, 'seat_wait', 0) + 1
                        if self.seat_wait > 25:
                            still_ok = False    # unreachable: pick another
                    if still_ok and pos.distance_squared(latched) > 1:
                        for (s2, t2) in cands:
                            if (s2.x, s2.y) == (latched.x, latched.y):
                                self._go(ct, pos, s2)
                                return
                    if not still_ok:
                        self.seat_goal = None
                        self.seat_wait = 0
                        self.graves.add((latched.x, latched.y))
                walk_to = None
                standing_on = False
                for s, t in cands:
