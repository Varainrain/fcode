...
# Pantheon lands 18.7 guns on the enemy core per game from an economy the same
# size as ours (4.1 harvesters); we land 4.6. Population, allocation, placement
# and transit were all tested and all failed, which leaves SURVIVAL: they build
# ~40 barriers a game and 39% of them sit in the 4-6 siege-seat band, armouring
# the forward position rather than home. A barrier is 3 Ti for 30 HP - three
# gunner shots - and per Oogway's own pathfinding, our own barriers are passable
# to us, so this costs no movement.
# NOT the aegis-v1 / bastion mechanism: those armoured HOME and were correctly
# rejected. This armours the sieger where it is actually being shot.
ARMOR_COST_FLOOR = 40
...
class Player:
    ...
    def _execute_direct_siege_plan(
        ...
        if not plans:
            # No seat we can shoot from - every one is occupied or covered.
            # That is exactly when Pantheon puts 30 HP down instead, and it is
            # the only moment a sieger has a spare action anyway.
    ...
    def _forward_armor(self, ct: Controller, myLoc: Position) -> bool:
        """Put 30 HP between us and whatever is shooting the siege seat."""
        if not ct.can_act() or ct.get_global_resources() <= ARMOR_COST_FLOOR:
            return False
        my_team = ct.get_team()
        threat = None
        for bid in ct.get_nearby_buildings():
            ...
            tiles = ct.get_attackable_tiles_from(
                ct.get_position(bid), ct.get_direction(bid), btype)
            if myLoc in tiles:
                threat = ct.get_position(bid)
                break
        if threat is None:
            return False
        ...
        for d in CARDINALS:
            ...
            if ct.get_tile_building_id(spot) is not None:
                continue
            dist = abs(spot.x - threat.x) + abs(spot.y - threat.y)
            if best_d is None or dist < best_d:
                if ct.can_build_barrier(spot):
                    best, best_d = spot, dist
        if best is not None:
    ...
    def _execute_siege_plan(self, ct: Controller, myLoc: Position, plan) -> None:
        ...
        if myLoc != stand:
            self.mapPf.moveTo(ct, stand)
            return
        if (not ct.can_build_gunner(seat, facing)
                and self._forward_armor(ct, myLoc)):
            return
        if (ct.can_act()
