...
# FIX 1: NON-BUILDER ENTITIES HAVE NO MAP.
# Every entity gets its own Player instance and only builderBot() calls
# mapPf.setupMap(), so in a gunner, sentinel or launcher the whole pathfinder is
# empty - mapPf.enemyCorePos and mapPf.teamCore are permanently None.
# mapPathfinding.py already notes this for gunners (coreThreatSpots reads the
# team core from slot 7 for exactly this reason), but nothing published the
# ENEMY core, so any future non-builder logic that needs it silently does
# nothing. That is not hypothetical: a launcher throw routine written against
# mapPf.enemyCorePos never fired once in a whole game.
# Slots 0-8 are taken (0 numSpawned, 1-6 map share, 7 team core, 8 symmetry).
ENEMY_CORE_SLOT = 9
...
class Player:
    ...
    def enemyCoreFromStore(self, ct: Controller):
        """The enemy core for entities that have no pathfinder of their own."""
        compact = ct.read_store(ENEMY_CORE_SLOT)
        if compact == 0:
            return None
        return Position(compact // 32, compact % 32)

    def publishEnemyCore(self, ct: Controller) -> None:
        """Builders share the target so map-less entities can use it."""
