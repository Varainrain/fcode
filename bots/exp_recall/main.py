...
class Player:
    ...
    def builderBot(self, ct: Controller):
        ...
        # A sieger holding a seat at their core is worth nothing if ours dies
        # first. While the recall is up, siegers defend instead.
        if ct.read_store(SLOT_RECALL) == 1 and self._is_sieger(ct):
            threat = unpack_position(ct.read_store(SLOT_HOME_THREAT))
            if threat is not None:
                self._run_home_defense(ct, myLoc, threat)
                return
