    ...
    def _spear(self, ct):
        ...
        for tile in list(self.placed):
            ...
            if ct.is_in_vision(p) and ct.get_tile_building_id(p) is None:
                # The killer covers a ZONE, not a tile. Measured (match
                # 0a8fcdb3 g3): after a seat died we planted the next one
                # RIGHT BESIDE it, inside the same three gunners' lines,
                # and lost that one and the builder too. Poison the whole
                # neighbourhood.
                for ax in (-1, 0, 1):
                    for ay in (-1, 0, 1):
                        self.graves.add((tile[0] + ax, tile[1] + ay))
        ...
        if want_build and can_act \
            ...
            if cands:
                ...
                mine = getattr(self, 'mine', [])
                def key(c):
                    s = c[0]
                    # NEVER EXTEND A LINE. Measured (match 6e555c14 g2):
                    # our four seats formed a row - (12,2),(13,2),(14,2),
                    # (15,1) against a core at (17,2) - and one gunner
                    # firing down that row ate them one at a time, t43,
                    # t48, t54, t59. adgato builds a 2x2 BLOCK, which no
                    # single line of fire can enfilade. Penalise any tile
                    # sharing a row or column with two seats we already
                    # hold, then hug the cluster.
                    row = sum(1 for m in mine if m.y == s.y)
                    col = sum(1 for m in mine if m.x == s.x)
                    lined = 1 if (row >= 2 or col >= 2) else 0
                    near = cheb(s, anchor) if anchor is not None else 0
                    return (lined, near, pos.distance_squared(s))
                cands.sort(key=key)
                walk_to = None
                standing_on = False
                for s, t in cands:
