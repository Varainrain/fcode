...
# slot 0 numSpawned, slot 1-6 map sharing, slot 7 initial target. Slots 8-10
# contain the opening role IDs through round 12, then become three dynamic
# defender IDs. Slot 12 is the defender count and 13-15 are their targets.
SLOT_WALLER_ID = 8
SLOT_SIEGER_1_ID = 9
SLOT_SIEGER_2_ID = 10
SLOT_ENEMY_CORE = 11
SLOT_DEFENSE_COUNT = 12
DEFENDER_ID_SLOTS = (8, 9, 10)
DEFENDER_TARGET_SLOTS = (13, 14, 15)
OPENING_ROLE_PUBLISH_ROUNDS = 12
DEFENSE_ECONOMY_FLOOR = 2
SIEGE_START = 45
...
def required_defense_fronts(
        threat_count: int, covering_guns: int, local_builders: int) -> int:
    """Size defense from pressure, while leaving a local economy floor."""
    if threat_count <= 0 or local_builders <= 0:
        return 0
    capacity = max(1, local_builders - DEFENSE_ECONOMY_FLOOR)
    pressure_gap = max(1, threat_count - covering_guns + 1)
    return min(3, threat_count, capacity, pressure_gap)


def choose_defense_fronts(candidates, sieger_ids, threats, coverage):
    """Assign distinct weakly-covered threats to nearby builders.

    `candidates` is [(id, pos)], `threats` is [(id, pos)], and `coverage`
    maps threat IDs to the number of live friendly guns that can answer it.
    """
    count = required_defense_fronts(
        len(threats), sum(coverage.get(tid, 0) for tid, _ in threats),
        len(candidates))
    ordered_threats = sorted(
        threats, key=lambda item: (coverage.get(item[0], 0), item[1].x,
                                   item[1].y, item[0]))
    unused = list(candidates)
    assignments = []
    for threat_id, threat_pos in ordered_threats[:count]:
        if not unused:
            break
        chosen = min(
            unused,
            key=lambda item: (
                ...
                item[1].distance_squared(threat_pos), item[0]))
        assignments.append((chosen[0], threat_id, threat_pos))
...
class Player:
    def __init__(self):
        ...
        self._baseRole = 0
        self._openingRoleIds = [None, None, None]
        self._defenseAssignments = []
        self._homeThreatLastSeen = None
        self._defenseTarget = None
        self._frontRepairActions = 0
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if ct.get_current_round() <= OPENING_ROLE_PUBLISH_ROUNDS:
            for slot, entity_id in zip(DEFENDER_ID_SLOTS,
                                       self._openingRoleIds):
                if entity_id is not None:
                    ct.write_store(slot, entity_id + 1)
            ct.write_store(SLOT_DEFENSE_COUNT, 0)
        else:
            self._assign_defense_fronts(ct)
    ...
    def builderBot(self, ct: Controller):
        ...
        self.turnsAlive += 1
        threat = self._assigned_defense_target(ct)
        if threat is not None:
            if threat != self._defenseTarget:
                self._defenseTarget = threat
                self._frontRepairActions = 0
    ...
    def _is_waller(self, ct: Controller) -> bool:
        return self._baseRole == 1

    def _is_sieger(self, ct: Controller) -> bool:
        return self._baseRole in (2, 3)

    def _sieger_role(self, ct: Controller) -> int:
        return self._baseRole - 1 if self._baseRole in (2, 3) else 0

    def _latch_base_role(self, ct: Controller) -> None:
        if self._baseRole or ct.get_current_round() > OPENING_ROLE_PUBLISH_ROUNDS:
            return
        encoded = ct.get_id() + 1
        for role, slot in enumerate(DEFENDER_ID_SLOTS, start=1):
            if ct.read_store(slot) == encoded:
                self._baseRole = role
                return

    def _assigned_defense_target(self, ct: Controller):
        encoded = ct.get_id() + 1
        count = min(3, ct.read_store(SLOT_DEFENSE_COUNT))
        for index in range(count):
            if ct.read_store(DEFENDER_ID_SLOTS[index]) == encoded:
                return unpack_position(
                    ct.read_store(DEFENDER_TARGET_SLOTS[index]))
        return None
    ...
    def _threat_coverage(self, ct: Controller, threats):
        coverage = {item[3]: 0 for item in threats}
        my_team = ct.get_team()
        friendly_guns = []
        for building_id in ct.get_nearby_buildings():
            if (ct.get_team(building_id) == my_team
                    and ct.get_entity_type(building_id)
                    == EntityType.GUNNER):
                friendly_guns.append((
                    ct.get_position(building_id),
                    ct.get_direction(building_id)))
        for _, _, _, threat_id, threat_pos in threats:
            for gun_pos, gun_dir in friendly_guns:
                if threat_pos in ct.get_attackable_tiles_from(
                        gun_pos, gun_dir, EntityType.GUNNER):
                    coverage[threat_id] += 1
        return coverage

    def _publish_defense_fronts(self, ct: Controller) -> None:
        ct.write_store(SLOT_DEFENSE_COUNT, len(self._defenseAssignments))
        for index in range(3):
            if index < len(self._defenseAssignments):
                builder_id, _, threat_pos = self._defenseAssignments[index]
                ct.write_store(DEFENDER_ID_SLOTS[index], builder_id + 1)
                ct.write_store(
                    DEFENDER_TARGET_SLOTS[index], pack_position(threat_pos))
            else:
                ct.write_store(DEFENDER_ID_SLOTS[index], 0)
                ct.write_store(DEFENDER_TARGET_SLOTS[index], 0)

    def _assign_defense_fronts(self, ct: Controller) -> None:
        ...
        if threats:
            ...
            self._defenseAssignments = []
            self._publish_defense_fronts(ct)
            return
        else:
            self._publish_defense_fronts(ct)
            return
        ...
        sieger_ids = {
            entity_id for entity_id in self._openingRoleIds[1:]
            if entity_id is not None
        }
        threat_pairs = [(item[3], item[4]) for item in threats]
        self._defenseAssignments = choose_defense_fronts(
            candidates, sieger_ids, threat_pairs,
            self._threat_coverage(ct, threats))
        self._publish_defense_fronts(ct)
    ...
    def _maintain_covering_gunner(
            self, ct: Controller, threat: Position) -> bool:
        """Spend a bounded number of actions keeping this front's gun alive."""
        if self._frontRepairActions >= FRONT_REPAIR_LIMIT:
            return False
        my_team = ct.get_team()
        damaged = []
        for building_id in ct.get_nearby_buildings():
            if (ct.get_team(building_id) != my_team
                ...
                continue
            position = ct.get_position(building_id)
            if threat not in ct.get_attackable_tiles_from(
                    position, ct.get_direction(building_id),
                    EntityType.GUNNER):
                continue
            missing = ct.get_max_hp(building_id) - ct.get_hp(building_id)
            if missing > 0:
                damaged.append((-missing, position.x, position.y, position))
        if not damaged:
            return False
        damaged.sort()
        position = damaged[0][3]
        if ct.can_heal(position):
            ct.heal(position)
            self._frontRepairActions += 1
        else:
            self.mapPf.moveTo(ct, position)
        return True
    ...
    def _run_home_defense(
        ...
        if self._friendly_gunner_covers(ct, threat):
            if self._maintain_covering_gunner(ct, threat):
                return
            self._heal_or_approach_core(ct, myLoc)
            return
    ...
    def _recall_is_up(self, ct: Controller) -> bool:
        # Dynamic fronts replace the lossy team-wide recall. Siegers never
        # abandon a live offense front merely because home is pressured.
        return False
    ...
    def runGunner (self, ct: Controller):
        ...
        if curTarget is not None:
            ...
            enemy_building = (
                targetId is not None and ct.get_team(targetId) != myTeam)
            enemy_builder = (
                bbId is not None and ct.get_team(bbId) != myTeam)
            if enemy_building or enemy_builder:
                ...
                for tile in ct.get_attackable_tiles_from(myPos, d, EntityType.GUNNER):
                    ...
                    if tileId is not None and ct.get_team(tileId) != myTeam:
                        ...
                        if tType in [EntityType.GUNNER, EntityType.SENTINEL]:
                            ...
                            curScore += 1
                    # Builder bots occupy a separate tile layer. The parent
                    # accidentally scored them only when an enemy building was
                    # on the same tile, so lone delivery specialists were
                    # invisible to gunner rotation.
                    if bbId is not None and ct.get_team(bbId) != myTeam:
                        curScore += 2
