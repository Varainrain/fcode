...
# SENTINEL GEOMETRY, measured on 2.3.4 with a probe bot (do not assume it):
#   gunner    cardinal 3, diagonal 2
#   sentinel  cardinal 5, diagonal 4
# The seat lists are NOT interchangeable - a gunner seat is not a sentinel seat.
sentinelLines = []
for _d in CARDINALS:
    _dx, _dy = _d.delta()
...
class MapPathfinder:
    ...
    def setupMap(self, ct: Controller):
        ...
        if self.distMap is None:
            self.distMap = [[16384 for _ in range(self.mapH)] for _ in range(self.mapW)]
        if self.conveyorMap is None:
            self.conveyorMap = [[[16384, 'stuck'] for _ in range(self.mapH)] for _ in range(self.mapW)]
    ...
    def fillConveyorDistTable(self, ct: Controller, curEnd: Position):
        ...
        enemyThreatened = self.enemyTurretThreatenedTiles(ct)
        w, h = self.mapW, self.mapH
        maxCost = (w + h) * (conveyorGunnerCost + 24) + 48 # because path ends when you go to a full conveyor
        for x in range(w):
            for y in range(h):
    ...
    def routeConveyor(self, ct: Controller, curEnd: Position):
        ...
        if ct.can_build_conveyor(curEnd, bestNextDir):
            ...
            if myLoc == curEnd:
                ...
                for d in CARDINALS:
                    newLoc = myLoc.add(d)
                    if newLoc.distance_squared(curEnd) == 1 and ct.can_move(d) and self.inMoveZone(newLoc):
                        ct.move(d)
            else:
    ...
    def inMoveZone(self, pos: Position) -> bool:
        if self.constrainedHome is None or self.constrainedFarCorner is None:
            return True
        return (pos.distance_squared(self.constrainedHome) <= 15 and
                pos.distance_squared(self.constrainedFarCorner) <= 32)
    ...
    def sentinelSpots(self, target: Position, mapW, mapH, blocked=True):
        """Every [position, facing] a SENTINEL could stand and hit target.

        Same walk as gunnerSpots but with the sentinel's longer reach (5/4
        against 3/2), which is why a sentinel needs two fewer tiles of travel
        to threaten a core.
        """
        spots = []
        for dx, dy, maxK, d in sentinelLines:
            x, y = target.x, target.y
            for k in range(1, maxK + 1):
                x -= dx
                y -= dy
