        ...
        for x in range(self.mapW):
            ...
            for y in range(self.mapH):
                if col[y] != 1 or (x, y) in threatened:
                    continue
                # HOME LEASH, EARLY. Measured (match dd289ee0 g2, 30x30):
                # our ONLY harvester went up at distance 13 from our core,
                # out in mid-map, while six enemy gunners were planted on
                # our doorstep from t22. A far first farm is a long chain,
                # a long walk and an undefended one. Before t70 nothing
                # beyond 8 tiles is worth claiming.
                if ct.get_current_round() < 70:
                    if abs(x - core.x) + abs(y - core.y) > 8:
                        continue
