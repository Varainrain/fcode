...
RALLY_MIN_NUM = 4       # with ATTACK_MOD=2: walker #2 solo, #4+ rally       # first rallying walker
RALLY_RADIUS2 = 10      # "together" = within this of each other at anchor
RALLY_PATIENCE = 25     # rounds to wait for a partner before going solo
...
def run(ct: Controller, player):
    ...
    # LINE WARFARE (Flotte decode 51b60877: their 22-gunner rolling line
    # beats camp-counters - lines have no keystone and no uncovered flank).
    # Rallied pairs now BUILD AS THEY ADVANCE: while marching in the enemy
    # half with a partner nearby, drop a corridor gunner wherever we have no
    # turret cover - the advance arrives as a FRONT of mutually-covering
    # turrets instead of two bodies. Capped at 2 drops per walker and gated
    # on a Ti surplus so scaled gunner costs never starve the destination
    # siege.
    if getattr(player, "_rallied", False) and getattr(player, "_corridor", 0) < 3:
        _foe2 = player.mapPf.enemyCorePos
        _tc2 = player.mapPf.teamCore
        if (_foe2 is not None and _tc2 is not None
                and myLoc.distance_squared(_foe2) < myLoc.distance_squared(_tc2)
                and myLoc.distance_squared(_foe2) > 36
                and ct.get_global_resources() >= ct.get_gunner_cost() + 120):
            _myTeam2 = ct.get_team()
            _partner = False
            for _u in ct.get_nearby_units():
                if (ct.get_team(_u) == _myTeam2
                        and ct.get_entity_type(_u) == EntityType.BUILDER_BOT):
                    _p = ct.get_position(_u)
                    if _p != myLoc and _p.distance_squared(myLoc) <= 10:
                        _partner = True
                        break
            _covered2 = False
            if _partner:
                for _b in ct.get_nearby_buildings():
                    if (ct.get_team(_b) == _myTeam2
                            and ct.get_entity_type(_b) in (EntityType.GUNNER, EntityType.SENTINEL)
                            and ct.get_position(_b).distance_squared(myLoc) <= 13):
                        _covered2 = True
                        break
                if not _covered2:
                    _fdx = (_foe2.x > myLoc.x) - (_foe2.x < myLoc.x)
                    _fdy = (_foe2.y > myLoc.y) - (_foe2.y < myLoc.y)
                    for _d in CARDINALS:
                        _n = myLoc.add(_d)
                        if not (0 <= _n.x < player.mapW and 0 <= _n.y < player.mapH):
                            continue
                        if ct.get_tile_building_id(_n) is not None:
                            continue
                        for _fd in DIRECTIONS:
                            if _fd.delta() == (_fdx, _fdy):
                                if ct.can_build_gunner(_n, _fd):
                                    ct.build_gunner(_n, _fd)
                                    player._corridor = getattr(player, "_corridor", 0) + 1
                                    return
                                break
...
def run_sentinel(ct: Controller, player):
    ...
    for tile in ct.get_attackable_tiles():
        ...
        if (bbId is not None and bId is None
                and any(tp.distance_squared(tile) <= 2 for tp in _etp)):
