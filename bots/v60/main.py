...
class Player:
    ...
    @_prof('attack')
    def _hitFacing(self, ct, spot, target, hint=None):
        """Engine-truth facing: the first facing whose ray ACTUALLY hits
        target from spot (ic3d watched misaligned diagonal gunners lose us
        sprint games - geometric (spot,dir) pairs and direction_to are
        never validated). Returns None if no facing hits: skip the seat."""
        order = ([hint] if hint is not None else []) + [d for d in DIRECTIONS if d is not hint]
        for f in order:
            try:
                if ct.can_fire_from(spot, f, EntityType.GUNNER, target):
                    return f
            except Exception:
                continue
        return None
    ...
    def runAttack(self, ct: Controller):
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            # CORE SEATS FIRST (ic3d: 'the rush stays a rush') — harass
            # only when no core seat is buildable this turn
            _didCore = False
            if ct.get_global_resources() >= 96:
                ...
                if gunnerStuff:
                    ...
                    gunnerDir = self._hitFacing(ct, gunnerSpot, _ec, gunnerDir) or gunnerDir
                    ...
                    if myDist < 1:
                        ...
                        return
                    _didCore = True
            if (not _didCore and ct.get_global_resources() >= 30
                    and self.attackHarvesterWithGunner(ct)):
                return
    ...
    def attackHarvesterWithGunner(self, ct: Controller) -> bool:
        ...
        if harvester is None:
            return False
        # ic3d watched our 2 opening rush gunners built REALLY far from
        # their core: this runs at Ti>=30, BEFORE the core-seat logic
        # (Ti>=96), so the rush chased mid-map harvesters. Harass only
        # harvesters actually near their core - the rush stays a rush.
        _ec = self.mapPf.enemyCorePos
        # d^2<=72 was still too loose (1a92b5b5 g2, ic3d watched: gunners
        # at manhattan 9-10 from their core, 2-3 from harvesters — the seat
        # follows the HARVESTER, so the radius must mean 'harvesters at the
        # core doorstep', where a harass gun also pressures the core)
        if _ec is not None and harvester.distance_squared(_ec) > 18:
            return False
        ...
        spotDir = self._hitFacing(ct, spotPos, harvester, spotDir) or spotDir
        ...
        for d in CARDINALS:
            ...
            if dist < 10 and dist != 5 and 0 <= spot.x < self.mapW and 0 <= spot.y < self.mapH:
                if ct.get_tile_building_id(spot) is None:
                    gunnerDir = self._hitFacing(ct, spot, target,
                                                spot.direction_to(target))
                    if gunnerDir is None:
                        continue
