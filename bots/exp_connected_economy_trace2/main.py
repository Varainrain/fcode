...
ECON_CUTOFF = 40
LATE_ROUTE_REPAIR_START = 80
LATE_ROUTE_MAX_LINKS = 8
LATE_ROUTE_RESOURCE_FLOOR = 100
LATE_ROUTE_SCORE_CAP = 6
...
def late_route_repair_allowed(
        round_number: int, resources: int, remaining_links: int) -> bool:
    """Allow a short, funded repair to an already-built economic asset."""
    return (
        round_number >= LATE_ROUTE_REPAIR_START
        and resources >= LATE_ROUTE_RESOURCE_FLOOR
        and remaining_links <= LATE_ROUTE_MAX_LINKS
    )


def choose_route_repairer(candidates, target: Position, excluded_ids=()):
    """One local builder owns a late repair: nearest, then stable entity ID."""
    eligible = [item for item in candidates if item[0] not in excluded_ids]
    if not eligible or target is None:
        return None
    return min(
        eligible,
        key=lambda item: (item[1].distance_squared(target), item[0]),
    )[0]
...
class Player:
    ...
    def _owns_late_route_repair(
            self, ct: Controller, target: Position) -> bool:
        myTeam = ct.get_team()
        candidates = [(ct.get_id(), ct.get_position())]
        for unitId in ct.get_nearby_units():
            if unitId == ct.get_id():
                continue
            if (ct.get_team(unitId) == myTeam
                    and ct.get_entity_type(unitId)
                    == EntityType.BUILDER_BOT):
                candidates.append((unitId, ct.get_position(unitId)))
        excluded = {
            encoded - 1
            for encoded in (
                ct.read_store(SLOT_SIEGER_1_ID),
                ct.read_store(SLOT_SIEGER_2_ID),
                ct.read_store(SLOT_HOME_DEFENDER_ID),
            )
            if encoded
        }
        return choose_route_repairer(
            candidates, target, excluded) == ct.get_id()
    ...
    def runBestState(self, ct: Controller, myLoc: Position):
        ...
        routeScore = 0 # proven orphan harvesters + their unfinished chains
        routePos = None
        routeDir = None
        routeLinks = 4096
        teamCore = self.mapPf.teamCore
        if teamCore is not None:
            for harvester, endTile, started, remainingLinks in (
                    self.mapPf.economyRouteCandidates(ct)):
                # Continuing an economic chain remains slightly more urgent
                # than starting its first link, preserving the parent's score
                # ordering while removing unrelated defensive dead ends.
                ceiling = 6 if started else 5
                floor = 2 if started else 1.6
                bScore = max(
                    floor,
                    ceiling
                    * max(0, 1 - endTile.distance_squared(teamCore) / 120)
                    * max(0, 1 - myLoc.distance_squared(endTile) / 60),
                )
                if bScore > routeScore:
                    routeScore = bScore
                    routePos = endTile
                    routeDir = Direction.CENTRE
                    routeLinks = remainingLinks
        if 78 <= ct.get_current_round() <= 100:
            owns = (
                routePos is not None
                and self._owns_late_route_repair(ct, routePos)
            )
            print(
                "CE2_TRACE", ct.get_current_round(), ct.get_id(), myLoc,
                "ti", ct.get_global_resources(), "attack", attackScore,
                "route", routeScore, routePos, routeLinks, "owns", owns,
            )
        ...
        # Preserve the pressure transition. Much later, an idle and well-funded
        # builder may finish a bounded route to an already-built harvester; it
        # may not discover new ore or override any visible enemy.
        currentRound = ct.get_current_round()
        if currentRound >= ECON_CUTOFF:
            harvestScore = 0
            if late_route_repair_allowed(
                    currentRound, ct.get_global_resources(), routeLinks
                    ) and self._owns_late_route_repair(ct, routePos):
                # At most the parent's normal route priority: nearby direct
                # threats still score up to 10, while low-value stalemate
                # targets can yield for the few actions needed to finish flow.
                routeScore = min(routeScore, LATE_ROUTE_SCORE_CAP)
            else:
                routeScore = 0
