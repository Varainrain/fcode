...
ECON_CUTOFF = 40
# FLANKING. Battlecode 2021 postmortem (Stone Tao): the breakthrough tactic was
# "sending your mucks at the enemy EC from different directions instead of a
# single one" - it splits the defender's attention and denies a single escape
# lane. Measured here, quadrants covered around the enemy core by guns within 6:
#   the teams that beat us  2.75 of 4
#   us                      1.83 of 4
#   Pantheon vs top teams   2.35-2.50
# We cluster on one face because _direct_siege_plans sorts seats by distance
# FROM THE BUILDER, so both siegers converge on whichever side they approach
# from. This adds a quadrant-coverage term ahead of distance: a seat in a
# quadrant we do not yet occupy outranks a closer one in a quadrant we do.
FLANK_BONUS = 3
...
class Player:
    ...
    @staticmethod
    def _quadrant(pos: Position, centre: Position) -> int:
        return (0 if pos.x >= centre.x else 1) | (0 if pos.y >= centre.y else 2)

    def _occupied_quadrants(self, ct: Controller, target: Position):
        """Which faces of their core our guns already cover."""
        taken = set()
        my_team = ct.get_team()
        for bid in ct.get_nearby_buildings():
            ...
            if abs(pos.x - target.x) + abs(pos.y - target.y) <= 6:
                taken.add(self._quadrant(pos, target))
        return taken
    ...
    def _direct_siege_plans(
        ...
        occupied = self._occupied_quadrants(ct, target)
        plans = []
        for seat in ct.get_nearby_tiles():
            if ct.get_tile_env(seat) != Environment.EMPTY:
                continue
            ...
            for approach in CARDINALS:
                ...
                if stand != myLoc:
                    ...
                    if not ct.is_tile_passable(stand):
                        continue
                quad = self._quadrant(seat, target)
                key = (
                    0 if diagonal else 1,
                    FLANK_BONUS if quad in occupied else 0,
                    abs(myLoc.x - stand.x) + abs(myLoc.y - stand.y),
