...
def armor_maintenance_action(
        threatened: bool, claimed: bool, building_is_friendly_barrier: bool,
        damaged: bool, site_empty: bool):
    """Pure maintenance policy for a previously claimed armor site."""
    if not threatened or not claimed:
        return None
    if building_is_friendly_barrier:
        return "heal" if damaged else None
    return "rebuild" if site_empty else None
...
class Player:
    ...
    def builderBot(self, ct: Controller):
        ...
        if (self._is_waller(ct)
            ...
            if self._pre_armor(
                    ct, myLoc, allow_new=ct.get_current_round() <= 60):
                return
            if (ct.get_current_round() <= 60
                    and self.wallDuty(ct, myLoc)):
                return
    ...
    def _pre_armor(
            self, ct: Controller, myLoc: Position, allow_new: bool = True) -> bool:
        """Build six firing-seat barriers, then repair them only under threat."""
        ...
        for x in range(tc.x - ARMOR_MAX_D, tc.x + ARMOR_MAX_D + 2):
            for y in range(tc.y - ARMOR_MAX_D, tc.y + ARMOR_MAX_D + 2):
                ...
                if not shoots:
                    continue
                eligible.append((abs(myLoc.x - x) + abs(myLoc.y - y), seat))

        threatened = ct.read_store(SLOT_HOME_THREAT) != 0
        if threatened:
            maintenance = []
            for distance, site in eligible:
                if site not in self._armorSites:
                    continue
                building_id = ct.get_tile_building_id(site)
                friendly_barrier = (
                ...
                action = armor_maintenance_action(
                    True, True, friendly_barrier, damaged,
                    building_id is None)
                if action is not None:
                    maintenance.append((0 if action == "heal" else 1,
                                        distance, site, action))
            if maintenance:
                _, distance, site, action = min(maintenance)
                if action == "heal" and ct.can_act() and ct.can_heal(site):
                    ct.heal(site)
                    return True
                if (action == "rebuild" and ct.can_act()
                        and ct.can_build_barrier(site)):
                    ct.build_barrier(site)
                    return True
                if distance > 1:
                    self.mapPf.moveTo(ct, site)
                    return True

        if not allow_new or len(self._armorSites) >= ARMOR_MAX:
            return False
        choices = [
            (distance, site) for distance, site in eligible
            if site not in self._armorSites
            and ct.get_tile_building_id(site) is None
        ]
        if not choices:
            return False
        distance, seat = min(choices)
        if ct.can_act() and ct.can_build_barrier(seat):
            ct.build_barrier(seat)
            self._armorSites.add(seat)
            return True
        if distance > 1:
