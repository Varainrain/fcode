...
HOME_COUNTER_TRACE_PATH = (
    "/mnt/c/Users/subodh/Downloads/"
    "fcode-gate-artifacts/v89-home-counter-traces/bounded_trace.log"
)


def home_counter_trace(*parts):
    with open(HOME_COUNTER_TRACE_PATH, "a", encoding="utf-8") as stream:
    ...
    def _run_home_defense(
        ...
        if ct.can_act() and ct.can_build_gunner(seat, facing):
            ...
            home_counter_trace(
                ...
                ct.get_id(),
                int(ct.can_act()),
                int(ct.can_build_gunner(seat, facing)),
                ct.get_global_resources(),
            )
