...
HEALER_STALL_ROUNDS = 6
HEALER_SUPPRESSION_COOLDOWN = 12
HEALER_PURSUIT_ROUNDS = 10
HEALER_TRACE_PATH = (
    "/mnt/c/Users/subodh/Downloads/"
    "fcode-gate-artifacts/v89-healer-traces/intercept_trace.log"
)


def healer_trace(*parts):
    with open(HEALER_TRACE_PATH, "a", encoding="utf-8") as stream:
...
def next_healer_stall_count(
        previous_hp, current_hp, has_direct_gunner,
        healer_count, current_count):
    """Count consecutive observations where visible healing stops HP progress."""
    if not has_direct_gunner or healer_count == 0:
        return 0
...
class Player:
    def __init__(self):
        ...
        self._enemyCoreObservedHp = None
        self._healerStallCount = 0
        self._healerSuppressionUntil = 0
        self._healerTargetId = None
        self._healerTargetUntil = 0
    ...
    def _run_home_defense(
        ...
        if plan is None or ct.get_global_resources() <= SIEGE_TITANIUM_FLOOR:
            self._heal_or_approach_core(ct, myLoc)
            return
    ...
    def _friendly_direct_gunner_exists(
            self, ct: Controller, target: Position) -> bool:
        target_tiles = set(core_footprint(target))
        my_team = ct.get_team()
        for building_id in ct.get_nearby_buildings():
            if (ct.get_team(building_id) != my_team
                    or ct.get_entity_type(building_id)
            ...
            if target_tiles.intersection(attack_tiles):
                return True
        return False

    def _healer_suppression_target(
        ...
        for unit_id in ct.get_nearby_units():
            if (ct.get_team(unit_id) == my_team
                    or ct.get_entity_type(unit_id)
                    != EntityType.BUILDER_BOT):
                continue
            position = ct.get_position(unit_id)
            if adjacent_to_core_footprint(position, target):
                healers.append((unit_id, position))

        current_hp = ct.get_hp(core_id)
        has_direct_gunner = self._friendly_direct_gunner_exists(ct, target)
        current_round = ct.get_current_round()
        active_target = next(
            (item for item in healers if item[0] == self._healerTargetId),
            None,
        )
        target_expired = (
            self._healerTargetId is not None
            and (
                not has_direct_gunner
                or active_target is None
                or current_round > self._healerTargetUntil
            )
        )
        if target_expired:
            self._healerTargetId = None
            active_target = None
            self._healerStallCount = 0
            self._healerSuppressionUntil = (
                current_round + HEALER_SUPPRESSION_COOLDOWN
            )
        if active_target is not None:
            self._enemyCoreObservedHp = current_hp
            return active_target[1]

        self._healerStallCount = next_healer_stall_count(
            self._enemyCoreObservedHp,
            current_hp,
            has_direct_gunner,
            len(healers),
            self._healerStallCount,
        )
        self._enemyCoreObservedHp = current_hp
        if (current_round < self._healerSuppressionUntil
                or self._healerStallCount < HEALER_STALL_ROUNDS
                or not healers):
            return None
        healers.sort(key=lambda item: (
            myLoc.distance_squared(item[1]),
            item[0],
        ))
        self._healerTargetId = healers[0][0]
        self._healerTargetUntil = current_round + HEALER_PURSUIT_ROUNDS
        healer_trace(
            "HEALER_TARGET",
            current_round,
            ct.get_id(),
    ...
    def _healer_intercept_plans(
            self, ct: Controller, plans, healer_target: Position, unsafe):
        intercepts = []
        for plan in plans:
            _, seat, stand, facing = plan
            if not ct.can_fire_from(
                    seat, facing, EntityType.GUNNER, healer_target):
                continue
            intercepts.append((
                0 if seat not in unsafe and stand not in unsafe else 1,
                plan,
            ))
        intercepts.sort(key=lambda item: (item[0], item[1][0]))
        return [item[1] for item in intercepts]
    ...
    def _run_core_siege(self, ct: Controller, myLoc: Position) -> None:
        ...
        healer_target = self._healer_suppression_target(
            ct, myLoc, target)
        if healer_target is not None:
            intercept_plans = self._healer_intercept_plans(
                ct, plans, healer_target, unsafe)
            if intercept_plans:
                if self._execute_siege_plan(
                        ct, myLoc, intercept_plans[0][1:]):
                    healer_trace(
                        "HEALER_BUILD",
            ...
            self._healerStallCount = 0
            self._healerSuppressionUntil = (
                ct.get_current_round() + HEALER_SUPPRESSION_COOLDOWN
            )
    ...
    def _execute_siege_plan(self, ct: Controller, myLoc: Position, plan) -> bool:
        seat, stand, facing = plan
        if myLoc != stand:
            self.mapPf.moveTo(ct, stand)
            return False
