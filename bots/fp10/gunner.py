...
class GunnerBot:
    ...
    def run_gunner(self, ct: Controller) -> None:
        ...
        if curOk and ct.can_fire(cur):
            # AMMO-BAIT GUARD: a 3-Ti barrier dropped into our line would eat
            # 5 shots (20 ammo) and be rebuilt forever, shielding the real
            # target behind it. Worth firing at only when it holds a builder
            # (they stand on things) or a rank>0 building.
            bId = ct.get_tile_building_id(cur)
            bbId = ct.get_tile_builder_bot_id(cur)
            worth = bbId is not None or (bId is not None
                                         and self._gun_good_target(ct, bId))
            if worth:
                ct.fire(cur)
                return
        elif curOk:
    ...
    def _gun_good_target(self, ct: Controller, bId) -> bool:
        ...
        if bType in (EntityType.HARVESTER, EntityType.CORE,
