...
def taxi_pickup_tile(launcherPos, dest):
    dx = (dest.x > launcherPos.x) - (dest.x < launcherPos.x)
    dy = (dest.y > launcherPos.y) - (dest.y < launcherPos.y)
    return Position(launcherPos.x + dx, launcherPos.y + dy)


def run_taxi(ct: Controller, player):
    """Taxi v3: throw the boarding-tile builder toward the FRONT (slot 14).
    No front, no throws - deliveries only ever land in friendly cover."""
    myLoc = ct.get_position()
    player.mapPf.setupMap(ct)
    fr = ct.read_store(SLOT_FRONT)
    if fr <= 0:
        return
    dest = Position((fr - 1) >> 5, (fr - 1) & 0x1F)
    pick = taxi_pickup_tile(myLoc, dest)
    ...
    for dx in range(-5, 6):
        for dy in range(-5, 6):
            ...
            if 0 <= t.x < player.mapW and 0 <= t.y < player.mapH:
                cands.append(t)
    cands.sort(key=lambda t: t.distance_squared(dest))
    for t in cands[:15]:
...
def run(ct: Controller, player):
    ...
    # TAXI RIDE (front-anchored): board only when a front EXISTS, home is
    # quiet (slot 7 threat bits clear), and we're still on our half.
    _frT = ct.read_store(SLOT_FRONT)
    _tcT = player.mapPf.teamCore
    if (_frT > 0 and _tcT is not None
            and ct.get_current_round() > 20
            and (ct.read_store(7) >> 10) == 0
            and getattr(player, "_taxi_wait", 0) < 8):
        _destT = Position((_frT - 1) >> 5, (_frT - 1) & 0x1F)
        if myLoc.distance_squared(_tcT) < myLoc.distance_squared(_destT):
