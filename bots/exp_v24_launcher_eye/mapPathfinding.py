...
class MapPathfinder:
    ...
    def setupMapObserver(self, ct: Controller):
        """Map upkeep for a NON-BUILDER entity: see tiles, fold in the shared
        board, and help eliminate symmetries - but never touch shareTiles,
        whose 12-bit channel is keyed on myNum and belongs to the builders.

        Why bother: LAUNCHER_VISION_RADIUS_SQ is 26 against a builder's 20, so a
        launcher parked at home is the widest permanent sensor we own, and the
        symmetry tracker resolves the enemy core by ruling out candidates from
        observed terrain. Pantheon report full symmetry shortly after their pad
        goes down - which is what lets them throw builders the RIGHT way at t2
        and land a gun at t13.
        """
        self.mapChanged = False
        if self.teamCore is None:
            for i in ct.get_nearby_buildings():
                if (ct.get_entity_type(i) == EntityType.CORE
                        and ct.get_team(i) == ct.get_team()):
                    self.teamCore = ct.get_position(i)
        if self.mapW is None or self.mapH is None:
            self.mapW = ct.get_map_width()
            self.mapH = ct.get_map_height()
