...
# LAUNCHER AS AN EYE. Pantheon report that full symmetry resolves shortly after
# their launcher goes down. The constants say why: a launcher sees radius^2 26
# to a builder's 20, and symmetry is eliminated by observing terrain. A pad at
# home is therefore a permanent wide sensor, and knowing the enemy core early is
# what lets a thrown builder be thrown the RIGHT way.
# Pad placement is off the core->enemy lane, because our own buildings block our
# own units and putting it on the axis cost eight turns of oscillation per
# builder when measured.
EYE_ROUND_LIMIT = 8
EYE_MIN_D = 2
EYE_MAX_D = 4
ENEMY_CORE_SLOT = 9
...
class Player:
    ...
    def run(self, ct: Controller) -> None:
        ...
        if etype == EntityType.CORE:
            ...
            self.runLauncherEye(ct)

    def runLauncherEye(self, ct: Controller) -> None:
        """A launcher earns its 20 Ti as a sensor before it throws anything."""
        self.mapPf.setupMapObserver(ct)
        target = self.mapPf.enemyCorePos
        if target is not None:
            ct.write_store(ENEMY_CORE_SLOT, target.x * 32 + target.y)

    def _buildEye(self, ct: Controller, myLoc: Position) -> bool:
        if ct.get_current_round() > EYE_ROUND_LIMIT or not ct.can_act():
            return False
        ...
        for b in ct.get_nearby_buildings():
            if (ct.get_team(b) == myTeam
                ...
                return False
        axis_x = 1 if abs(target.x - teamCore.x) >= abs(target.y - teamCore.y) else 0
        best = None
        for d in CARDINALS:
            spot = myLoc.add(d)
            dc = abs(spot.x - teamCore.x) + abs(spot.y - teamCore.y)
            if not (EYE_MIN_D <= dc <= EYE_MAX_D):
                continue
            if not ct.can_build_launcher(spot):
                continue
            perp = abs(spot.y - teamCore.y) if axis_x else abs(spot.x - teamCore.x)
            key = (0 if perp >= 1 else 1,
                   abs(spot.x - target.x) + abs(spot.y - target.y))
