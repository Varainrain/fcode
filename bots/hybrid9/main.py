...
SPEAR_DIST = 40     # spear only in the LONG band, see run()



class Player(eco.EcoBot, attack.AttackBot, gunner.GunnerBot):
    def __init__(self):
        self.spear = None   # built lazily if the map calls for it
    ...
    def run(self, ct: Controller) -> None:
        ...
        # MAP DISPATCH ON THE CURRENT ENGINE. hybrid5 pairs the spear with a
        # v180-era fp (752-line eco.py against this one's 978, and none of
        # this campaign's fixes - no seat graves, no HARV_TI 45, no seat
        # denial), which is why it loses the SHORT band 35% and the MID band
        # 25% while winning the LONG band 64%. The spear was carrying it and
        # the stale engine was sinking it. This build keeps the live fp18
        # everywhere and switches to the spear only in the band where the
        # spear measurably wins: distance >= 40 scored 57-64% for both
        # hybrids, while 26 (hybrid5) drags spear mode down into the mid
        # band where it scores 25%.
        mode = ct.read_store(SLOT_MODE)
        if mode == 0 and ct.get_entity_type() == EntityType.CORE:
            me = ct.get_position()
            dist = (abs(me.x - (self.mapW - 2 - me.x))
                    + abs(me.y - (self.mapH - 2 - me.y)))
    ...
    def spawn_allowed(self, ct: Controller) -> bool:
        ...
        if ct.get_current_round() > 100:
            ...
            if harv < 4 and ct.get_global_resources() > 200:
                return True
        # OOGWAY'S HEALBOT ANSWER TO THE SENTINEL RUSH (his idea, 2026-08-23:
        # "spam a bunch of eco bots whenever we face an attack of 3 or more
        # sentinels before round 100, and they will naturally heal the core
        # and kill enemy turrets - top ladder uses it"). This is a NARROWER
        # trigger than the one I tested and rejected: fp19 spawned on any core
        # damage or any enemy building near the ring and gated 43%/42%,
        # because that fires in nearly every game. Three enemy SENTINELS
        # before t100 is the spear rush and nothing else - not adgato plants
        # four between t31 and t35 and our core is dead by t53 (b7ff118f g1),
        # and healing at +4/turn against ~18 was never going to hold. More
        # hands is the only thing that scales the heal rate.
        if ct.get_current_round() < 100:
            sent = 0
            myTeam = ct.get_team()
            for b in ct.get_nearby_buildings():
