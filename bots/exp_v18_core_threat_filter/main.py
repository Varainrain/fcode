...
class Player:
    ...
    def runDefend(self, ct: Controller):
        ...
        enemyGunners = []
        for b in ct.get_nearby_buildings():
            if (ct.get_team(b) != myTeam and
                    ct.get_entity_type(b) in (EntityType.GUNNER, EntityType.SENTINEL) and
                    self.turretThreatensCore(ct, b)):
    ...
    def turretThreatensCore(self, ct: Controller, turretId: int) -> bool:
        """True only when the turret's current facing reaches our core."""
        teamCore = self.mapPf.teamCore
        if teamCore is None:
            return False
        coreTiles = {
            teamCore,
            teamCore.add(Direction.EAST),
            teamCore.add(Direction.SOUTH),
            teamCore.add(Direction.SOUTH).add(Direction.EAST),
        }
        turretType = ct.get_entity_type(turretId)
        attackable = ct.get_attackable_tiles_from(
            ct.get_position(turretId),
            ct.get_direction(turretId),
            turretType,
        )
        return any(tile in coreTiles for tile in attackable)
