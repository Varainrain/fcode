...
class Player(eco.EcoBot, attack.AttackBot, gunner.GunnerBot):
    ...
    def run(self, ct: Controller) -> None:
        ...
        if etype == EntityType.CORE:
            ...
            # fp10: sentinels exist now (atk_sent_siege seats them). Their
            # fire is INDIRECT - barriers never block it (measured 882 dmg
            # through a full wall) - which is the entire counter to walled
            # and heal-tanked cores. No get_sentinel_target() in the API, so
            # the runtime picks its own target each turn.
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        spare = ct.get_global_resources() - 60
    ...
    def spawn_allowed(self, ct: Controller) -> bool:
        ...
        # CREW FLOOR: get_unit_count() counts living units incl. the core.
        # After a wipe with a low bank the 349 gate is ~140 passive-income
        # rounds away - the team is dead long before. Below 4 living units,
        # spawn whenever affordable.
