...
def isCoreHealStand(tile: Position, teamCore: Position) -> bool:
    """Builders can heal only an orthogonally adjacent core tile."""
    ...
    return any(tile.distance_squared(coreTile) == 1 for coreTile in coreTiles)
        ...
        for dx in range(-1, 3):
            for dy in range(-1, 3):
                ...
                if ct.get_tile_building_id(tile) is not None:
                    continue
                if not isCoreHealStand(tile, teamCore):
                    continue
