...
class Player:
    ...
    def _friendly_covering_gunner(
            self, ct: Controller, threat: Position):
        my_team = ct.get_team()
        covering = []
        for building_id in ct.get_nearby_buildings():
            if (ct.get_team(building_id) != my_team
            ...
            if threat in ct.get_attackable_tiles_from(
                    ...
                    EntityType.GUNNER):
                position = ct.get_position(building_id)
                covering.append((position.x, position.y, building_id))
        if not covering:
            return None
        covering.sort()
        return covering[0][2]
    ...
    def _friendly_gunner_covers(
        ...
        return self._friendly_covering_gunner(ct, threat) is not None
    ...
    def _run_home_defense(
        ...
        if ct.is_in_vision(threat):
            ...
            if not any(
                ...
                return
        covering_gunner = self._friendly_covering_gunner(ct, threat)
        if covering_gunner is not None:
            gunner_pos = ct.get_position(covering_gunner)
            if (self._counterTendHeals < COUNTER_TEND_HEAL_LIMIT
                    and ct.get_hp(covering_gunner) < ct.get_max_hp(covering_gunner)
                    and ct.can_heal(gunner_pos)):
                ct.heal(gunner_pos)
                self._counterTendHeals += 1
                return
            self._heal_or_approach_core(ct, myLoc)
            return
