...
class Player:
    ...
    def runBestState(self, ct: Controller, myLoc: Position):
        ...
        # UNLESS the core is under attack: v8's unrated scrimmages improved
        # against the two teams whose plan this copies (Prompt Engineers 20% ->
        # 80%, Orizon 0% -> 20%) and got WORSE against the three that rush our
        # core (Pantheon 2-3 -> 0-5, Flotte 2-3 -> 0-5, SmartFridge 2-3 -> 1-4).
        # Zeroing the economy states pushes the builders that used to work near
        # home out to attack, emptying our half exactly when a rush lands. While
        # the recall is up, defence outranks the transition.
        if (ct.get_current_round() >= ECON_CUTOFF
                and ct.read_store(SLOT_RECALL) != 1):
            harvestScore = 0
            routeScore = 0
