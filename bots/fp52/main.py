...
class Player(eco.EcoBot, attack.AttackBot, gunner.GunnerBot):
    def __init__(self):
        self.homing = False  # step_home hysteresis latch
    ...
    def spawn_allowed(self, ct: Controller) -> bool:
        ...
        if ct.get_current_round() > 100:
            ...
            if harv < 4 and ct.get_global_resources() > 200:
                return True
        # HEALERS AGAINST THE SENTINEL RUSH (Oogway's trigger + ic3d's safety
        # rule). Three enemy SENTINELS before t100 is the spear rush and
        # nothing else: not adgato plants four between t31 and t35 and our
        # core is dead by t53, and a heal is +4 against roughly 18 incoming,
        # so the only thing that scales is MORE HANDS. Oogway's version alone
        # measured 90% against our spear replica. ic3d's addition is the part
        # that was missing - spawn them where they will not simply be shot,
        # which spawn_bot now enforces by preferring tiles no visible enemy
        # turret can fire on.
        if ct.get_current_round() < 100:
            sent = 0
            myTeam = ct.get_team()
    ...
    def _spawn_safe(self, ct: Controller, tile) -> bool:
        """Is this tile outside every visible enemy turret's current line?"""
        myTeam = ct.get_team()
        for b in ct.get_nearby_buildings():
            if ct.get_team(b) == myTeam:
                continue
    ...
    def spawn_bot(self, ct: Controller) -> None:
        ...
        if not spawnable:
            return
        # SPAWN OUT OF THE LINE OF FIRE (ic3d). A builder spawned onto a tile a
        # sentinel is already aimed at dies before it heals anything, which is
        # how a defensive spawn turns into a donation. Fall back to the full
        # list if every tile is covered - a bot on a bad tile still beats no
        # bot at all.
        safe = [t for t in spawnable if self._spawn_safe(ct, t)]
        if safe:
            spawnable = safe
        myLoc = ct.get_position()
