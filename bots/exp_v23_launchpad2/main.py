...
# LAUNCH PAD - measured from the 0-5 unrated sweep by Pantheon (#1) on 2026-08-03.
# Their first gun lands t13; ours lands t31. The cause is in their build list:
# 5 launchers across 5 games, 100% of them within 4 tiles of THEIR OWN core, and
# 26 barriers 100% within 6 of OURS. They are not walking their builders out -
# they throw them.
# Engine facts, measured directly: a launcher picks up a friendly builder
# standing 1-2 tiles away and throws it up to 7 tiles, on a 1-turn cooldown, for
# 20 Ti once. One pad next to our core therefore saves ~7 turns for EVERY
# outbound builder for the rest of the game.
# (An earlier attempt built a 4-launcher CHAIN and taxed the siegers to lay it -
# 156 Ti and it scored 33%. Pantheon builds ONE, at home, paid for by a builder
# that was standing there anyway.)
# v1 built the pad but only moved the first gun t24 -> t22, because outbound
# attackers path straight at the enemy core and rarely pass within the launcher's
# 1-2 tile pickup range. Pantheon reaches t13 with the SAME single pad, so the
# missing piece is routing: an attacker that is still near home detours one step
# to the pad, gets thrown ~7 tiles, and walks from there.
LAUNCHPAD_RIDE_RADIUS = 7    # only detour while still this close to our own core
LAUNCHPAD_MAX = 1
LAUNCHPAD_TITANIUM_FLOOR = 120
LAUNCH_MIN_GAIN = 3
...
class Player:
    ...
    def _ride_launchpad(self, ct: Controller, myLoc: Position) -> bool:
        """Walk one step onto the pad instead of the whole way to their core."""
        teamCore = self.mapPf.teamCore
        enemyCore = self.mapPf.enemyCorePos
        if teamCore is None or enemyCore is None:
            return False
        # only while still at home; once out on the map, keep marching
        if abs(myLoc.x - teamCore.x) + abs(myLoc.y - teamCore.y) > LAUNCHPAD_RIDE_RADIUS:
            return False
        myTeam = ct.get_team()
        pad = None
        ...
        if d <= 2:
            return False          # already in pickup range - let the pad throw us
        if d > LAUNCHPAD_RIDE_RADIUS:
            return False
        self.mapPf.moveTo(ct, pad)
        return True
    ...
    def runAttack(self, ct: Controller):
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            if self._ride_launchpad(ct, myLoc):
                return
    ...
    def runLauncher(self, ct: Controller) -> None:
        """Throw any friendly builder standing next to us toward their core."""
    ...
    def _build_launchpad(self, ct: Controller, myLoc: Position) -> bool:
        """One launcher beside our core, built by a defender already standing there."""
        teamCore = self.mapPf.teamCore
        if teamCore is None or self.mapPf.enemyCorePos is None:
            return False
        if not ct.can_act() or ct.get_global_resources() <= LAUNCHPAD_TITANIUM_FLOOR:
            return False
        myTeam = ct.get_team()
        pads = 0
        for b in ct.get_nearby_buildings():
            if (ct.get_team(b) == myTeam
                    and ct.get_entity_type(b) == EntityType.LAUNCHER):
                pads += 1
        if pads >= LAUNCHPAD_MAX:
            return False
        if abs(myLoc.x - teamCore.x) + abs(myLoc.y - teamCore.y) > 4:
            return False
        # prefer a tile on the enemy side so the throw arc points outward
        best = None
        for d in CARDINALS:
            spot = myLoc.add(d)
            if not ct.can_build_launcher(spot):
                continue
            key = abs(spot.x - self.mapPf.enemyCorePos.x) + abs(spot.y - self.mapPf.enemyCorePos.y)
