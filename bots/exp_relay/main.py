...
# THE ATTACK ROUTE. Measured against the engine, not inferred: a launcher picks
# up a friendly builder standing 1-2 tiles away and throws it up to 7 tiles, on
# a 1-turn cooldown, for 20 Ti base. Guns only matter within ~6 of the enemy
# core, and our siegers WALK 25-30 tiles to get there - that transit, not the
# dispatch time, is what caps our first gun at t52. Launchers spaced ~6 apart
# turn the trip into one turn per hop, and the chain is permanent: every
# builder after the first rides it. The core sits on ~470 unspent Ti at t45,
# so a 4-launcher route is already paid for.
RELAY_SPACING = 6          # launchers this far apart -> each throw lands in pickup range
RELAY_MIN_GAIN = 3         # only throw if it advances the bot at least this far
RELAY_TITANIUM_FLOOR = 200 # never spend the siege's gun money on road
RELAY_MAX = 5
...
class Player:
    ...
    def runLauncher(self, ct: Controller) -> None:
        ...
        No coordination and no store slot: a builder that wanders into pickup
        range gets flung forward, so the chain works for every unit that walks
        past it, including ones spawned much later.
        """
    ...
    def _lay_relay(self, ct: Controller, myLoc: Position, target: Position) -> bool:
        """Drop one launcher if we are on the road and none is near enough."""
        if ct.get_global_resources() <= RELAY_TITANIUM_FLOOR:
            return False
        if not ct.can_act():
            return False
        to_core = abs(myLoc.x - target.x) + abs(myLoc.y - target.y)
        if to_core <= RELAY_SPACING:
            return False          # close enough to just build the gun
        built = 0
        for uid in ct.get_nearby_buildings():
            if (ct.get_team(uid) == ct.get_team()
                ...
                if abs(lp.x - myLoc.x) + abs(lp.y - myLoc.y) < RELAY_SPACING:
                    return False  # one already covers this stretch
        if built >= RELAY_MAX:
            return False
        for d in CARDINALS:
            spot = myLoc.add(d)
