...
class Player:
    ...
    def _run_core_siege(self, ct: Controller, myLoc: Position) -> None:
        ...
        # A fortified core can occupy every legal firing seat with 3-Ti
        # barriers. With no direct plan, v8 walks at the core forever. Sieger 2
        # may open exactly one visible seat while Sieger 1 keeps direct pressure.
        if not plans:
            blocker = self._visible_core_seat_blocker(ct, target)
            if blocker is not None:
                if self._friendly_gunner_covers(ct, blocker):
                    ct.write_store(
                        SLOT_SIEGER_2_COUNTER_TARGET,
                        pack_position(blocker))
                    self.mapPf.moveTo(ct, target)
                    return
                breach_plan = self._safe_counter_plan(
                    ct, myLoc, blocker, unsafe)
                if breach_plan is not None:
                    ct.write_store(
                        SLOT_SIEGER_2_COUNTER_TARGET,
                        pack_position(blocker))
                    self._execute_siege_plan(ct, myLoc, breach_plan)
                    return
    ...
    def _visible_core_seat_blocker(
            self, ct: Controller, target: Position):
        """Return a visible enemy barrier occupying a real direct-fire seat."""
        my_team = ct.get_team()
        core_tiles = [
        ...
        for building_id in ct.get_nearby_buildings():
            if (ct.get_team(building_id) == my_team
                    or ct.get_entity_type(building_id) != EntityType.BARRIER):
                continue
            position = ct.get_position(building_id)
            is_firing_seat = any(
                ct.can_fire_from(
                    position, facing, EntityType.GUNNER, core_tile)
        ...
        return min(blockers)[4] if blockers else None
