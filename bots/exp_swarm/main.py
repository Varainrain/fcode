...
class Player:
    ...
    def _is_sieger(self, ct: Controller) -> bool:
        """Every builder delivers guns, not just the two designated ones.

        ISOLATION TEST. spar_pantheon combined this with damped economy states
        and landed at exactly 49% (kills 79-79) - dead even with the shipped
        bot. Those two changes may simply cancel: allocation helping, starved
        economy hurting. This changes ONLY the allocation, so the economy stays
        at the shipped bot's 4.0 harvesters / 25 conveyors, which already
        matches Pantheon's 4.1 / 17.7.

        The gap this targets: Pantheon converts an equivalent supply line into
        18.7 guns on the enemy core per game. We manage 4.6, with two siegers
        doing all the delivering while everyone else farms forever.
        """
        return True

    def _is_sieger_designated(self, ct: Controller) -> bool:
