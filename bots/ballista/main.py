...
# LAUNCHER OPENING (Pantheon's, per LAUNCHER-HANDOFF.md: pad t1-3 two tiles
# out and OFF the core->enemy axis; 3-4 throws in the first five turns; the
# thrown builder BOLTS straight in; first gun ~t13 instead of ~t21).
# slot 9  = pad built flag        slot 10 = enemy core pos, packed+1
# slot 11 = ride claim (id+1)     slot 12 = pad pos, packed+1
# The three landmines this integration avoids (all documented, all measured):
#  1. launchers have no mapPf -> runLauncher touches NO mapPf state; target
#     comes from slot 10 (builders publish) or point-mirror of slot 7.
#  2. mapPf.moveTo dead-ends on our own conveyors -> the bolt march uses its
#     own greedy stepper, never moveTo.
#  3. our own buildings block our own exit -> pad goes PERPENDICULAR to the
#     core->enemy axis, out of the corridor every builder marches through.
SLOT_PAD_FLAG = 9
SLOT_ENEMY_CORE = 10
SLOT_RIDE_CLAIM = 11
SLOT_PAD_POS = 12
RIDE_UNTIL = 10       # throws only matter in the opening (Pantheon: t2-t5)
BOLT_BAND = 6         # leave bolt state inside this manhattan of the target


def packPos1(pos):
    return pos.x * 32 + pos.y + 1
...
class Player:
    def __init__(self):
        self._lastPos = None      # for landing detection (position jump)
        self._bolt = False        # committed straight-march state
        self._rode = False        # set on LANDING, never on claim (v6 bug)
        self._rideWait = 0        # anti-deadlock (v8: riders waited t6-t89)
        self._padTried = False
    ...
    def builderBot(self, ct: Controller):
        ...
        # ---- launcher opening (see slot map at top) ----
        # publish the enemy core for the launcher (landmine 1: it has no map)
        if self.mapPf.enemyCorePos is not None and ct.read_store(SLOT_ENEMY_CORE) == 0:
            ct.write_store(SLOT_ENEMY_CORE, packPos1(self.mapPf.enemyCorePos))
        # LANDING DETECTION -> BOLT LATCH. The one piece 13 attempts missed:
        # a position jump >2 in one turn can only be a throw. Latch the
        # committed march HERE (on landing), not on claim (v6: claim-latch
        # disqualified every loser of the one-claim-per-turn race).
        if self._lastPos is not None:
            _jump = abs(myLoc.x - self._lastPos.x) + abs(myLoc.y - self._lastPos.y)
            if _jump > 2:
                self._bolt = True
                self._rode = True
        self._lastPos = myLoc
        if self._bolt:
            if self.runBolt(ct, myLoc):
                return
        if self.mapPf.myNum % 2 == 1: # attacking
            if self.launcherOpening(ct, myLoc):
                return
            ...
            self.runAttack(ct)
        else: # economy (defenders 4/6 included, clamped to their home radius)
    ...
    def _boltTarget(self, ct: Controller):
        t = unpackPos1(ct.read_store(SLOT_ENEMY_CORE))
        if t is not None:
            return t
        if self.mapPf.enemyCorePos is not None:
            return self.mapPf.enemyCorePos
        c = unpackPos1(ct.read_store(7) + 1) if ct.read_store(7) else None
        if c is None:
            return None
        return Position(self.mapW - 1 - c.x, self.mapH - 1 - c.y)

    def runBolt(self, ct: Controller, myLoc: Position) -> bool:
        """Committed straight march after a throw: no gunner-spot search, no
        re-riding, own stepper (landmine 2: moveTo dead-ends on our own
        conveyor ring, then waits 3-10 turns and moves RANDOMLY). Hands
        back to the normal attack state inside the firing band."""
        tgt = self._boltTarget(ct)
        if tgt is None:
            self._bolt = False
            return False
        d = abs(myLoc.x - tgt.x) + abs(myLoc.y - tgt.y)
        if d <= BOLT_BAND:
            self._bolt = False           # arrived: normal runAttack takes over
            return False
        if ct.get_move_cooldown() != 0:
            return True
        dx = tgt.x - myLoc.x
        dy = tgt.y - myLoc.y
        stepx = Direction.EAST if dx > 0 else Direction.WEST
        stepy = Direction.SOUTH if dy > 0 else Direction.NORTH
        # primary axis first, then the other, then perpendicular sidesteps
        order = ([stepx, stepy] if abs(dx) >= abs(dy) else [stepy, stepx])
        if dx == 0:
            order = [stepy, Direction.EAST, Direction.WEST]
        elif dy == 0:
            order = [stepx, Direction.SOUTH, Direction.NORTH]
        else:
            order = order + [d3 for d3 in (Direction.NORTH, Direction.SOUTH,
        ...
        for mv in order:
            if ct.can_move(mv):
                ct.move(mv)
                return True
        return True                      # boxed in: hold, try next turn

    def launcherOpening(self, ct: Controller, myLoc: Position) -> bool:
        """Pad build (first attacker, t<=5) then ride claims (any attacker,
        t<=RIDE_UNTIL). Returns True if the turn was used."""
        rnd = ct.get_current_round()
        if rnd > RIDE_UNTIL or self._rode:
            return False
        core = unpackPos1(ct.read_store(7) + 1) if ct.read_store(7) else None
        if core is None:
            return False
        # ---- pad build: one launcher, perpendicular to the enemy axis ----
        if ct.read_store(SLOT_PAD_FLAG) == 0:
            if self.mapPf.myNum != 1 or self._padTried or rnd > 5:
                return False
            tgt = self._boltTarget(ct) or Position(self.mapW // 2, self.mapH // 2)
            dx = tgt.x - core.x
            dy = tgt.y - core.y
            # perpendicular offset: if the enemy is mostly along x, pad goes
            # +-2 in y, and vice versa (landmine 3: never in the corridor)
            if abs(dx) >= abs(dy):
                cands = [Position(core.x, core.y + 2), Position(core.x, core.y - 2),
                         Position(core.x + 1, core.y + 2), Position(core.x + 1, core.y - 2)]
            else:
                cands = [Position(core.x + 2, core.y), Position(core.x - 2, core.y),
                         Position(core.x + 2, core.y + 1), Position(core.x - 2, core.y + 1)]
            for pad in cands:
                if not (0 <= pad.x < self.mapW and 0 <= pad.y < self.mapH):
                    continue
                dpad = abs(myLoc.x - pad.x) + abs(myLoc.y - pad.y)
                if dpad == 1:
                    if ct.can_act() and ct.can_build_launcher(pad):
                        ct.build_launcher(pad)
                        ct.write_store(SLOT_PAD_FLAG, 1)
                        ct.write_store(SLOT_PAD_POS, packPos1(pad))
                        return True
                    continue
                if dpad > 1 and dpad <= 4 and ct.get_move_cooldown() == 0:
                    # short direct walk, own stepper (not moveTo)
                    for mv in (Direction.NORTH, Direction.SOUTH,
                               Direction.EAST, Direction.WEST):
                        n = myLoc.add(mv)
                        if (abs(n.x - pad.x) + abs(n.y - pad.y) < dpad
                                and ct.can_move(mv)):
                            ct.move(mv)
                            return True
            self._padTried = True
            return False
        # ---- ride: claim only while ADJACENT to the pad (v11 mechanism) ----
        pad = unpackPos1(ct.read_store(SLOT_PAD_POS))
        if pad is None:
            return False
        dpad = abs(myLoc.x - pad.x) + abs(myLoc.y - pad.y)
        if dpad > 3:
            return False                 # too far: not worth the detour
        if self._rideWait >= 3:
            self._rode = True            # anti-deadlock: give up, walk (v8)
            return False
        if dpad > 1:
            if ct.get_move_cooldown() == 0:
                for mv in (Direction.NORTH, Direction.SOUTH,
                    ...
                    if (abs(n.x - pad.x) + abs(n.y - pad.y) < dpad
                        ...
                        return True
            return True
        ct.write_store(SLOT_RIDE_CLAIM, ct.get_id() + 1)
        self._rideWait += 1
        return True                      # stand and wait for the throw

    def runLauncher(self, ct: Controller) -> None:
        """Self-contained: NO mapPf (landmine 1). Throw the claimed adjacent
        builder as far toward the enemy core as legal, gain >= 3."""
        me = ct.get_position()
        claim = ct.read_store(SLOT_RIDE_CLAIM)
        if claim == 0:
            return
        rider = claim - 1
        try:
            rpos = ct.get_position(rider)
        except Exception:
            ct.write_store(SLOT_RIDE_CLAIM, 0)
            return
        if rpos is None or abs(rpos.x - me.x) + abs(rpos.y - me.y) > 2:
            return                       # not here yet; keep the claim warm
        tgt = None
        v = ct.read_store(SLOT_ENEMY_CORE)
        if v:
            tgt = unpackPos1(v)
        if tgt is None:
            c = unpackPos1(ct.read_store(7) + 1) if ct.read_store(7) else None
            if c is None:
                return
            tgt = Position(ct.get_map_width() - 1 - c.x,
                           ct.get_map_height() - 1 - c.y)
        best = None
        rd = abs(rpos.x - tgt.x) + abs(rpos.y - tgt.y)
        for tile in ct.get_nearby_tiles(26):
            try:
                if not ct.can_launch(rpos, tile):
                    continue
            except Exception:
                continue
            td = abs(tile.x - tgt.x) + abs(tile.y - tgt.y)
            if rd - td < 3:
                continue                 # must actually buy distance
            if best is None or td < best[0]:
                best = (td, tile)
        if best is not None:
            try:
                ct.launch(rpos, best[1])
            except Exception:
                pass
            ct.write_store(SLOT_RIDE_CLAIM, 0)
    ...
    def repairCandidate(self, ct: Controller, myLoc, myTeam, convDirs, convSafe):
        ...
        for key, (pos, direction) in self.knownConnectedConveyors.items():
            if key in convDirs or not ct.is_in_vision(pos):
                continue
            if not self.mapPf.inMoveZone(pos):
                continue
    ...
    def resumeCommittedRoute(self, ct: Controller) -> bool:
        ...
        if self.routeTarget is None or self.mapPf.teamCore is None:
            return False
        if not self.mapPf.inMoveZone(self.routeTarget):
