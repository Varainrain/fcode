...
class AttackBot:
    ...
    def _atk_walk_adjacent(self, ct: Controller, myLoc, tile: Position) -> bool:
        ...
        if not cands:
            return False
        # LATCH THE APPROACH TILE. This sorted by distance FROM WHERE WE ARE
        # STANDING, and that changes with every step - so two neighbours of the
        # same target keep swapping places as we walk and the attacker paces
        # between them forever. ic3d watched it "going back and forth trying to
        # destroy 2 harvesters but never ending it", and the same shape shows up
        # trying to break a barrier. The spear line hit this exact bug and a
        # latched target cut its oscillations from 27 of 50 moves to 1.
        # Keep the chosen neighbour until we arrive, it becomes unusable, or the
        # target itself changes.
        key = (tile.x, tile.y)
        held = getattr(self, 'atkApproach', None)
        if held is not None and held[0] == key and held[1] in cands                 and myLoc.distance_squared(held[1]) > 0:
            return self._atk_walk_to(ct, held[1])
        cands.sort(key=lambda p: myLoc.distance_squared(p))
        self.atkApproach = (key, cands[0])
