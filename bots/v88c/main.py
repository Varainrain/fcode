...
RECALL = 1
JAM_SLOT = 10        # jammer claim round (one jam action/round team-wide)
CB_SLOT = 11         # counter-battery claim round (one builder/round)
SENT_BCAST_SLOT = 12 # core-confirmed sentinel threats, 2x (dx,dy) nibble pairs
SEAL_CAP_SLOT = 15   # total seal barriers built this game (cap 20 = 60 Ti)
SEAL_BUILD_CAP = 20
SEAL_SLOT = 13       # seal-duty claim round (10-15 free)
HARV_SLOT = 14       # team harvesters built (income-first policy counter)
HARVEST_TARGET = 7   # sporks fields 7-13; we measured 1-4 in the same games
HARVEST_UNTIL = 300  # time bound: the boost is an OPENING policy, not forever
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        # publish enemy SENTINELS inside firing geometry of the core to
        # slot 12 (typed channel; slot 7 mixes gunners in and skips turrets
        # with a builder nearby). Builders' vision 20 never sees d5-9 parks;
        # core vision 36 covers every legal firing tile (32).
        slot12 = 0
        nSent = 0
        for b in ct.get_nearby_buildings():
            if nSent >= 2:
            ...
            dy = sPos.y - myLoc.y
            if -8 <= dx <= 7 and -8 <= dy <= 7:
                slot12 |= (((dx & 0xF) << 4) | (dy & 0xF)) << (8 * nSent)
                nSent += 1
        ct.write_store(SENT_BCAST_SLOT, slot12)
        ...
        if RECALL:
            hurt = 1 if ct.get_hp() < RECALL_HP else 0
            # EARLY PACK-RECALL: SmartFridge's hyper-rush (2 barrier-armored
            # gunners at d2-3 by t13, 14 dmg/turn, core dead t42) beat the
            # hp<400 trigger by arithmetic -- healers arrived ~t28 at 12/turn
            # and our core healed 8 hp all game. The core SEES its attackers
            # (vision 36): >=2 enemy turrets in firing range before t120 =
            # sound the horn NOW. 4-5 healers at 16-20/turn out-heal the
            # pair; their eco-less all-in starves if the core survives.
            if (not hurt and ct.get_current_round() < 120
                    and ct.get_hp() < ct.get_max_hp()):
                # ...but only once the first shot has LANDED: a parked-but-
                # blocked battery must not lock healthy eco at home
                # (measured: sporkmic's sentinel parks false-alarmed 5
                # builders into 120 turns of nursing an undamaged core)
                nAtk = 0
                for b in ct.get_nearby_buildings():
                    if ct.get_team(b) == myTeam:
                        continue
                    if ct.get_entity_type(b) not in (EntityType.GUNNER, EntityType.SENTINEL):
                        continue
                    aPos = ct.get_position(b)
                    if min(aPos.distance_squared(Position(myLoc.x + ox, myLoc.y + oy))
                           for ox in (0, 1) for oy in (0, 1)) <= 36:
                        nAtk += 1
                        if nAtk >= 2:
                            hurt = 1
    ...
    def coreThreats(self, enemyTurrets):
        ...
        if core is None:
            return []
        out = []
        for g in enemyTurrets:
            d2 = min(g.distance_squared(Position(core.x + ox, core.y + oy))
                     for ox in (0, 1) for oy in (0, 1))
            if d2 <= 36:                       # sentinel firing range is 32
                out.append(g)
        return out

    def _rayPlug(self, sent, core):
        """First empty tile on the sentinel->core ray, adjacent to the
        sentinel. None if the sentinel is not ray-aligned with any core
        tile (then it cannot hit the core at all)."""
        for ox in (0, 1):
            for oy in (0, 1):
                c = Position(core.x + ox, core.y + oy)
                vx, vy = c.x - sent.x, c.y - sent.y
                if vx == 0 and vy == 0:
                    continue
                if vx != 0 and vy != 0 and abs(vx) != abs(vy):
                    continue                   # not a firing line
                sx = (vx > 0) - (vx < 0)
                sy = (vy > 0) - (vy < 0)
                return Position(sent.x + sx, sent.y + sy)
        return None

    def _jamVetoed(self, ct: Controller, myTeam, plug) -> bool:
        """ESCORT VETO (the drumlin death-march lesson): gunners near the
        plug shred builders -- that fight belongs to CB + seal, not us."""
        try:
            for b in ct.get_nearby_buildings():
                if (ct.get_team(b) != myTeam
                        and ct.get_entity_type(b) == EntityType.GUNNER
                        and ct.get_position(b).distance_squared(plug) <= 20):
                    return True
        except Exception:
            return True
        return False

    def jamTask(self, ct: Controller, myLoc, threats) -> bool:
        """ic3d's jammer: keep a barrier on the parked sentinel's muzzle.
        3 Ti per 20 Ti of their ammo, forever, from behind the wall.
        Review-hardened: an out-of-vision plug means APPROACH (the radar
        case -- tile queries RAISE outside vision), candidates are vetted
        before commitment, unjammable muzzles are skip-listed."""
        ...
        if core is None:
            return False
        myTeam = ct.get_team()
        tKeys = set((t.x, t.y) for t in threats)
        # release: confirmed dead (visible empty/ours) OR vanished from
        # both vision and radar (the radar stops publishing dead parks)
        if self.jamSent is not None:
            sp = Position(self.jamSent[0], self.jamSent[1])
            gone = False
            try:
                if ct.is_in_vision(sp):
                    bid = ct.get_tile_building_id(sp)
                    gone = bid is None or ct.get_team(bid) == myTeam
                elif (sp.x, sp.y) not in tKeys:
                    gone = True
            except Exception:
                pass
            if gone:
                self.jamSent = None
        # engage: vet BEFORE committing -- a disqualified nearest threat
        # must not starve a jammable one behind it
        if self.jamSent is None:
            for t in sorted(threats, key=lambda t: abs(t.x - myLoc.x) + abs(t.y - myLoc.y)):
                if (t.x, t.y) in self.jamSkip:
                    continue
                p_ = self._rayPlug(t, core)
                if p_ is None:
                    continue
                if not (0 <= p_.x < self.mapW and 0 <= p_.y < self.mapH):
                    continue
                if abs(myLoc.x - p_.x) + abs(myLoc.y - p_.y) > 6:
                    continue                   # out of leash: someone closer
                if self._jamVetoed(ct, myTeam, p_):
                    continue
                self.jamSent = (t.x, t.y)
                break
        if self.jamSent is None:
            return False
        sent = Position(self.jamSent[0], self.jamSent[1])
        plug = self._rayPlug(sent, core)
        if plug is None or not (0 <= plug.x < self.mapW and 0 <= plug.y < self.mapH):
            self.jamSent = None
            return False
        if self._jamVetoed(ct, myTeam, plug):
            self.jamSent = None                # escort arrived: stand down
            return False
        d1 = abs(myLoc.x - plug.x) + abs(myLoc.y - plug.y)
        if d1 > 6:
            self.jamSent = None                # drifted too far -- release
            return False
        plugVisible = False
        try:
            if ct.is_in_vision(plug):
                plugVisible = True
                if ct.get_tile_env(plug) != Environment.EMPTY:
                    self.jamSkip.add((sent.x, sent.y))
                    self.jamSent = None        # muzzle unbuildable forever
                    return False
                bid = ct.get_tile_building_id(plug)
                if bid is not None:
                    if ct.get_team(bid) != myTeam:
                        self.jamSkip.add((sent.x, sent.y))
                        self.jamSent = None    # enemy building on the muzzle
                    return False               # ours alive: free for eco
        except Exception:
            return False
        rnd = ct.get_current_round()
        try:
            if ct.read_store(JAM_SLOT) >= rnd:
                return False
            if ct.read_store(SEAL_CAP_SLOT) >= SEAL_BUILD_CAP:
                return False                   # shared defensive budget
        except Exception:
            return False
        if plugVisible and d1 == 1 and ct.can_build_barrier(plug):
            ct.write_store(JAM_SLOT, rnd + 1)
            ct.write_store(SEAL_CAP_SLOT, ct.read_store(SEAL_CAP_SLOT) + 1)
            ct.build_barrier(plug)
            return True
        if d1 == 0:
            ct.write_store(JAM_SLOT, rnd + 1)
            self.mapPf.moveTo(ct, self.mapPf.teamCore)  # own-tile trap: step off
            return True
        ct.write_store(JAM_SLOT, rnd + 1)
        self.drawState(ct, C_FIGHT_TURRET, plug)
        self.mapPf.moveTo(ct, plug)            # approach -- incl. the radar case
        return True

    def counterBatteryTask(self, ct: Controller, myLoc, threats) -> bool:
        """OUR sentinel vs their parked sentinel: range parity plus home-turf
        heals = we win every artillery duel we accept. One claimant/round
        (slot 11), max 2 defensive sentinels alive, threat-triggered only."""
        if not threats or ct.get_global_resources() < ct.get_sentinel_cost() + 10:
            return False
        core = self.mapPf.teamCore
        if core is None:
            return False
        ...
        try:
            for b in ct.get_nearby_buildings():
                if (ct.get_team(b) == myTeam
                        ...
                        and coreFootprintManhattan(ct.get_position(b), core) <= 5):
                    defSents += 1
        except Exception:
            return False
        if defSents >= 2:
            return False
        threat = min(threats, key=lambda t: coreFootprintManhattan(t, core))
        rnd = ct.get_current_round()
        try:
            if ct.read_store(CB_SLOT) >= rnd:
                return False
        except Exception:
            return False
        for d in CARDINALS:
            ...
            if not (0 <= spot.x < self.mapW and 0 <= spot.y < self.mapH):
                continue
            if not (1 <= coreFootprintManhattan(spot, core) <= 4):
                continue                       # close enough for heal support
            if spot.distance_squared(threat) > 32:
                continue                       # must actually reach the threat
            try:
                if ct.get_tile_building_id(spot) is not None:
                    continue
                facing = spot.direction_to(threat)
                if ct.can_build_sentinel(spot, facing):
                    ct.write_store(CB_SLOT, rnd + 1)
                    ct.build_sentinel(spot, facing)
                    return True
            except Exception:
                continue
        # no adjacent spot: walk toward the threat-side of the core
        anchor = None
        best = None
        for sp in self.sealTiles():
            dd = abs(sp.x - threat.x) + abs(sp.y - threat.y)
            if best is None or dd < best:
                best, anchor = dd, sp
        if anchor is None:
            return False
        d1 = abs(myLoc.x - anchor.x) + abs(myLoc.y - anchor.y)
        if d1 > 5 or d1 == 0:
            return False
        ct.write_store(CB_SLOT, rnd + 1)
        self.drawState(ct, C_FIGHT_TURRET, anchor)
        self.mapPf.moveTo(ct, anchor)
        return True
    ...
    def sealTask(self, ct: Controller, myLoc, threats) -> bool:
        ...
        try:
            if ct.read_store(SEAL_CAP_SLOT) >= SEAL_BUILD_CAP:
                return False                   # 23-built/19-shredded lesson:
        except Exception:                      # never fund an ammo war forever
            return False
    ...
    def entombTask(self, ct: Controller, myLoc) -> bool:
        ...
        GRAVEDIGGER MODE (the not-adgato answer): their heal wall repairs
        100% of siege damage (measured 1771/1771, 826/826, 595/595 -- ten
        games, perfect repair; the ONE game our damage beat their heal we
        won). When this builder watches the enemy core hp fail to drop for
        30 turns of siege, opportunistic capping isn't enough: it walks the
        heal ring DELIBERATELY and caps every empty cardinal tile. 8 heal
        spots exist; killing 6 turns their 24+ hp/turn repair into <=8 vs
        our sustained 9+/turn -- the math flips. Enemy builders can only
        shoot buildings for 2 dmg: removing one 3-Ti cap costs them 15
        shots and 30 Ti of NOT healing."""
        graves = 10 if self.sgStall >= 30 else 4
        if self.entombPlaced >= graves or ct.get_global_resources() < 30:
            return False
        ...
        myTeam = ct.get_team()
        # heal-stall detector: enemy core visible and hp not dropping
        try:
            cid = ct.get_tile_building_id(ec)
            if cid is not None and ct.get_team(cid) != myTeam:
                hp = ct.get_hp(cid)
                if self.sgLastHp is not None and hp >= self.sgLastHp:
                    self.sgStall += 1
                else:
                    self.sgStall = max(0, self.sgStall - 3)
                self.sgLastHp = hp
        except Exception:
            pass
        siegeTurrets = []
        ...
        for d in CARDINALS:
            ...
            try:
                ...
                continue
        # gravedigger walk: no adjacent gap, heal-stall confirmed -- go to
        # the nearest empty CARDINAL heal tile and cap it next turn
        if self.sgStall >= 30:
            best = None; bestD = 99
            for ox, oy in ((-1, 0), (2, 0), (-1, 1), (2, 1),
                           (0, -1), (1, -1), (0, 2), (1, 2)):
                q = Position(ec.x + ox, ec.y + oy)
                if not (0 <= q.x < self.mapW and 0 <= q.y < self.mapH):
                    continue
                try:
                    ...
                    if ct.get_tile_building_id(q) is not None:
                        continue
                    if any(self._onCoreRay(g, coreTiles, q) for g in siegeTurrets):
                        continue
                except Exception:
                    continue
                d1 = abs(myLoc.x - q.x) + abs(myLoc.y - q.y)
                if 0 < d1 < bestD:
                    bestD, best = d1, q
            if best is not None and bestD <= 5:
                self.drawState(ct, C_MARCH_ATTACK, best)
    ...
    def runAttack(self, ct: Controller):
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            if self.siegeSentinelTask(ct, myLoc):
                return
    ...
    def siegeSentinelTask(self, ct: Controller, myLoc) -> bool:
        """The Pantheon formula (d96022a1): ONE eco-backed sentinel parked
        in firing range of the enemy core out-damages any heal wall
        (9/turn vs sporks' measured 5.2/turn). Attackers try this before
        gunner spam; the gunner escort forms behind it as normal."""
        ec = self.mapPf.enemyCorePos
        if ec is None:
            return False
        if ct.get_global_resources() < ct.get_sentinel_cost() + 40:
            return False                       # escort + heal budget stays
        myTeam = ct.get_team()
        coreTiles = [ec, ec.add(Direction.EAST), ec.add(Direction.SOUTH),
                     ec.add(Direction.SOUTH).add(Direction.EAST)]
        try:
            for b in ct.get_nearby_buildings():
                if (ct.get_team(b) == myTeam
                        and ct.get_entity_type(b) == EntityType.SENTINEL
                        and min(ct.get_position(b).distance_squared(c)
                                for c in coreTiles) <= 32):
                    return False               # one parked siege gun, like Pantheon
        except Exception:
            return False
        for d in CARDINALS:
            spot = myLoc.add(d)
            ...
            aligned = None
            for c in coreTiles:
                vx, vy = c.x - spot.x, c.y - spot.y
                if vx == 0 and vy == 0:
                    aligned = None
                    break                      # ON the core footprint: no
                if (vx == 0 or vy == 0 or abs(vx) == abs(vy)) \
                        and spot.distance_squared(c) <= 32:
                    aligned = c
                    break
            if aligned is None:
                continue
            try:
                if ct.get_tile_building_id(spot) is not None:
                    continue
                facing = spot.direction_to(aligned)
                if ct.can_build_sentinel(spot, facing):
                    ct.build_sentinel(spot, facing)
                    return True
            except Exception:
    ...
    def runEco(self, ct: Controller):
        ...
        try:
            _s12 = ct.read_store(SENT_BCAST_SLOT)
            _home = self.mapPf.teamCore
            if _s12 and _home is not None:
                _seen = set((s.x, s.y) for s in enemySentinels)
                for _i in (0, 1):
                    _b = (_s12 >> (8 * _i)) & 0xFF
                    if _b == 0:
                        continue
                    ...
                    if _dy >= 8:
                        _dy -= 16
                    _sp = Position(_home.x + _dx, _home.y + _dy)
                    if (_sp.x, _sp.y) not in _seen:
                        enemySentinels.append(_sp)
        except Exception:
            pass
        ...
        _sentThreats = self.coreThreats(enemySentinels)
        if self.counterBatteryTask(ct, myLoc, _sentThreats): # kill the artillery
            return

        if self.jamTask(ct, myLoc, _sentThreats): # plug the muzzle (3 Ti vs 20 Ti ammo)
            return

        # GUNNER PACKS are sieges, not harass: >=2 gunners inside the core
        # threat radius trigger the seal too (their range is only 13 -- a d2
        # barrier hard-blocks the ray). SINGLE gunners stay with the chassis
        # turret-response: the drumlin 0/10 came from builders chasing lone
        # harassers, and this threshold is what keeps that fixed. 9-loss
        # autopsy: 6 of 9 rated losses opened with a gunner pack at d3-6 and
        # seal/CB read zero because everything was sentinel-scoped.
        _core = self.mapPf.teamCore
        _sealThreats = _sentThreats
        if _core is not None:
            _packGuns = [g for g in enemyTurrets
                         if coreFootprintManhattan(g, _core) <= 5]
            if len(_packGuns) >= 2:
                _sealThreats = _sentThreats + _packGuns

        if self.sealTask(ct, myLoc, _sealThreats): # reactive seal (sentinels + gunner packs)
            return

        # INCOME BEFORE PLUMBING: harvestTask sat DEAD LAST in this chain, so
        # any unfinished conveyor line starved new harvester construction.
        # Measured vs sporks (96ba6136): our h1-h4 vs their h7-h13, our cv31
        # vs their cv93 -- we were not losing the eco war, we were not
        # FIGHTING it. A harvester is income; a conveyor is only plumbing for
        # income that already exists. Time-bounded opening policy.
        try:
            _nh = ct.read_store(HARV_SLOT)
        except Exception:
            _nh = HARVEST_TARGET
        # THE PROSPECTOR (role split, not a priority flip). Measured the hard
        # way: making ALL eco builders dig first built 6 harvesters and mined
        # ZERO -- routeConveyorTask is what EXTENDS chains to the core, so
        # starving it leaves every harvester on a stub. So exactly ONE eco
        # builder per team (myNum % 4 == 2) prospects while the rest plumb
        # normally. Income and plumbing both keep running.
        # MAP GATE (measured, 15-map sweep): the prospector WINS eco-race maps
        # (hive d36 9/10, jackpot d28 9/10, saga d28 8/10) and LOSES tempo
        # maps (drumlin d26 0/10, nordkap d12 1/10, moonrise d9 0/10) -- on
        # close-core maps the diverted builder is a body lost from the fight
        # and the extra harvesters never connect. Cores far apart == the game
        # is decided by income; cores close == decided by tempo.
        _ecoRace = False
        _ec = self.mapPf.enemyCorePos
        _hc = self.mapPf.teamCore
        if _ec is not None and _hc is not None:
            _ecoRace = abs(_ec.x - _hc.x) + abs(_ec.y - _hc.y) >= 27
        if (_ecoRace and self.mapPf.myNum % 4 == 2 and _nh < HARVEST_TARGET
                and ct.get_current_round() < HARVEST_UNTIL):
    ...
    def harvestTask(self, ct: Controller, myLoc, myTeam) -> bool:
        ...
        if bestTile is not None:
            ...
            if ct.can_build_harvester(bestTile):
                ...
                try:                       # store lag can overshoot by the
                    ct.write_store(HARV_SLOT, ct.read_store(HARV_SLOT) + 1)
                except Exception:          # number of same-round builders --
                    pass                   # harmless for a threshold policy
            return True
        return False