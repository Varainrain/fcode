...
SENT_BCAST_SLOT = 12 # core-published sentinel threats, 2x (dx,dy) nibbles
SEAL_CAP_SLOT = 15   # defensive barriers built this game (cap = 60 Ti total)
SEAL_BUILD_CAP = 20
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        # RADAR: builder vision^2 is 20 (~4.5 tiles) so a sentinel parked at
        # 5-6 tiles is invisible to them -- that is how 2d0e6c04 g3 was lost.
        # CORE vision^2 is 36, which covers every tile a sentinel can legally
        # fire from (32). The core is the right sensor; publish what it sees.
        slot12 = 0
        nSent = 0
        for b in ct.get_nearby_buildings():
            if nSent >= 2:
    ...
    def sealTask(self, ct: Controller, myLoc, threats) -> bool:
        ...
        try:
            if ct.read_store(SEAL_CAP_SLOT) >= SEAL_BUILD_CAP:
                return False       # never fund an ammo war forever: their
        except Exception:          # shots cost 10 Ti, our barriers 3 -- but
            return False           # only while the budget lasts
    ...
    def runBestState(self, ct: Controller):
        ...
        # DEFENSE BAND: only fires while a sentinel is parked inside firing
        # range of our core (rare) -- and then it outranks everything except
        # protect/healCore, because a parked sentinel that is never answered
        # is simply a timer on our core (1618 dmg in 2d0e6c04 g3).
        _sentThreats = []
        try:
            ...
            for b in ct.get_nearby_buildings():
                if (ct.get_team(b) != myTeam
                        and ct.get_entity_type(b) == EntityType.SENTINEL):
                    _seen.append(ct.get_position(b))
            _s12 = ct.read_store(SENT_BCAST_SLOT)
            if _s12 and _home is not None:
                _known = set((s.x, s.y) for s in _seen)
                for _i in (0, 1):
                    _b = (_s12 >> (8 * _i)) & 0xFF
                    if _b == 0:
                        continue
                    ...
                    if (_sp.x, _sp.y) not in _known:
                        _seen.append(_sp)
            _sentThreats = self.coreThreats(_seen)
        except Exception:
            _sentThreats = []

        # TRIGGER (measured twice, both ways):
        #  - firing at ANY sentinel cost 16 pts vs the rush bench (69->53):
        #    during an opening assault, tempo + the chassis' protect state
        #    beat diverting a builder to a 30-Ti sentinel.
        #  - gating on "no gunner escort" recovered the bench BUT would have
        #    stayed silent for all 150 turns of 2d0e6c04 g3 (Lorem parked a
        #    gunner at d3 the whole game) -- i.e. it would not have fixed the
        #    match it was built for. Rejected.
    ...
    def protectTask(self, ct: Controller, tatrget: Position):
