...
CB_SLOT = 11         # counter-battery claim round
SENT_BCAST_SLOT = 12 # core-confirmed sentinel threats, 2x (dx,dy) nibble pairs
SEAL_CAP_SLOT = 15   # total defensive barriers built this game (cap 20 = 60 Ti)
SEAL_BUILD_CAP = 20
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if self.numSpawned < 5 or (globalTitanium > 360
            ...
            if spawnableTiles:
                ...
                if nextNum % 2 == 1 and nextNum not in (5, 7): # attacking bots
                    ...
                    sortTarget = nearbyOres[((nextNum-1) //2) % len(nearbyOres)]
        ...
        # sentinel radar: builders' vision (20) never sees d5-9 artillery
        # parks; core vision (36) covers every legal firing tile (32)
        slot12 = 0
        nSent = 0
        for b in ct.get_nearby_buildings():
            if nSent >= 2:
        ...
        if RECALL:
            ...
            # EARLY HORN: >=2 enemy turrets in firing range before t120 AND
            # first damage landed -> recall NOW (healers by ~t14 beat the
            # 14 dmg/turn gunner pair; waiting for hp<400 measured 8 hp of
            # total healing in a t42 loss). Damage-landed guard keeps parked
            # -but-blocked batteries from false-alarming healthy eco.
    ...
    def runBestState(self, ct: Controller):
        """MEMORYLESS STATE SCORING (OogwayGeneralist architecture): every
        turn every eco state is scored from observable state alone, and the
        argmax is executed. No persistent intent, no fixed priority chain.
        The caps encode urgency (protect > heal > route > harvest); the
        distance/damage multipliers let a nearby urgent lower state beat a
        far marginal higher one. Per-state gates (move zone, coverage,
        closest-ally, affordability) live inside each scorer."""
        myLoc = ct.get_position()
        myTeam = ct.get_team()
        ...
        # SMART BIDDING (the v84 lesson): fixed-chain defense taxed the
        # mirror -20%; deleting it got swept 0-5 by sporks. Defense bids
        # ONLY while the core is actually losing the heal war (memoryless:
        # hp < 85% of max, re-read every turn).
        _defArmed = False
        try:
            if coreId is not None and ct.get_hp(coreId) < int(0.85 * ct.get_max_hp(coreId)):
                _defArmed = True
        except Exception:
            pass
        ...
        if _defArmed and bestScore < 10.0:
            if self.counterBatteryTask(ct, myLoc, _sentThreats):
                return
            if self.jamTask(ct, myLoc, _sentThreats):
                return
            if self.sealTask(ct, myLoc, _sealThreats):
                return
