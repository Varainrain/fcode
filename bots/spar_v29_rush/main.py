...
class Player:
    ...
    def builderBot(self, ct: Controller):
        ...
        # SPARRING PARTNER, NEVER A SUBMISSION. spar_rush is obsolete: v29 beats
        # it 97%, because it was built from the v8-era engine. We still have no
        # local opponent that kills fast on a COMPETITIVE chassis, which is the
        # blind spot that has made every tempo experiment unmeasurable - the
        # matchup we actually lose (Pantheon, first gun t7) has no proxy here.
        # This is v29 with almost every builder attacking: same pathfinder, same
        # seat selection, same engine quality, maximum tempo. Do not submit it.
        if self.mapPf.myNum != 2: # attacking (spar profile: all but one)
