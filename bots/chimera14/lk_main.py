...
class Player:
    ...
    def run(self, ct: Controller) -> None:
        ...
        if etype == EntityType.CORE:
            ...
            # ---------------- LAUNCHER-KICK: disrupt a passing enemy builder ----
            # Only while still travelling (no seat claimed yet - don't abandon a
            # held seat for this), only with titanium to spare, only a SHORT
            # detour, and only if no defending gunner/sentinel already covers the
            # build spot (a lone launcher next to a defended economy dies in ~5
            # rounds to a single gunner - measured live via bots/_lktest probe).
            # One attempt per game: win, get denied by a defender, or give up
            # after LKICK_MAX_CHASE_TURNS - either way stop re-detouring.
                    ...
                    if bestDsq <= 2:
                        ...
                        for d in DIRECTIONS:
                            ...
                            if (spot.x, spot.y) in covered:
                                continue   # a defender covers this spot - don't feed it a free kill
                            try:
                        ...
                        if built:
                            return
                        # not built (covered, or no free adjacent spot) - fall
                        # through to the normal rush logic below this turn
                    else:
                        self.lkickChaseTurns += 1
                        if self.lkickChaseTurns > LKICK_MAX_CHASE_TURNS:
                            self.lkickDone = True   # not closing the gap - stop burning rush tempo
                        else:
            ...
            if self.lastCoreHp is not None:
