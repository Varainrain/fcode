...
class Player(eco.EcoBot, attack.AttackBot, gunner.GunnerBot):
    ...
    def run(self, ct: Controller) -> None:
        ...
        if etype == EntityType.CORE:
            ...
            # Without this a placed gunner is 20 Ti of scenery. There is no
            # SENTINEL branch because the API has no get_sentinel_target(), so
            # none are ever built.
            self.run_gunner(ct)
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        spare = ct.get_global_resources() - 0
