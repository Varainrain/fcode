...
S_HEAL_CORE  = _fp("S_HEAL_CORE", 6.0)   # WILD: healing at 4hp/1Ti is cheap but
                                         # it is also PASSIVE -- outranking eco
                                         # with it is how the nursing spiral starts     # any visible damaged core
...
class Player:
        ...
        if self.numSpawned < 5 or (globalTitanium > 360
            ...
            if spawnableTiles:
                ...
                if nextNum % 3 == 0: # attacking bots
                    ...
                    sortTarget = nearbyOres[(nextNum - 2 - nextNum//3) % len(nearbyOres)]
    ...
    def builderBot(self, ct: Controller):
        ...
        if RECALL:
            try:
                ...
                pass
        if self.mapPf.myNum % 3 == 0: # attacking (1/4)
    ...
    def _bldCount(self, ct: Controller, myTeam) -> int:
        n = 0
        try:
            for b in ct.get_nearby_buildings():
                if ct.get_team(b) == myTeam:
    ...
    def scoreRouteConv(self, ct: Controller, myLoc, myTeam):
        ...
        # EVERY building taxes EVERY future turret (+1% each). Past a dense
        # local cluster, another conveyor costs more than it carries.
        if self._bldCount(ct, myTeam) >= 22:
            return 0, None
        for b in ct.get_nearby_buildings():
