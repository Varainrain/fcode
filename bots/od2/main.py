...
class Player:
    ...
    def buildGunnerFor(self, ct: Controller, target: Position) -> bool:
        ...
        for d in CARDINALS:
            spot = myLoc.add(d)
            # DEFENSE MODULE step 1 — entombment veto (Besvikomat 4-1
            # receipt; 0788a40d: our own buildings sealed the core pocket,
            # 14 builders parked 450 turns). No-ops beyond d3 from core.
            if (self.mapPf.teamCore is not None
                    and DA.would_entomb(ct, self.mapPf.teamCore, spot,
                                        ct.get_team(), self.mapW, self.mapH)):
                continue
    ...
    def runEco(self, ct: Controller):
        ...
        # DEFENSE MODULE step 2 — split-duty heal claim (Oops 4-1 receipt,
        # two t1000 tiebreak survivals). The fight block below returns
        # during any siege, making the heal branch unreachable exactly when
        # it matters — the third recurrence of the heal-zero bug family.
        # ONE unit per round claims heal duty (slot 14); the rest fight on.
        if (self.mapPf.teamCore is not None
                and DA.claim_heal_duty(ct, self.mapPf.teamCore)):
