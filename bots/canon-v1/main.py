...
class Player:
    ...
    def _canonicalFor(self, ct: Controller, myLoc: Position, target: Position,
        ...
        """True if I am the ally builder that should act on `target` —
        the closest one, ties broken by lower id. Cheap, needs no comms:
        every bot computes the same answer from its own vision."""
    ...
    def runBestState(self, ct: Controller, myLoc: Position):
        ...
        if self.attackBan == 0:
            if ct.get_global_resources() > 120 and ct.get_id() > 4: # dont attack if u r broke and leave one bot for defense
                ...
                # Cambridge team). This bot has NO claim system — every
                # builder that sees a target independently decides to go
                # hit it, so several pile onto the same thing. attackBan is
                # the crude stand-in: after merely SEEING a friendly gunner
                # it refuses to attack anything for 4-12 turns. Instead:
                # only the builder CLOSEST to a given target acts on it
                # (tiebreak by id, so both sides agree without messaging).
                # Others fall through to economy or pick a different target.
