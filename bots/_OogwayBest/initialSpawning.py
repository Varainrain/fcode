...
class initialSpawn:
    ...
    def targetFor(self, angle, coreCorners):
        index = anglePerDir.index(angle)
        start = Position(coreCorners[index // 4].x + spawnPoints[index].x,
                        coreCorners[index // 4].y + spawnPoints[index].y)
        return Position(start.x + directionMoves[index].x,
                        start.y + directionMoves[index].y)
    ...
    def setBestFive (self, ct: Controller):
        ...
        center = Position(mapW // 2, mapH // 2)
        angles = self.pickBestRays(ct, rayScores, coreCorners)
        angles.sort(key=lambda a: self.targetFor(a, coreCorners).distance_squared(center))
        return angles