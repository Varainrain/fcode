...
symSlot = 8 # slots 0-7 are taken (0 numSpawned, 1-6 map sharing, 7 initial target)
symBits = {'flipX': 1, 'flipY': 2, '180': 4} # store *killed* symmetries so 0 = nothing ruled out

openCost = 1
barrierCost = 8
gunnerCost = 30 # gunners cost more, so higher
...
class MapPathfinder:
    ...
    def checkPassable (self, ct: Controller, tile: Position): # Only team barriers, or empty tiles are passbale
        ...
        if tileId is not None and not ct.is_tile_passable(tile):
            ...
            if tTeam != ct.get_team() or (tType != EntityType.BARRIER):
                return False
        return True
    def addWall(self, tile: Position): # keyed so re-seeing a tile every turn cant duplicate it
        key = tile.x * 32 + tile.y
        if key not in self.wallKeys:
            self.wallKeys[key] = True
            self.allWalls.append(tile)

    def addOre(self, tile: Position):
        key = tile.x * 32 + tile.y
        if key not in self.oreKeys:
            self.oreKeys[key] = True
            self.allOres.append(tile)
    ...
    def getNewTiles (self, ct: Controller):
        for tile in ct.get_nearby_tiles():
            ...
            if tileEnv == Environment.WALL:
                self.addWall(tile)
            elif tileEnv == Environment.ORE_TITANIUM:
                self.addOre(tile)
    ...
    def updateBoard (self, ct: Controller):
        ...
        for i in range(16):
            ...
            if not self.seenTiles[x][y] or self.fullMap[x][y] != t_type:
                ...
                if t_type == 2:
                    self.addWall(Position(x, y))
                elif t_type == 1:
                    self.addOre(Position(x, y)) # ore another bot found counts too
                self.mapChanged = True
            ...
            if tileId is not None:
                ...
                if tType == EntityType.CONVEYOR:
                    ...
                    if ct.get_stored_resource(tileId) is not None:
                        return [28, 'done']  # loaded
                    if endDist > otherDist:
                        return [48, 'done']
                    return [4, 'working']  # empty
