...
SLOT_HOME_THREAT = 15
TEAM_CORE_SLOT = 6 # core-written every turn: its own position. Gunners never call
# setupMap(), so they read our core's spot from here instead of mapPf.teamCore.
...
RECALL_SLOT = 12 # core-written: 1 while under attack and low HP, else 0
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        ct.write_store(TEAM_CORE_SLOT, ct.get_position().x * 32 + ct.get_position().y)
    ...
    def runBestState(self, ct: Controller, myLoc: Position):
        ...
        # attack, max score of 15 (core threat) / 10
        attackScore, attackPos = self.scoreAttack(ct, myLoc, myTeam, nearbyUnits)

        # retreat, max score of 9 (core recall)
    ...
    def scoreAttack(self, ct: Controller, myLoc: Position, myTeam, nearbyUnits):
        # attack, max score of 15 (core threat) / 10
        attackScore = 0
        attackPos = None
        if ct.get_global_resources() > 10:
            coreThreat = self.nearestEnemyGunner(ct, ct.get_team())
            if coreThreat is not None:
                attackScore = 15
