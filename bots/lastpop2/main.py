...
DELTA_DIR = {(0, -1): Direction.NORTH, (0, 1): Direction.SOUTH,
             (1, 0): Direction.EAST, (-1, 0): Direction.WEST}

# --- lastpopperian_ clone parameters, measured from 5 of their wins ---
# (see bots/lastpop/main.py docstring for the full replay table)
CREW = 3            # they field only 3-4 builders ALL GAME; we run 5-8
LATE_BUILDER = 70   # their 4th appeared t73-83
SENT_ROUND = 5      # one home sentinel, seen t4-13
SENT_OUT = 4        # ~4-5 tiles out on the enemy-facing side
SENT_SLOT = 8       # free store slot: 1 once the sentinel is placed
SIEGE_SLOT = 9      # free store slot: how many builders have left to siege
SIEGE_START = 45    # their attack guns land t59-84; travel takes ~20-40
SIEGE_CREW = 2      # one or two builders make the trip, never more
SIEGE_MIN = 2       # their guns sat at manhattan 2-5 from our core...
SIEGE_MAX = 5
...
class Player:
    def __init__(self):
        ...
        self.turnsAlive = 0
        self.attackBan = 0
        self.mySpawnIdx = None    # clone: spawn order (0-based)
        self.sentDone = False     # clone: the one home sentinel
        self.sieger = None        # clone: None=undecided, True=on the siege
        self.mapW = None
        self.mapH = None
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if (self.fiveDirections and len(self.fiveDirections) > 0
                and self.numSpawned < CREW):   # clone: lean crew
        ...
        # LEAN CREW — the single biggest difference from us. They spend
        # the titanium a 5th/6th builder would cost on conveyors and guns.
        _cap = CREW + (1 if ct.get_current_round() >= LATE_BUILDER else 0)
        if self.numSpawned < _cap and globalTitanium > 80 + 60 * self.numSpawned:
            for i in ct.get_nearby_tiles():
                if ct.can_spawn(i):
                    ct.spawn_builder(i)
                    self.numSpawned += 1          # clone: this branch never
                    ct.write_store(0, self.numSpawned)  # counted, so the cap
    ...
    def builderBot(self, ct: Controller):
        ...
        self.turnsAlive += 1
        if self.mySpawnIdx is None:
            # core increments slot 0 BEFORE the new builder runs, so the
            # first builder reads 1 — normalise to 0-based
            self.mySpawnIdx = max(0, ct.read_store(0) - 1)
        # Claim the sentinel job through a FREE store slot (8-15 are
        # unused by this bot — STORE_SIZE is 16). Deriving it from spawn
        # index races: a builder born at t0 first runs at t1, by which
        # point the core may have bumped the counter already.
        # THE SIEGE — the actual killing mechanism, and the part a
        # "champion with fewer builders" does not reproduce. At t45 one or
        # two builders permanently leave the economy, walk to their core,
        # and seat gunners at manhattan 2-5 from it, DIAGONALS FIRST (an
        ...
        if (ct.read_store(SENT_SLOT) == 0
                and ct.get_current_round() >= SENT_ROUND
                and self.mapPf.teamCore is not None
                and not self.sentDone):
            if self.homeSentinel(ct, myLoc):
                return
    ...
    def doSiege(self, ct: Controller, myLoc: Position) -> None:
        tc = self.mapPf.teamCore
        if tc is None:
            return
        target = Position(self.mapW - 1 - tc.x, self.mapH - 1 - tc.y)
        for b in ct.get_nearby_buildings():
            try:
                if (ct.get_entity_type(b) == EntityType.CORE
                        and ct.get_team(b) != ct.get_team()):
                    target = ct.get_position(b)
                    break
            except Exception:
                pass
        if abs(myLoc.x - target.x) + abs(myLoc.y - target.y) > SIEGE_MAX + 3:
            self.mapPf.moveTo(ct, target)
            return
        seat = None
        for t in ct.get_nearby_tiles():
            d = abs(t.x - target.x) + abs(t.y - target.y)
            if not (SIEGE_MIN <= d <= SIEGE_MAX):
                continue
            try:
            ...
            key = (0 if diag else 1,
                   abs(myLoc.x - t.x) + abs(myLoc.y - t.y))
            if seat is None or key < seat[0]:
                seat = (key, t, face)
        if seat is not None:
            _, spot, face = seat
            if abs(myLoc.x - spot.x) + abs(myLoc.y - spot.y) == 1:
                if (ct.can_act()
                        and ct.get_global_resources() > ct.get_gunner_cost() + 5
                        and ct.can_build_gunner(spot, face)):
                    ct.build_gunner(spot, face)
                return
            self.mapPf.moveTo(ct, spot)
            return
        if ct.can_act():
            for d in CARDINALS:
                n = myLoc.add(d)
                ...
                if bid is not None and ct.get_team(bid) != ct.get_team():
                    if (abs(n.x - target.x) + abs(n.y - target.y) <= 5
                            and ct.can_fire(n)):
                        ct.fire(n)
                        return
        self.mapPf.moveTo(ct, target)

    def homeSentinel(self, ct: Controller, myLoc: Position) -> bool:
        """Their one piece of home defence: a sentinel ~4 tiles out on the
        enemy-facing side, placed t4-13. Range^2 32 single-tile line, so it
        covers the APPROACH LANE rather than the doorstep — which is why a
        3-builder economy never gets punished for being thin at home."""
        tc = self.mapPf.teamCore
        if ct.get_global_resources() < ct.get_sentinel_cost() + 60:
            return False
        goal = Position(self.mapW - 1 - tc.x, self.mapH - 1 - tc.y)
        dx = 1 if goal.x > tc.x else -1
        dy = 1 if goal.y > tc.y else -1
        spot = Position(max(0, min(self.mapW - 1, tc.x + dx * SENT_OUT)),
                        max(0, min(self.mapH - 1, tc.y + dy * (SENT_OUT - 1))))
        d = abs(myLoc.x - spot.x) + abs(myLoc.y - spot.y)
        ...
        if not ct.can_act():
            return True
        # Face the enemy core — do NOT ask can_fire_from against it: the
        # core is ~20-38 tiles away and a sentinel's range^2 is 32, so that
        # check can never pass and the sentinel was never built at all.
        # It guards the APPROACH LANE, not the core itself.
        try:
            face = spot.direction_to(goal)
        except Exception:
            face = None
        if face is None or face == Direction.CENTRE:
            face = DELTA_DIR.get((dx, 0), Direction.EAST)
        if not ct.can_build_sentinel(spot, face):
            for f in DIRECTIONS:                 # any legal facing beats none
                if ct.can_build_sentinel(spot, f):
                    face = f
                    break
        if ct.can_build_sentinel(spot, face):
            ct.build_sentinel(spot, face)
            ct.write_store(SENT_SLOT, 1)      # job done, team-wide
        self.sentDone = True                  # and I personally stop trying
        return True
