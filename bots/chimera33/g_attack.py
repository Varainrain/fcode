...
from g_mapPathfinding import CARDINALS, DIRECTIONS, gunnerLines, sentinelLines
from g_eco import closest_claimant, core_footprint_manhattan
...
def get_attackable_tiles(ct: Controller, player, lines=None):
    ...
    coreAttackers = set()
    # blocked=False for SENTINEL seats: their fire is indirect, and with
    # blocked=True our own entomb wall (built FIRST, per the blueprint)
    # would zero out every seat - the seat-finder bug class found via the
    # wave fixture. Gunner seats keep the ray check.
    blocked = lines is None
    for corner in coreCorners:
        for spot in player.mapPf.gunnerSpots(corner, player.mapW, player.mapH, blocked, lines=lines):
