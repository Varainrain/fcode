...
SLOT_SIEGER_2_ID = 10
SLOT_ENEMY_CORE = 11
SLOT_SIEGER_1_TARGET = 12
SLOT_SIEGER_2_TARGET = 13
SIEGE_START = 45
SIEGE_TITANIUM_FLOOR = 120
SIEGE_STALL_ROUNDS = 16
SIEGE_PAUSE_ROUNDS = 32
...
def siege_role_counters(role: int, defender_count: int) -> bool:
    """Return whether a numbered sieger should switch to counter-battery."""
    return (role == 2 and defender_count >= 1) or defender_count >= 3
...
class Player:
    def __init__(self):
        ...
        self._siegeStall = 0
        self._siegePauseUntil = 0
        self._siegeLastDistance = None
        self._siegeLastDefenders = None
        self._siegeLastCoreHp = None
        self._counterSeat = None
        self._counterStand = None
        self._counterFacing = None
        self._counterTarget = None
    ...
    def builderBot(self, ct: Controller):
        ...
        if self._is_sieger(ct):
            if ct.get_current_round() < self._siegePauseUntil:
                self._clear_counter_assignment(ct)
            elif (ct.get_current_round() >= SIEGE_START
                    and (self.siegeCommitted
                         or ct.get_global_resources() > SIEGE_TITANIUM_FLOOR)):
                self.siegeCommitted = True
                if self._run_adaptive_siege(ct, myLoc):
                    return
    ...
    def _sieger_target_slot(self, ct: Controller):
        role = self._sieger_role(ct)
        if role == 1:
            return SLOT_SIEGER_1_TARGET
        if role == 2:
            return SLOT_SIEGER_2_TARGET
        return None

    def _clear_counter_assignment(self, ct: Controller) -> None:
        slot = self._sieger_target_slot(ct)
        if slot is not None:
    ...
    def _direct_siege_zone(self, ct: Controller, target: Position):
        """Visible core-firing seats plus the cardinal stands that service them."""
        core_tiles = [
        ...
        for core_tile in core_tiles:
            for attack_offset in gunnerAttacks:
                seat = Position(
                    core_tile.x - attack_offset.x,
                    core_tile.y - attack_offset.y,
                )
                if not (0 <= seat.x < self.mapW
                        and 0 <= seat.y < self.mapH
                        and ct.is_in_vision(seat)):
                    continue
                facing = seat.direction_to(core_tile)
                ...
                for approach in CARDINALS:
                    ...
                    if (0 <= stand.x < self.mapW
                            and 0 <= stand.y < self.mapH):
                        zone.add(stand)
        return zone

    def _visible_core_defenders(self, ct: Controller, target: Position):
        defenders = []
        core_hp = None
        my_team = ct.get_team()
        siege_zone = self._direct_siege_zone(ct, target)
        for building_id in ct.get_nearby_buildings():
            ...
            if building_type == EntityType.CORE:
                core_hp = ct.get_hp(building_id)
            elif building_type in (EntityType.GUNNER, EntityType.SENTINEL):
                facing = ct.get_direction(building_id)
                attack_tiles = ct.get_attackable_tiles_from(
                    position, facing, building_type)
                if siege_zone.intersection(attack_tiles):
                    defenders.append((
                        position.x,
                        position.y,
                        building_id,
        ...
        defenders.sort()
        return defenders, core_hp

    def _update_siege_progress(
            self, ct: Controller, objective: Position,
            defender_count: int, core_hp) -> bool:
        """Return False on the turn that a stalled sieger enters its pause."""
        distance = ct.get_position().distance_squared(objective)
        progressed = (
            self._siegeLastDistance is None
            or distance < self._siegeLastDistance
            or (self._siegeLastDefenders is not None
                and defender_count < self._siegeLastDefenders)
            or (core_hp is not None and self._siegeLastCoreHp is not None
                and core_hp < self._siegeLastCoreHp)
        )
        self._siegeLastDistance = distance
        self._siegeLastDefenders = defender_count
        if core_hp is not None:
            self._siegeLastCoreHp = core_hp
        if progressed:
            self._siegeStall = 0
        else:
            self._siegeStall += 1
        if self._siegeStall < SIEGE_STALL_ROUNDS:
            return True
        self._siegePauseUntil = ct.get_current_round() + SIEGE_PAUSE_ROUNDS
        self._siegeStall = 0
        self._siegeLastDistance = None
        self._siegeLastDefenders = None
        self._siegeLastCoreHp = None
    ...
    def _mark_siege_build(self) -> None:
        self._siegeStall = 0

    def _run_adaptive_siege(self, ct: Controller, myLoc: Position) -> bool:
        target = self._enemy_core_target()
        if target is None:
            self._clear_counter_assignment(ct)
            return True
        defenders, core_hp = self._visible_core_defenders(ct, target)
        role = self._sieger_role(ct)
        if siege_role_counters(role, len(defenders)):
            target_index = 0 if role == 2 else 1
            selected = defenders[target_index % len(defenders)]
            objective = selected[3]
            if not self._update_siege_progress(
                    ct, objective, len(defenders), core_hp):
                return False
            self._run_counter_battery(ct, myLoc, selected, defenders)
            return True
        self._clear_counter_assignment(ct)
        if not self._update_siege_progress(
                ct, target, len(defenders), core_hp):
            return False
        self._run_direct_core_siege(ct, myLoc, target)
        return True

    def _defender_attack_tiles(self, ct: Controller, defenders):
        unsafe = set()
        for _, _, _, position, building_type, facing in defenders:
            unsafe.update(ct.get_attackable_tiles_from(
                position, facing, building_type))
        return unsafe

    def _counter_plan(self, ct: Controller, myLoc: Position,
                      target: Position, defenders):
        unsafe = self._defender_attack_tiles(ct, defenders)
        best = None
        for seat in ct.get_nearby_tiles():
            if seat in unsafe or ct.get_tile_env(seat) != Environment.EMPTY:
                continue
            building_id = ct.get_tile_building_id(seat)
            maintained = seat == self._counterSeat
    ...
    def _run_counter_battery(
            self, ct: Controller, myLoc: Position, selected, defenders) -> None:
        target = selected[3]
        target_slot = self._sieger_target_slot(ct)
        if target_slot is not None:
            ct.write_store(target_slot, pack_position(target))

        target_changed = self._counterTarget != target
        if target_changed:
            self._counterSeat = None
            self._counterStand = None
            self._counterFacing = None
            self._counterTarget = target

        if self._counterSeat is not None:
            gunner_id = ct.get_tile_building_id(self._counterSeat)
            if (gunner_id is not None
                    and ct.get_team(gunner_id) == ct.get_team()
                    and ct.get_entity_type(gunner_id) == EntityType.GUNNER):
                if (ct.get_hp(gunner_id) < ct.get_max_hp(gunner_id)
                        and ct.can_heal(self._counterSeat)):
                    ct.heal(self._counterSeat)
                return

        plan = self._counter_plan(ct, myLoc, target, defenders)
        if plan is None:
            self.mapPf.moveTo(ct, target)
            return
        seat, stand, facing = plan
        self._counterSeat = seat
        self._counterStand = stand
        self._counterFacing = facing
        if myLoc != stand:
        ...
        if (ct.can_act()
            ...
            self._mark_siege_build()

    def _run_direct_core_siege(
            self, ct: Controller, myLoc: Position, target: Position) -> None:
        """Walk to an exact gunner firing seat and rebuild it indefinitely."""
        target = self._enemy_core_target()
        if target is None:
            return
        core_tiles = [
