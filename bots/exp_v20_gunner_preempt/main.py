...
class Player:
    ...
    def runGunner(self, ct: Controller):
        ...
        if curTarget is not None:
            ...
            currentEnemy = enemyBuilding or enemyBuilder
        ...
        if bestDir != myDir:
            ...
            if ct.get_global_resources() > floor and ct.can_rotate(bestDir):
                ct.rotate(bestDir)
                return
        # Direction priority must be evaluated before firing. Previously any
        # target already in the barrel short-circuited this scan, so a gunner
        # could spend the entire core race on a turret or barrier despite an
        # immediately available enemy-core facing. If rotation is deliberately
        # unaffordable, retain the useful current shot instead of idling.
        if currentEnemy and ct.can_fire(curTarget):
            ct.fire(curTarget)
