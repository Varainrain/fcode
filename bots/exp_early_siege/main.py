...
SLOT_HOME_THREAT = 15
# Ladder measurement 2026-08-01: the teams above us put a gun within 6 tiles of
# the enemy core at a median of t19 (Powered by SmartFridge) and t24 (Pantheon);
# we manage t60. An econ probe on the parent shows titanium is NEVER the
# constraint - the core holds >120 Ti from round 0 and peaks at 470 by t45 with
# 5 builders - so this constant alone was deciding that we arrive last while
# sitting on unspent resources. Nothing else changes: the same two siegers, the
# same seats, the same titanium floor on every build.
SIEGE_START = 25
