...
GUN_IDLE_MAX = 40   # idle turns before a stranded gunner scraps itself
    ...
    def run_gunner(self, ct: Controller) -> None:
        ...
        if not curOk and bestDir is None:
            ...
            if self._gun_worth_holding(ct, myPos, myTeam):
                ...
                return
            self.idleTurns += 1
            # FOUR TURNS IS FAR TOO SHORT A FUSE. A gunner costs 20 Ti and this
            # threw it away after four quiet turns - ic3d watched one "kill
            # himself" in the visualiser. A turret covering an approach can
            # idle for fifty turns and then be the thing that saves the core,
            # and our gunner build counts (21, 27, 38 in single games) look a
            # lot like paying for the same turret over and over. Keep the
            # escape hatch for a genuinely stranded turret, but make it a
            # last resort rather than a lull.
            if self.idleTurns > GUN_IDLE_MAX:
                ct.self_destruct()
        else:
            self.idleTurns = 0

    def _gun_good_target(self, ct: Controller, bId) -> bool:
        """The satisfactory-target set: harvester, core, turret, LOADED conveyor or
        splitter. Deliberately excludes builder bots (they self-heal +4/turn
        against our 7 and simply walk away) and empty conveyors and barriers (a
