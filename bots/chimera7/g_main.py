...
from g_mapPathfinding import MapPathfinder, CARDINALS
...
import g_javelin as javelin

ATTACK_MOD = _fi("ATTACK_MOD", 97)         # GUNGNIR: no walking attackers -
                                           # builder 1 is the spear, 2-3 run
                                           # the (minimal) economy behind it
...
class Player:
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        # GUNGNIR: THREE builders, like the blueprint (e3eb4324: Jython beat
        # #2 Lorem 4-1 with 3 builders both fast games). Two more spawns of
        # NOT-spawning is ~110 Ti - that money IS the rush: 5-7 launchers,
        # the entomb, and two mortars all come out of the opening bank.
        if self.numSpawned < 3 or (globalTitanium > 360 and curRound - self.lastSpawn >= 8):
            self.spawnBot(ct)
        ...
        if _siegePhase == 1:
            ...
            AMMO_CEILING = 80              # TWO mortars drink 10/turn between
            self._s1turns = 0              # them; the kill window is ~35 turns
        else:
            self._s1turns = 0
    ...
    def builder_bot(self, ct: Controller):
        ...
        if self.mapPf.myNum == 2:
            if javelin.home_garrison(ct, self, myLoc):
                return
