...
# THE WALL, measured: role allocation and economy are the same knob. Economy
# work happens only in the non-sieger branch, so 2 siegers gives 4.6 guns on the
# enemy core and 4.0 harvesters, 3 siegers gives 0 and 0, and all-siege gives
# 4.5 and 0. Pantheon gets 18.7 guns from an economy the same size as ours, so
# their builders must farm AND deliver.
# This does not move anyone into the sieger role. It lets ANY builder that
# already happens to be near their core place a core-capable gun before falling
# through to its normal economy state. Delivery becomes opportunistic instead of
# exclusive, so nothing is taken away from income.
OPPORTUNIST_BAND = 8
...
class Player:
    ...
    def builderBot(self, ct: Controller):
        ...
        if (self._is_sieger(ct)
            ...
            return
        if self._opportunist_gun(ct, myLoc):
            return
    ...
    def _opportunist_gun(self, ct: Controller, myLoc: Position) -> bool:
        """Any builder standing near their core plants a core-capable gun.

        Reuses the sieger's own seat selection, so the seat still has to pass
        can_fire_from() against a core tile - this cannot place the mid-map guns
        that exp_core_pressure showed are worthless offensively.
        """
        if not ct.can_act():
            return False
        if ct.get_global_resources() <= SIEGE_TITANIUM_FLOOR:
            return False
        target = self._enemy_core_target()
        if target is None:
            return False
        if abs(myLoc.x - target.x) + abs(myLoc.y - target.y) > OPPORTUNIST_BAND:
            return False
        for plan in self._direct_siege_plans(ct, myLoc, target):
            seat, stand, facing = plan[1:]
            if stand != myLoc:
                continue          # do not divert; only build from where we are
