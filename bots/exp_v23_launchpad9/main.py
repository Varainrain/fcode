...
TEAM_CORE_SLOT = 7
LAUNCH_MIN_EXIT = 7
# v8 DEADLOCKED: riders reached the pad, wrote their claim and stood still
# waiting for a throw that never came - only one claim survives the single slot
# each turn, so the losers waited forever (traced: b3 frozen t6 -> t89).
# Wait at most this many turns, then give up and walk like everyone else.
...
class Player:
    ...
    def _claim_ride(self, ct: Controller, myLoc: Position) -> bool:
        ...
        Replay trace of launchpad7 showed our builders walking the ENTIRE way -
        t2:20, t3:19, t4:18 - never once thrown, while Pantheon's are already at
        14 by t2. Cause: this bailed out when mapPf.enemyCorePos is None, which
        is precisely the opening before symmetry resolves. The launcher already
        had a fallback target; the rider did not, so the pad sat unused through
        the only turns where a six-tile head start is worth anything.
        """
        teamCore = self.mapPf.teamCore
        target = self.mapPf.enemyCorePos
        if target is None:
        ...
        if self.rideWait > LAUNCH_WAIT_LIMIT:
            return False               # someone else has the pad; just march
