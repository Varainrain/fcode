...
class MapPathfinder:
    ...
    def shareTiles(self, ct: Controller, curRound):
        """EVEN/ODD TWO-BANK BROADCAST (bench variant of v58's round-robin).

        v58: one builder writes all 6 slots per round, chosen by
        round % numBuilders, carrying 16 tiles. With N builders a discovery
        waits up to N rounds to reach the team, and a bot with only 3 new tiles
        still burns the whole 16-slot payload padding with 0xFFF.

        Here the 6 slots are two independent banks of 3 (8 tiles each). Builders
        are split by the parity of their index: odd indices own slots 1-3, even
        own slots 4-6. Each bank round-robins over only its own half, so a
        builder broadcasts roughly every N/2 rounds instead of N, and two
        builders broadcast every round instead of one. Engine store writes are
        buffered a round and land atomically, so the banks cannot be read torn -
        which is what the even/odd read/write split guards against elsewhere.
        """
        n = ct.read_store(0)
        if not n:
            return
        bank = self.myNum % 2                       # 1 -> slots 1-3, 0 -> slots 4-6
        base = 1 if bank == 1 else 4
        groupSize = (n + 1) // 2 if bank == 1 else n // 2
        if groupSize < 1:
            return
        myIdx = (self.myNum - 1) // 2
        if curRound % groupSize != myIdx % groupSize:
            return
        ...
        combined = 0
        needToRemove = []
        for i in range(8):                          # 8 tiles x 12 bits = 96 bits
        ...
        for base in (1, 4):
            ...
            for i in range(8):
                ...
                if not self.seenTiles[x][y] or self.fullMap[x][y] != t_type:
                    ...
                    self.mapChanged = True
                self.newTiles = [t for t in self.newTiles
                                 if not (t[0].x == x and t[0].y == y)]
    ...
    def moveTo (self, ct: Controller, target: Position):
        ...
        if bestDir is not None and ct.can_move(bestDir) and self.inMoveZone(myLoc.add(bestDir)):
            ...
            self.stuckTurns += 1
            if self.stuckTurns > 2 + (ct.get_id() % 3):
                movableDirs = []
                for i in CARDINALS:
                    nextPos = myLoc.add(i)
