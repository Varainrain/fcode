...
class Player:
    ...
    def harvestTask(self, ct: Controller, myLoc, myTeam) -> bool:
        ...
        if bestTile.distance_squared(myLoc) > 1:
            ...
            self.stepOffHarvestTile(ct, teamCore)
        return True

    def stepOffHarvestTile(self, ct: Controller, teamCore: Position) -> bool:
        """Leave an ore tile toward home so it can be built next turn."""
        myLoc = ct.get_position()
        dirs = sorted(CARDINALS,
                      key=lambda d: myLoc.add(d).distance_squared(teamCore))
        for d in dirs:
            if ct.can_move(d):
                ct.move(d)
                return True
