...
class Player:
    ...
    def _spear(self, ct):
        ...
        if foe_hp is not None:
            ...
            self.foe_hp = foe_hp
            # TELL THE SENTINELS. A core that will not drop under a
            # standing volley is being tended: opponents now park EIGHT
            # builders on their core (+32 hp/turn against our 4x18-per-2 =
            # 36), which is a stalemate they win on economy. Killing a
            # tender takes three shots and removes 4 hp/turn permanently.
            ct.write_store(4, 1 if getattr(self, 'stall', 0) >= 6 else 0)
    ...
    def _sentinel(self, ct):
        my = ct.get_team()
        tenders = ct.read_store(4) == 1      # builder says: heals win
        best, rank = None, 0
