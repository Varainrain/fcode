...
# Ladder diagnosis 2026-08-01 (200 live games): between t150 and t300 we win
# 29% against the top four teams, our core taking 722 damage while dealing 268,
# with the enemy core FULLY healing our siege in 12 of 23 losses. We field MORE
# gunners than they do (30 v 23) and still convert a third of the damage; the
# only long games we win take 122 gunners, i.e. the sole answer the bot
# currently has to a healed core is to overwhelm it.
# Two net recoveries, not one: a single heal tick is ordinary, a core that keeps
# regaining HP is out-repairing the siege.
HEAL_LOCK_TRIGGER = 2
HEAL_LOCK_MAX = 15
HEAL_LOCK_TRACE_PATH = (
    "/mnt/c/Users/subodh/Downloads/"
    "fcode-gate-artifacts/heal-lock-traces/heal_lock_trace.log"
)


def heal_lock_trace(*parts):
    try:
        with open(HEAL_LOCK_TRACE_PATH, "a", encoding="utf-8") as stream:
            stream.write(" ".join(str(part) for part in parts) + chr(10))
    except Exception:
        pass
...
def pack_heal_lock(hp: int, evidence: int) -> int:
    return ((max(hp, 0) & 0xFFFF) << 4) | (max(min(evidence, HEAL_LOCK_MAX), 0))


def unpack_heal_lock(value: int):
    """Return (last seen enemy core hp, net recovery count). (None, 0) if unset."""
    if value == 0:
        return None, 0
    return (value >> 4) & 0xFFFF, value & 0xF


def next_heal_evidence(previous_hp, current_hp: int, evidence: int) -> int:
    """A recovering enemy core counts up, a falling one counts back down.

    Symmetric so that a siege which starts converting again releases the unit
    back to the siege instead of latching home for the rest of the game.
    """
    if previous_hp is None:
        return evidence
    if current_hp > previous_hp:
        return min(evidence + 1, HEAL_LOCK_MAX)
    if current_hp < previous_hp:
        return max(evidence - 1, 0)
    return evidence
...
class Player:
    ...
    def builderBot(self, ct: Controller):
        ...
        if (self._is_waller(ct)
            ...
            if self.wallDuty(ct, myLoc):
                return
        if (self._is_sieger(ct) and ct.get_current_round() >= SIEGE_START
    ...
    def _observe_enemy_core_heal(self, ct: Controller, target: Position) -> None:
        """Publish whether the enemy core is out-healing the siege.

        Pure observation of a visible entity's HP. No map, dimension, side,
        opponent, or turn-number input — a bot that repairs faster than we
        shoot looks the same here whoever it belongs to.
        """
        if target is None:
            return
        my_team = ct.get_team()
        core_id = None
        for core_tile in core_footprint(target):
            if not ct.is_in_vision(core_tile):
                continue
            building_id = ct.get_tile_building_id(core_tile)
        ...
        if core_id is None:
            return
        current_hp = ct.get_hp(core_id)
        previous_hp, evidence = unpack_heal_lock(ct.read_store(SLOT_HEAL_LOCK))
        heal_lock_trace(
            "OBSERVE", ct.get_current_round(), ct.get_id(),
            "prev", previous_hp, "cur", current_hp,
            "evidence", next_heal_evidence(previous_hp, current_hp, evidence))
        ct.write_store(SLOT_HEAL_LOCK, pack_heal_lock(
            current_hp, next_heal_evidence(previous_hp, current_hp, evidence)))

    def _heal_locked(self, ct: Controller) -> bool:
        _, evidence = unpack_heal_lock(ct.read_store(SLOT_HEAL_LOCK))
        return evidence >= HEAL_LOCK_TRIGGER
    ...
    def _run_core_siege(self, ct: Controller, myLoc: Position) -> None:
        ...
        if target is None:
            return
        self._observe_enemy_core_heal(ct, target)
        # HEAL-LOCK RETREAT. Only role 2 — the sieger that already gives up
        # its seat to counter a defender — and only while home is genuinely
        # threatened, so siege pressure is never fully abandoned and an
        # unthreatened home cannot pull anyone off the core. Both conditions
        # are read from state, and both release the unit when they clear.
        if self._sieger_role(ct) == 2 and self._heal_locked(ct):
            threat = unpack_position(ct.read_store(SLOT_HOME_THREAT))
            heal_lock_trace(
                "HL_LOCKED", ct.get_current_round(), ct.get_id(),
                "threat", threat.x if threat is not None else None,
                threat.y if threat is not None else None)
            if threat is not None:
                heal_lock_trace(
                    "HL_RETREAT", ct.get_current_round(), ct.get_id(),
                    myLoc.x, myLoc.y, "->", threat.x, threat.y)
                self._run_home_defense(ct, myLoc, threat)
                return
