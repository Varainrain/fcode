...
def immediatePressureScore(coverage, spot: Position, direction) -> tuple:
    return (coverage, spot.x, spot.y, DIRECTIONS.index(direction))
...
class Player:
    ...
    def runDefend(self, ct: Controller):
        myLoc = ct.get_position()
        myTeam = ct.get_team()
        if self.buildImmediateCorePressure(ct):
            return
    ...
    def buildImmediateCorePressure(self, ct: Controller) -> bool:
        """Let any role take a safe direct-core gun available this turn."""
        if (self.mapPf.enemyCorePos is None or
                ct.get_action_cooldown() != 0 or
                ct.get_global_resources() < 80):
            return False
        enemyCoverage = self.enemyTurretCoverage(ct)
        candidates = []
        for spot, direction in self.getAttackableTiles(ct):
            ...
            if ct.get_tile_env(spot) != Environment.EMPTY:
                continue
            coverage = enemyCoverage.get((spot.x, spot.y), 0)
            if coverage > 1 or not ct.can_build_gunner(spot, direction):
                continue
            candidates.append((
                immediatePressureScore(coverage, spot, direction),
                spot, direction))
        if not candidates:
            return False
        _, spot, direction = min(candidates)
        ct.build_gunner(spot, direction)
        return True
