...
class Player:
    ...
    def builderBot(self, ct: Controller):
        ...
        self.turnsAlive += 1
        # 2.3 REACTIVE CORE DEFENSE: the old code was written when builders
        # could only hit their own tile, so a builder standing next to the
        # gunner killing our core did nothing (lastpopperian cores us ~r94
        # while our bots loiter adjacent). Builders can now fire orthogonally
        # adjacent tiles — use it. Scoped to home + emergency so it costs the
        # economy nothing when there's no threat (proactive home-guards
        # already failed at 43-45%; this is different — react, don't recall).
        if self.defendCore(ct, myLoc):
            return
    ...
    def defendCore(self, ct: Controller, myLoc: Position) -> bool:
        tc = self.mapPf.teamCore
        if tc is None:
            return False
        # only builders already near home participate — never pull a miner
        # or attacker off task (that was the failed home-guard)
        # TIGHTENED from 10: at manhattan 10 builders plink at turrets that
        # are not actually threatening the core and burn their whole turn
        # doing it (isolated 168-game gate vs the same bot without this:
        # 42%, kills 47-83). Only the true doorstep is worth a turn.
        if abs(myLoc.x - tc.x) + abs(myLoc.y - tc.y) > 4:
            return False
        myTeam = ct.get_team()
        TURRETS = (EntityType.GUNNER, EntityType.SENTINEL, EntityType.LAUNCHER)
        # (1) reactive fire: shoot an orthogonally-adjacent enemy turret NOW
        if ct.can_act():
            for d in CARDINALS:
                adj = myLoc.add(d)
                if not (0 <= adj.x < self.mapW and 0 <= adj.y < self.mapH):
                    continue
                if not ct.is_in_vision(adj):
                    continue
                bid = ct.get_tile_building_id(adj)
                if (bid is not None and ct.get_team(bid) != myTeam
                        and ct.get_entity_type(bid) in TURRETS):
                    if ct.can_fire(adj):
                        ct.fire(adj)
                        return True
        return False
