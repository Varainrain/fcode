...
# Killing a core is 50 gunner shots at 2 ammo each = 100 AMMO. The parent caps
# the pool at 20 - ten shots, 100 damage, one fifth of a core - and only refills
# when it drops below 20, once per turn. Several guns on a 1-turn cooldown drain
# that in a single turn, so the siege spends most of its life dry while the core
# sits on ~470 unspent titanium. Conversion is 1:1, same-turn, once per team per
# turn, so holding titanium instead of ammo buys nothing.
# Pantheon kills by t50 in 86% of their wins; that is ~100 ammo spent inside 50
# turns, which the parent's policy cannot physically supply.
AMMO_TARGET = 120        # a core's worth of shots plus margin
# First attempt reserved only 150 Ti and made things WORSE - 4.8 guns and 248
# core damage against the parent's 6.2 and 585 - because SIEGE_TITANIUM_FLOOR
# is 120, so converting down to 150 left a 30 Ti margin and the siege could
# barely build. Guns must always outrank ammo.
AMMO_TITANIUM_RESERVE = 260
        ...
        if globalTitanium > 80 + 60 * self.numSpawned:
            for i in ct.get_nearby_tiles():
                if ct.can_spawn(i):
                    ...
                    break
        if globalAmmo < AMMO_TARGET and globalTitanium > AMMO_TITANIUM_RESERVE:
            want = min(AMMO_TARGET - globalAmmo,
                       globalTitanium - AMMO_TITANIUM_RESERVE)
