...
RECALL_HP = 490      # adgatomic: heal wall answers ANY core damage (100% repair)
RECALL = 1
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if self.numSpawned < 6 or (globalTitanium > 300
                                   and _rnd - self.lastSpawn >= 6):
            spawnableTiles = []
            for tile in ct.get_nearby_tiles():
    ...
    def builderBot(self, ct: Controller):
        ...
        if RECALL:
            try:
                ...
                pass
        if self.mapPf.myNum % 3 == 1 and self.mapPf.myNum not in (5, 7): # attacking (adgatomic: 1/3 only)
    ...
    def runAttack(self, ct: Controller):
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            # ADGATOMIC ATTRITION: never all-in. Past the midline, dig in --
            # gunner facing their core, barrier beside it (their g20-39 +
            # bar8-34 wall pattern). Advance only behind a finished wall.
            _home = self.mapPf.teamCore
            _ec = self.mapPf.enemyCorePos
            if (_home is not None
                    and myLoc.distance_squared(_home) * 2 > myLoc.distance_squared(_ec)):
                if ct.get_global_resources() >= 40:
                    for _d in CARDINALS:
                        _s = myLoc.add(_d)
                        if not (0 <= _s.x < self.mapW and 0 <= _s.y < self.mapH):
                            continue
                        try:
                            if ct.get_tile_building_id(_s) is not None:
                                continue
                            _f = _s.direction_to(_ec)
                            if ct.can_build_gunner(_s, _f):
                                ct.build_gunner(_s, _f)
                                return
                            if ct.can_build_barrier(_s):
                                ct.build_barrier(_s)
                                return
                        except Exception:
                            continue
    ...
    def runEco(self, ct: Controller):
        ...
        if self.sealTask(ct, myLoc, self.coreThreats(enemySentinels)): # reactive anti-sentinel seal
            return
