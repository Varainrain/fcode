...
class Player:
    ...
    def findGunnerSpot(self, ct):
        ...
        for spotPos, spotDir in coreAttackers:
            ...
            if ct.get_tile_building_id(spotPos) is not None:
                continue
            if ct.get_tile_builder_bot_id(spotPos) is not None:
                continue
    ...
    def runGunner(self, ct: Controller):
        ...
        for directionIndex, d in enumerate(DIRECTIONS):
            ...
            score = (coreHits, coreThreatHits, gunnerHits, builderHits, otherHits)
    ...
    def runDefend(self, ct: Controller):
        ...
        uncovered = [g for g in enemyGunners if not self.friendlyTurretCanHit(ct, g)]
        if uncovered:
    ...
    def friendlyTurretCanHit(self, ct: Controller, target: Position) -> bool:
        """Whether a real friendly turret has an unobstructed ray to target."""
        myTeam = ct.get_team()
        for b in ct.get_nearby_buildings():
            if ct.get_team(b) != myTeam:
                continue
            ...
            if bType not in (EntityType.GUNNER, EntityType.SENTINEL):
                continue
            if ct.can_fire_from(ct.get_position(b), ct.get_direction(b), bType, target):
                return True
        return False
    ...
    def rayBlockedByTeam(self, ct: Controller, gunnerSpot: Position, attackPos: Position, d: Direction, myTeam) -> bool:
        ...
        for _ in range(max(self.mapW, self.mapH)):
            ...
            if bId is not None and ct.get_team(bId) == myTeam:
                return True
            bbId = ct.get_tile_builder_bot_id(Position(x, y))
