...
# SEAT DENIAL — derived from the lastpopperian_ autopsy, not guessed.
# They kill us by seating gunners at manhattan 2-5 from our core, mostly on
# DIAGONALS, and rebuilding each one we kill. oogerebus3's ring wall denies
# ORTHOGONAL ring tiles, so it does not deny their seats at all — wrong
# shape for the actual threat.
# A "seat" here is any tile from which a gunner could actually hit a core
# tile, asked of the ENGINE (can_fire_from) rather than assumed, so it is
# exact for whatever the current ray rules are.
SEAT_BOX = 4          # search this far around the core footprint
SEAT_SLOT = 10        # free store slot (STORE_SIZE 16; bot uses 0-7)
SEAT_CREW = 2         # builders on denial duty
SEAT_UNTIL = 90       # their first seat lands t59-84; hold cover past it
SEAT_RESERVE = 60     # conveyor is 3 Ti — denial is cheap in titanium
...
class Player:
    def __init__(self):
        ...
        self.turnsAlive = 0
        self.attackBan = 0
        self.seats = None         # cached threat tiles around our core
        self.seatDuty = None      # None=undecided, True=on denial duty
        self.mapW = None
        self.mapH = None
    ...
    def builderBot(self, ct: Controller):
        ...
        self.turnsAlive += 1
        if (ct.get_current_round() <= SEAT_UNTIL
                and self.mapPf.teamCore is not None):
            if self.seatDuty is None:
                claimed = ct.read_store(SEAT_SLOT)
                self.seatDuty = claimed < SEAT_CREW
                if self.seatDuty:
                    ct.write_store(SEAT_SLOT, claimed + 1)
            if self.seatDuty and self.denySeats(ct, myLoc):
                return
    ...
    def seatTiles(self, ct: Controller):
        """Every tile from which a gunner could hit one of our core tiles.
        Asked of the engine per tile/facing, so it stays correct if the ray
        rules change again (2.3 already moved them once)."""
        if self.seats is not None:
            return self.seats
        tc = self.mapPf.teamCore
        foot = [Position(tc.x + a, tc.y + b) for a in (0, 1) for b in (0, 1)]
        footset = {(p.x, p.y) for p in foot}
        out = []
        for dx in range(-SEAT_BOX, SEAT_BOX + 2):
            for dy in range(-SEAT_BOX, SEAT_BOX + 2):
                t = Position(tc.x + dx, tc.y + dy)
                if (t.x, t.y) in footset:
                    continue
                ...
                for f in DIRECTIONS:
                    for c in foot:
                        try:
                            if ct.can_fire_from(t, f, EntityType.GUNNER, c):
                                threat = True
                                break
                        except Exception:
                            continue
                ...
                if threat:
                    out.append(t)
        # nearest first: the close diagonal seats are the ones they use
        out.sort(key=lambda p: abs(p.x - tc.x) + abs(p.y - tc.y))
        self.seats = out
        return out

    def denySeats(self, ct: Controller, myLoc: Position) -> bool:
        """Fill threat seats with core-facing CONVEYORS. Conveyors cost 3 Ti
        and also carry titanium home, so denial doubles as economy — the
        user's own observation that beat barriers. Returns True if the turn
        was used."""
        if ct.get_global_resources() < ct.get_conveyor_cost() + SEAT_RESERVE:
            return False
        tc = self.mapPf.teamCore
        target = None
        for t in self.seatTiles(ct):
            try:
        ...
        if target is None:
            return False
        d = abs(myLoc.x - target.x) + abs(myLoc.y - target.y)
        if d == 0:
            for mv in CARDINALS:
                if ct.can_move(mv):
                    ct.move(mv)
        ...
        if not ct.can_act():
            return True
        # face the neighbour nearest the core so the line chains inward and
        # never dead-ends (a dead-end conveyor swallows the harvest chain)
        face, best = None, None
        for f in CARDINALS:
            n = target.add(f)
            dd = abs(n.x - tc.x) + abs(n.y - tc.y)
            if best is None or dd < best:
                best, face = dd, f
        if face is not None and ct.can_build_conveyor(target, face):
