...
class Player(eco.EcoBot, attack.AttackBot, gunner.GunnerBot):
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        # Slot 11: core HP as a PERCENT, +1 so an unwritten slot (0) is
        # distinguishable from a dead core. MEASURED in the HTTP 418 loss
        # (4a6904bf g2): step_core_siege can only read core HP through
        # get_nearby_buildings, i.e. only when the core is IN VISION - and a
        # builder sees radius^2 20, about 4.4 tiles. Our builders were laying
        # conveyors 5-8 tiles out, so the emergency step bailed on
        # `coreId is None` EVERY turn while the core took 502 damage over 67
        # turns and we answered with 14 conveyors, several of them rebuilt on
        # the same shot-out tile. The core knows its own HP; publishing it
        # makes the emergency reachable from anywhere on the map.
        ct.write_store(11, _hp * 100 // ct.get_max_hp() + 1)
