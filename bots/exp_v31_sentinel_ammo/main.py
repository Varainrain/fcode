...
# SENTINEL RUSH — the 2.3.4 rebalance inverts the unit economy.
#                gunner        sentinel
#   dmg/turn        7.0            9.0
#   dmg/ammo       1.75           1.80
#   dmg/Ti         0.35           0.60      <- 71% better per titanium
#   HP               25             40
#   reach       card 3/diag 2  card 5/diag 4   <- TWO fewer tiles to travel
# Gunners were nerfed on four axes at once (cost 10->20, damage 10->7, ammo
# 2->4, HP 40->25) while sentinels were buffed (cooldown 3->2, HP 30->40).
# The reach matters most here: Phase 0 measured our first gun as TRAVEL-bound,
# and a sentinel threatens the core from 5 tiles instead of 3.
# Attack seats now use sentinel geometry and build sentinels. Defence is left
# on gunners deliberately - one change at a time.
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        # SENTINEL AMMO. The pool was capped at 28 (the comment saying 50 is
        # stale), which was sized for 4-ammo gunners: 28/4 = 7 shots available
        # per turn. A sentinel shot costs 10, so that cap lets at most TWO
        # sentinels fire in a turn. exp_v31_sentinel built 9.2 of them and
        # starved seven at 30 Ti each, and gated 21% (kills 17-241) - the units
        # were fine, they had nothing to shoot with.
        # 2.3.4 also raised the cost of a 500 HP core from 100 ammo to ~280.
        # Conversion is 1:1 and once per turn, so holding titanium instead of
        # ammo buys nothing; keep a working reserve and bank the rest as ammo.
        AMMO_TARGET = 140
        convertAmount = min(AMMO_TARGET - globalAmmo, globalTitanium - 40)
    ...
    def getAttackableTiles(self, ct):
        ...
        for corner in coreCorners:
            for spot in self.mapPf.sentinelSpots(corner, self.mapW, self.mapH, True):
