...
def is_late_reinforcement(
        spawn_round: int, is_waller: bool, is_designated_sieger: bool) -> bool:
    """Late production becomes offense; the established economy stays intact."""
    return (
        spawn_round >= ECON_CUTOFF
        and not is_waller
        and not is_designated_sieger
    )
...
class Player:
    ...
    def builderBot(self, ct: Controller):
        myLoc = ct.get_position()
        if self.spawnRound is None:
            self.spawnRound = ct.get_current_round()
        ...
        late_reinforcement = is_late_reinforcement(
            self.spawnRound, self._is_waller(ct), designated_sieger)
        if ((designated_sieger or late_reinforcement)
