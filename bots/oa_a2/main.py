...
class Player:
    ...
    def runAttack(self, ct: Controller):
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            # A2: 2.3.6 scales every build cost with our own unit/building count
            # (measured: gunner 20 Ti at t0, 58-72 Ti once we field 9-12 units).
            # A flat 96/30 gate therefore blocks the cheap early seats and, late,
            # leaves the core under its 28 Ti ammo-conversion floor. Ask the engine.
            gunCost = ct.get_gunner_cost()
            if ct.get_global_resources() < gunCost + 28:
                if ct.get_global_resources() >= gunCost and self.attackHarvesterWithGunner(ct):
                    return
