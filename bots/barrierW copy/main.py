...
C_BARRIER      = (120, 60, 255) # Purple | attack: covering enemy-half ore with a barrier
...
class Player:
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            if ct.get_global_resources() < 96:
                if ct.get_global_resources() >= 30 and self.attackHarvesterWithGunner(ct):
                    return
                if self.hasEnemyOreInVision(ct, myLoc) and self.barrierTask(ct, myLoc):
                    return
    ...
    def hasEnemyOreInVision(self, ct: Controller, myLoc) -> bool:
        ...
        for tile in ct.get_nearby_tiles():
            ...
            if ct.get_tile_building_id(tile) is not None:
                continue
            if (1.6 * tile.distance_squared(enemyCore)) < myLoc.distance_squared(enemyCore):
                return True
        return False

    @_prof('barrier')
    def barrierTask(self, ct: Controller, myLoc) -> bool:
        if ct.get_global_resources() < ct.get_barrier_cost():
            return False
        enemyCore = self.mapPf.enemyCorePos
        teamCore = self.mapPf.teamCore
        if enemyCore is None or teamCore is None:
            return False
        enemyThreatened = self.mapPf.enemyTurretThreatenedTiles(ct)
        best = None
        bestKey = None
        for tile in ct.get_nearby_tiles(20):
            if self.mapPf.fullMap[tile.x][tile.y] != 1:
                continue
            if (1.6 * tile.distance_squared(enemyCore)) >= myLoc.distance_squared(enemyCore):
                continue
            ...
            if ct.get_tile_building_id(tile) is not None:
                continue
            key = (myLoc.distance_squared(tile), -tile.distance_squared(enemyCore))
            if bestKey is None or key < bestKey:
                bestKey = key
                best = tile
        if best is None:
            return False
        self.drawState(ct, C_BARRIER, best)
        myDist = myLoc.distance_squared(best)
        if myDist > 1:
