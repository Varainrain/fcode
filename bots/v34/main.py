...
class Player:
    ...
    def cornerShields(self, ct: Controller) -> bool:
        """USER-OBSERVED COUNTER to the Pantheon opening (2 diagonal
        sentinels at standoff kill our 5 gunners): every diagonal sentinel
        line into a core corner passes through the ONE tile diagonally
        adjacent to that corner. Four barriers - 12 Ti total - block all
        four diagonal lines. Breaking one costs the sentinel 2 shots and
        20 ammo; we rebuild for 3 Ti + one action. Maintain them forever.
        ...
        if ct.get_global_resources() < 25:
            return False
        # 4 diagonal corner tiles (block diagonal lines) + 8 cardinal
        # LANE tiles at distance 2 (block straight lines; NOT the d1 ring,
        # which stays free for spawns and heals). Pantheon scrim e55533da:
        # corner shields alone stopped the diagonal kill and they rerouted
        # to cardinal columns (atoll t75: sentinel at (14,7), our core
        # (14,2) - straight down the lane). Full seal = 12 barriers, 36 Ti.
