...
BACKTRACK_PENALTY = 3   # cost added to the tile we left last turn
...
class MapPathfinder:
    ...
    def getNewTiles(self, ct: Controller):
        for tile in ct.get_nearby_tiles():
            ...
            if not self.checkPassable(ct, tile) and tileEnv != Environment.WALL:
                tileEnv = 'Unpassable'
            tileType = tileTypes.get(tileEnv)
            if self.seenTiles[x][y] == False or self.fullMap[x][y] != tileType:
                self.checkTileSymmetry(tile, tileType)  # before marking it seen, so it can't test itself
    ...
    def moveTo(self, ct: Controller, target: Position):
        ...
        for d in CARDINALS:
            ...
            if 0 <= nextPos.x < self.mapW and 0 <= nextPos.y < self.mapH:
                ...
                # BACKTRACK PENALTY. MEASURED in the Lorem Ipsum 5-0
                # (e7e038e6 g1, v205 WITH oni's hysteresis already in):
                # builder 11 made 651 moves over just 72 tiles with 265 of
                # them returning to the tile it left one turn earlier - 41%
                # - and builder 6 was at 37%. Roughly 440 builder-turns
                # thrown away in one game. The cause is structural: moveTo
                # always walks toward whatever target the ladder picked THIS
                # turn, and when two steps claim the builder in alternate
                # turns the best direction genuinely reverses. A small
                # penalty on the tile we just left breaks that symmetry
                # without forbidding a real return - a genuinely better path
                # still wins by more than the penalty.
                if self.lastPos is not None and nextPos == self.lastPos:
                    posDist += BACKTRACK_PENALTY
        ...
        if bestDir is not None and ct.can_move(bestDir) and self.inMoveZone(myLoc.add(bestDir)):
            self.lastPos = myLoc          # the tile we are leaving this turn
            ct.move(bestDir)
            self.stuckTurns = 0
        else:
            self.stuckTurns += 1
