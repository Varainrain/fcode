    ...
    if tileId is not None:
        ...
        if tType == EntityType.CONVEYOR:
            ...
            if ct.get_stored_resource(tileId) is not None:
                return 24, 'done'
            # MERGE TOWARD THE CORE. These two costs were the wrong way round
            # and it is what builds the blobs: an EMPTY tile costs 1, so at a
            # join cost of 30 the router would rather lay up to twenty-nine
            # fresh conveyors than merge into one of our own chains that is
            # already heading for the core - which is how five builders end up
            # running parallel lines through the same ground. MEASURED on the
            # game ic3d screenshotted (0b8fb835 g2): 68 conveyor tiles, 54%
            # with three or more conveyor neighbours, a 33-tile clump filling
            # 61% of a 6x9 box fifteen tiles from our core, and 10.6 conveyors
            # per harvester against O(1)'s 3.5.
            # endDist is the chain end's distance to the core, otherDist this
            # tile's: endDist > otherDist means this conveyor is CLOSER in, so
            # joining it sends our resources onward. That is the cheap case.
