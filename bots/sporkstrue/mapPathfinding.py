...
class MapPathfinder:
    ...
    def _noteEnemyConvDeaths(self, ct: Controller):
        """CUT-AND-CAP support (ic3d's watch, verified in data: Jython beat
        #1 sporks doing this — 53 sporks conveyor deaths, 3 capped with
        barriers so the line can never rebuild in place; Flotte did it to
        us). Track visible ENEMY conveyor tiles; when one disappears,
        remember the tile for a few rounds so a nearby builder can cap it."""
        if not hasattr(self, 'enemyConvSeen'):
        ...
        for txy in gone:
            ...
            if not hasattr(self, 'enemyConvDead'):
                self.enemyConvDead = set()
            self.enemyConvDead.add(txy)
        # expire old cuts
    ...
    def conveyorTileCostSafe(self, ct: Controller, tile: Position, enemyThreatened, curEnd: Position):
        ...
        if cost is not None and hasattr(self, 'deadConv'):
            ...
            if d:
                cost += 45 * min(2, d)
        # CORRIDOR STEAL (sporkstrue): enemy conveyor graves are pre-cleared
        # corridor -- discount so our lines take them over (real sporks
        # retakes our corridors within ~1 round of the death)
        if cost is not None and hasattr(self, 'enemyConvDead')                 and (tile.x, tile.y) in self.enemyConvDead:
            cost = max(1, cost - 20)
        return [cost, status]
