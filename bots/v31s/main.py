...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        # sentinel siege drinks 10 ammo/shot every 2 rounds (gunner: 4/shot)
        # — the old 28 ceiling starves two firing sentinels
        convertAmount = min(60 - globalAmmo, globalTitanium - 16)
    ...
    def runAttack(self, ct: Controller):
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            if ct.get_global_resources() >= 80:
                # 2.3.4 META (measured from Pantheon and Pareto-ion, who both
                # switched within a DAY of the patch: ZERO gunners, 2-5
                # sentinels + barriers, killing us t46-88): the sentinel is
                # now the siege weapon. 18dmg/2rounds = 9/round vs the nerfed
                # gunner's 7; range^2 32 vs 13 — seats sit ~5.6 tiles out,
                # OUTSIDE enemy gunner reach; 40hp vs 25; flat 30 Ti with NO
                # +20%% scaling. Gunner logic kept as fallback only.
                sentStuff = self.findSentinelSpot(ct)
                if sentStuff:
                    sentSpot, sentDir = sentStuff
                    if ct.can_build_sentinel(sentSpot, sentDir):
                        ct.build_sentinel(sentSpot, sentDir)
                        return
                    myDist = myLoc.distance_squared(sentSpot)
                    if myDist < 1:
                        self.mapPf.moveTo(ct, self.mapPf.teamCore)
                    elif myDist > 1:
                        self.mapPf.moveTo(ct, sentSpot)
                    return
    ...
    def findSentinelSpot(self, ct):
        """Standoff sentinel seat: any visible empty tile from which a
        sentinel's line reaches a core tile (asked of the ENGINE per
        facing, so it survives ray-rule changes). Prefers uncovered seats,
        then near-me; ties toward the core."""
        ...
        best = None
        bestScore = None
        for spot in ct.get_nearby_tiles():
            try:
                if not ct.is_in_vision(spot):
                    continue
                if ct.get_tile_building_id(spot) is not None:
                    continue
                if ct.get_tile_env(spot) != Environment.EMPTY:
                    continue
            except Exception:
                continue
            if spot.distance_squared(enemyCore) > 40:
                continue                  # hopeless — sentinel range^2 is 32
            seatCov = enemyCoverage.get((spot.x, spot.y), 0)
            if seatCov > 1:
                continue
            face = None
            for f in DIRECTIONS:
                ...
                for c in corners:
                    try:
                        if ct.can_fire_from(spot, f, EntityType.SENTINEL, c):
                            ok = True
                            break
                    except Exception:
                        continue
            ...
            if face is None:
                continue
            score = (seatCov, myLoc.distance_squared(spot),
                     spot.distance_squared(enemyCore))
