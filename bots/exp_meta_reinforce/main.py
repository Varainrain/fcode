...
ECON_CUTOFF = 40
# Ordinary builders may convert earlier only after the opening exists and they
# personally observe strategic contact. This is deliberately local evidence:
# no opponent, map-name, map-size, or spawn-order fingerprint is involved.
REINFORCE_CONTACT_ROUND = 18
REINFORCE_MIN_BUILDERS = 5
...
def should_commit_reinforcement(
        current_round: int, num_spawned: int, is_waller: bool,
        already_committed: bool, strategic_contact: bool) -> bool:
    """Return whether an ordinary builder should join direct siege."""
    if is_waller:
        return False
    if already_committed or current_round >= ECON_CUTOFF:
        return True
    return (
        current_round >= REINFORCE_CONTACT_ROUND
        and num_spawned >= REINFORCE_MIN_BUILDERS
        and strategic_contact
    )
...
class Player:
    def __init__(self):
        ...
        self.reinforcementCommitted = False
    ...
    def builderBot(self, ct: Controller):
        ...
        designated_sieger = self._is_sieger(ct)
        reinforcement = self._should_reinforce_siege(ct)
        if ((designated_sieger or reinforcement)
    ...
    def _has_strategic_contact(self, ct: Controller) -> bool:
        """Observe only enemies that justify converting economy to pressure."""
        my_team = ct.get_team()
        for entity_id in ct.get_nearby_entities():
            if ct.get_team(entity_id) == my_team:
                continue
            if ct.get_entity_type(entity_id) in (
                    EntityType.CORE, EntityType.GUNNER,
                    EntityType.SENTINEL, EntityType.LAUNCHER):
                return True
        return False

    def _should_reinforce_siege(self, ct: Controller) -> bool:
        """Latch freed ordinary builders into the existing siege lifecycle."""
        commit = should_commit_reinforcement(
            ct.get_current_round(),
            self.numSpawned,
            self._is_waller(ct),
            self.reinforcementCommitted,
            self._has_strategic_contact(ct),
        )
        if commit:
            self.reinforcementCommitted = True
        return commit
