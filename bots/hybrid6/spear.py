...
class SpearPlayer:
    ...
    def _spear(self, ct):
        ...
        # WAIT FOR THE PICKUP. A launcher can only throw a builder standing
        # beside it on the launcher's own turn - marching on immediately
        # means every stone after the first is wasted (measured: four
        # launchers built, exactly one throw).
        if getattr(self, 'wait_throw', 0) > 0:
            self.wait_throw -= 1
            return
        if far > 7 and self.built == 0:
            # RELAY: on a long march two turns buys five-plus tiles. Capped
            # at three launchers and gated so the four-sentinel volley is
            # never starved - transport never outranks the spear itself.
            # One stone at a time: build, get thrown, build again where we
            # land. Three launchers all raised next to our own core bought
            # exactly one throw - the chain has to advance with us.
            lastl = getattr(self, 'last_lau', None)
            moved_on = lastl is None or cheb(pos, lastl) >= 4
            if (getattr(self, 'lau', 0) < 4 and can_act and moved_on
                    and far > 12
                ...
                for d in CARDINALS:
                    ...
                    if ct.can_build_launcher(sp):
                        ...
                        self.last_lau = pos
                        self.wait_throw = 3     # stand still to be picked
                        return
        ...
        if want_build and can_act \
            ...
            if cands:
                ...
                # BLOCKED-RAY FILTER: a seat with one of our own
                # sentinels between it and the target can never fire -
                # sentinel shots stop at the first occupied tile. Stacking
                # a column looked compact and was worthless.
                mine_set = {(m.x, m.y) for m in getattr(self, 'mine', [])}
                def clear(c):
                    s, t = c
                    dx = (t.x > s.x) - (t.x < s.x)
                    dy = (t.y > s.y) - (t.y < s.y)
                    ...
                    while (x, y) != (t.x, t.y):
                        ...
                        y += dy
                    return True
                cands = [c for c in cands if clear(c)] or cands
                cands.sort(key=key)
                walk_to = None
                standing_on = False
                for s, t in cands:
        ...
        for t in ct.get_attackable_tiles():
            ...
            if bId is not None:
                ...
                if bt == EntityType.CORE:
                    ...
                    r = 1       # conveyor/barrier: worth a shot ONLY to
                                # clear the line. Measured (skald vs fp14):
                                # our front sentinel sat behind an enemy
                                # harvester it ranked ZERO and never fired
                                # - four live sentinels, full ammo, 2 total
                                # damage in a 103-turn game.
            if r > rank:
                rank, best = r, t
