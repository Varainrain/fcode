        ...
        for dx in range(-1, 3):
            for dy in range(-1, 3):
                ...
                if self.mapPf.getTileEnv(tile) > 1: # wall or unpassable
                    continue
                # Controller tile-entity queries are vision-gated. A defender
                # spawned far from home still knows the core ring from shared
                # map state, but must not query its live occupant until close.
                if (ct.is_in_vision(tile) and
                        ct.get_tile_building_id(tile) is not None):
                    continue
