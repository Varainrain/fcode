...
RECALL_CORE_HP = 400
EMERGENCY_AMMO_TARGET = 20
EMERGENCY_TITANIUM_RESERVE = 28
HOME_GUNNER_ROTATE_FLOOR = 25
...
def emergency_ammo_conversion(
        global_ammo: int, global_titanium: int, under_attack: bool) -> int:
    """Amount of titanium to convert while preserving one repair/build reserve."""
    if not under_attack:
        return 0
    return max(0, min(
        EMERGENCY_AMMO_TARGET - global_ammo,
        global_titanium - EMERGENCY_TITANIUM_RESERVE,
    ))
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        emergency_conversion = emergency_ammo_conversion(
            globalAmmo, globalTitanium, under_attack)
        if emergency_conversion > 0:
            if ct.can_convert_ammo(emergency_conversion):
                ct.convert_ammo(emergency_conversion)
        elif globalAmmo < 20 and globalTitanium > 100:
    ...
    def _assign_home_defender(self, ct: Controller) -> None:
        ...
        if self._homeDefenderId is None:
            ct.write_store(SLOT_HOME_DEFENDER_ID, 0)
            # The threat channel also steers nearby friendly gunners. Publish
            # it even when no builder is available to become the responder.
        ...
        if curTarget is not None:
            ...
            if targetId is not None and ct.get_team(targetId) != ct.get_team():
                if ct.can_fire(curTarget):
                    ct.fire(curTarget)
                    return
        # v6's live anti-Pantheon controller was only partially ported into v8:
        # recall came across, but home gunners lost their explicit response to
        # the exact core-hitting turret. Reuse the responder's verified threat
        # channel, and lower the rotation reserve only for a shot on that tile.
        home_threat = unpack_position(ct.read_store(SLOT_HOME_THREAT))
        if home_threat is not None:
            threat_direction = None
            for direction in DIRECTIONS:
                if ct.can_fire_from(
                        myPos, direction, EntityType.GUNNER, home_threat):
                    threat_direction = direction
                    if direction == myDir:
                        break
            if (threat_direction is not None
                    and threat_direction != myDir
                    and ct.get_global_resources() > HOME_GUNNER_ROTATE_FLOOR
                    and ct.can_rotate(threat_direction)):
                ct.rotate(threat_direction)
                return
