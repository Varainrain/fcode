...
# THE PACKAGE. Measured separately, each half loses:
#   launcher only  38-40% (two implementations) - faster first gun t24 -> t20,
#                  but kills 96-198: builders arrive early and ALONE and die.
#   armour only    47% - guns survive better but still arrive late.
# Pantheon runs both: a t1 pad throwing builders from t2, AND 26 barriers a game
# placed 100% within 6 tiles of OUR core to keep the arrivals alive. The pad
# buys tempo; the armour is what makes early arrival survivable. Neither is the
# mechanism on its own.
TEAM_CORE_SLOT = 7
LAUNCH_MIN_EXIT = 7
FWD_ARMOR_TITANIUM_FLOOR = 100
FWD_ARMOR_BAND = 6          # must land at least this far from our own core
...
class Player:
    ...
    def _claim_ride(self, ct: Controller, myLoc: Position) -> bool:
        """Attacker-only: walk to the pad and announce that we want the throw."""
        teamCore = self.mapPf.teamCore
        target = self.mapPf.enemyCorePos
