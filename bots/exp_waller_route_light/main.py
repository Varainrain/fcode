...
def choose_route_repairer(
        candidates, target: Position, excluded_ids=(), preferred_ids=()):
    ...
    return min(
        eligible,
        key=lambda item: (
            0 if item[0] in preferred_ids else 1,
...
class Player:
    ...
    def builderBot(self, ct: Controller):
        ...
        if (ct.get_current_round() >= LATE_ROUTE_REPAIR_START
                and self._is_waller(ct)):
            self.mapPf.refreshEconomyGraph(ct)
    ...
    def _owns_late_route_repair(
        ...
        wallerEncoded = ct.read_store(SLOT_WALLER_ID)
        preferred = {wallerEncoded - 1} if wallerEncoded else set()
        return choose_route_repairer(
            candidates, target, excluded, preferred) == ct.get_id()
    ...
    def runBestState(self, ct: Controller, myLoc: Position):
        ...
        # Preserve the parent's exact transition. Only after round 80 may one
        # nearest non-specialist finish a short route to an already-built mine.
        ...
        if currentRound >= ECON_CUTOFF:
            ...
            routePos = None
            routeLinks = 4096
            if currentRound >= LATE_ROUTE_REPAIR_START:
                for _, endTile, started, remainingLinks in (
                        self.mapPf.lateEconomyRouteCandidates(ct)):
                ...
                if (routePos is None
                        or not late_route_repair_allowed(
                            currentRound, ct.get_global_resources(),
                            routeLinks)
                        or not self._is_waller(ct)
                        or healScore > 0
                        or not self._owns_late_route_repair(ct, routePos)):
                    routeScore = 0
                    routePos = None
                else:
        ...
        if bestScore == attackScore:
            ...
            if routeLate:
                self.mapPf.routeLateConveyor(ct, routePos)
            else:
