...
# v5 made the mechanism real - 17.1 launches a game, first gun t24 -> t20 - and
# still gated 40% (kills 103-188). Pantheon launches 3-4 times a WHOLE GAME; we
# launched 17. Our attackers kept returning for another ride: walk to the pad,
# stand still to be thrown, land, drift back inside the claim radius, repeat.
# The ride overhead swamped the 7 tiles it bought. One ride per builder, ever.
...
class Player:
    ...
    def _claim_ride(self, ct: Controller, myLoc: Position) -> bool:
        """Attacker-only, once per builder: walk to the pad and claim the throw."""
        if getattr(self, "hasRidden", False):
            return False
        teamCore = self.mapPf.teamCore
        target = self.mapPf.enemyCorePos
        ...
        self.hasRidden = True          # one ride each - never come back
