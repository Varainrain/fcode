...
def choose_recall_mode(core_hp: int, threat_count: int) -> int:
    """Escalate damaged cores fully; hedge early multi-turret pressure once."""
    if threat_count == 0:
        return RECALL_NONE
    if core_hp < RECALL_CORE_HP:
        return RECALL_ALL_SIEGERS
    if threat_count >= 2:
        return RECALL_SIEGER_2
    return RECALL_NONE
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        # A damaged core keeps the proven two-sieger recall. Before damage,
        # multiple real shooters recall only Sieger 2, preserving one race.
        threats = self._home_threats(ct)
        ct.write_store(
            SLOT_RECALL, choose_recall_mode(ct.get_hp(), len(threats)))
    ...
    def _recall_is_up(self, ct: Controller) -> bool:
        mode = ct.read_store(SLOT_RECALL)
        return (
            mode == RECALL_ALL_SIEGERS
            or (mode == RECALL_SIEGER_2 and self._sieger_role(ct) == 2)
        )
