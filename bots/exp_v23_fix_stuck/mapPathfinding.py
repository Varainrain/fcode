...
class MapPathfinder:
    ...
    def moveTo (self, ct: Controller, target: Position):
        ...
        if bestDir is not None and ct.can_move(bestDir):
            ...
            self.stuckTurns += 1
            # FIX 2: our own conveyors are not passable to us, so a builder whose
            # only downhill step is through the home ring finds bestDir = None
            # and then waits 3-10 turns (2 + id%8) before trying anything, and
            # what it finally tries is a RANDOM legal direction. HANDOFF records
            # this same dead-end costing aegis-v1 seven turns "2 tiles short"
            # while the core died.
            # Sidestep on the FIRST blocked turn instead, and pick the legal
            # neighbour that actually reduces distance-to-target rather than a
            # random one. Falls through to the original random walk if no
            # neighbour improves, so genuinely boxed-in units still shake loose.
            stepped = False
            bestSideDist, bestSide = self.distMap[myLoc.x][myLoc.y], None
            for i in CARDINALS:
                nextPos = myLoc.add(i)
                if not (0 <= nextPos.x < self.mapW and 0 <= nextPos.y < self.mapH):
                    continue
                if not ct.can_move(i):
                    continue
                nearbyLaunchers.sort(key=lambda lp: lp.distance_squared(nextPos))
                if nearbyLaunchers and nearbyLaunchers[0].distance_squared(nextPos) < 4:
                    continue
                d2 = self.distMap[nextPos.x][nextPos.y]
                if d2 < bestSideDist:
                    bestSideDist, bestSide = d2, i
            if bestSide is not None:
                ct.move(bestSide)
                self.stuckTurns = 0
                stepped = True
            if not stepped and self.stuckTurns > 2 + (ct.get_id() % 8):
                movableDirs = []
                for i in CARDINALS:
                    nextPos = myLoc.add(i)
                    if ct.can_move(i):
