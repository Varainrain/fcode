...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        myLoc = ct.get_position()
        # THE CORE IS THE BIGGEST EYE WE OWN AND IT WAS DOING NO MAPPING AT ALL.
        # CORE_VISION_RADIUS_SQ is 36 against a builder's 20, and the core never
        # moves, so it observes the widest patch from turn zero - yet runCore
        # never touched mapPf.
        # Why it matters: measured, v29 resolves the enemy core at a MEDIAN of
        # t20, and an attacker cannot aim before it knows where to go, so our
        # first gun lands t22-30 against Pantheon's t7-14. Symmetry is eliminated
        # by observing terrain, and eliminations propagate through the shared
        # mask in slot 8, so anything the core rules out helps every builder.
        # Observer path only: it must not touch shareTiles, whose channel is
        # keyed on myNum and belongs to the builder round-robin.
