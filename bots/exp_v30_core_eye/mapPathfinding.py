...
class MapPathfinder:
    ...
    def setupMapObserver(self, ct: Controller):
        """Map upkeep for an entity that must not touch the builders' channel.

        Skips shareTiles, whose 12-bit slots are keyed on myNum and belong to the
        builder round-robin. Everything else runs, so this entity's vision feeds
        the SHARED symmetry mask (slot 8) that every builder reads.
        """
        self.mapChanged = False
        if self.teamCore is None:
            # A CORE does not appear in its own get_nearby_buildings(), so the
            # usual lookup leaves teamCore None and updateSymmetry() returns
            # immediately - the observer runs and contributes nothing. Take our
            # own position when we ARE the core.
            if ct.get_entity_type() == EntityType.CORE:
                self.teamCore = ct.get_position()
            else:
                for i in ct.get_nearby_buildings():
