        ...
        if etype == EntityType.CORE:
            ...
            if self.mySpot is not None:
                if ct.is_in_vision(self.mySpot):
                    ...
                    # squatter fix v2: only a PERSISTENT squatter (2 consecutive
                    # observations) drops the claim - transients pass through
                    if (tileBId is not None
                            or ct.get_tile_env(self.mySpot) not in [Environment.EMPTY]):
                        self.mySpot = None
                        self._sqSeen = 0
                    elif ct.get_tile_builder_bot_id(self.mySpot) is not None:
                        self._sqSeen = getattr(self, "_sqSeen", 0) + 1
                        if self._sqSeen >= 2:
                            self.mySpot = None
                            self._sqSeen = 0
                    else:
