        ...
        for tile in ct.get_nearby_tiles():
            ...
            if self.seenTiles[x][y] == False or self.fullMap[x][y] != tileType:
                if tileEnv != 'Unpassable':
                    # NEVER MIRROR A BLOCKAGE. Terrain is symmetric;
                    # buildings and units are not. A single enemy builder
                    # walking past marked its tile impassable AND stamped
                    # the same verdict on the mirror tile across the map,
                    # where we may never look again - permanent phantom
                    # walls the spear then pathed around. Treating enemy
                    # builders as passable locally was tried too and is
                    # WORSE (27% vs fp14): the phantom walls were partly
                    # keeping us out of enemy fire. Only the mirroring is
                    # unambiguously wrong.
