...
class Player:
    ...
    def _core(self, ct):
        ...
        if n < 10 and ti >= ct.get_builder_bot_cost():
    ...
    def _builder(self, ct):
        ...
        if self.last is not None and self.last == pos:
            ...
            self.stuck = 0
        self.last = pos
        # THE EIGHT-HEALER TANK: the counter-meta our spear now meets on
        # the ladder - two farmers, one cager, and EIGHT builders parked on
        # the core healing +4 each. 32 hp/turn against a four-sentinel
        # volley's 36 is a stalemate they win on economy.
        if self.num in (1, 2):
            self._farm(ct, pos)
        elif self.num == 3:
            self._cage(ct, pos)
