...
class Player:
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        # FENRIR radar: the core is the only unit whose vision (36) covers
        # every legal sentinel firing tile (32). Publish parked artillery.
        defense.core_publish_radar(ct, myLoc, myTeam)
        ...
        AMMO_CEILING = 24
        # JAVELIN SIEGE staging. 1 = landed, mortar unfunded: throttle the
        # ammo pipe so the sentinel lump can form (measured: at ceiling 24
        # the conversion + attacker gunners pinned Ti under 61 for 60 turns
        # and the mortar never stood). 2 = mortar firing: open the pipe -
        # it drinks 10 every 2 rounds (Jython burned ~490 into one core).
        # (radar ammo throttle removed: it was BACKWARDS - when artillery
        # parks on us our defense gunners need MORE ammo, not less. Gated
        # 1/10 on nordkap with it; the CB lump has to compete fairly.)
        _siegePhase = ct.read_store(javelin.SLOT_SIEGE)
        if _siegePhase == 1:
            AMMO_CEILING = 12
            # GC: if the javelin dies before the mortar stands, nobody else
            # clears the flag and the war chest would starve eco forever. A
            # living javelin re-writes 1 every turn, so clearing after 50
            # quiet turns only ever kills a STALE flag.
            self._s1turns = getattr(self, "_s1turns", 0) + 1
            if self._s1turns > 50:
                ct.write_store(javelin.SLOT_SIEGE, 0)
                self._s1turns = 0
        elif _siegePhase == 2:
            AMMO_CEILING = 60
            self._s1turns = 0
    ...
    def builder_bot(self, ct: Controller):
        ...
        # JAVELIN: builder JAV_NUM is the relay infiltrator. It never recalls
        # and never does eco - tempo is the entire strategy (Jython reached
        # the enemy core at t12 on a coreDist-48 map).
        if self.mapPf.myNum in javelin.JAV_NUMS:
            if javelin.run_javelin(ct, self):
                return
        ...
        if self.mapPf.myNum in DEFEND_NUMS and self.mapPf.teamCore is not None:
            ...
            # FENRIR: the defender answers the meta's kill mechanism -
            # counter-battery against parked sentinels (indirect fire cuts
            # both ways; their walls cannot protect their artillery from
            # ours). The ring vaccine was cut: our eco already occupies the
            # ring with conveyors for free, and barriers there just stole
            # the delivery hub from our own economy.
            if defense.counter_battery(ct, self, myLoc):
                return
