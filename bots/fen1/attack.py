...
def run(ct: Controller, player):
    ...
    enemyCore = player.mapPf.enemyCorePos
    myTeam = ct.get_team()
    # TRANSPORT ONLY PAYS ON LONG TRIPS (measured: on coreDist-14 royale the
    # bus - walk to launcher, wait, throw - cost more turns than walking and
    # gated us 0/10 there; Jython's own first launcher averages t~300).
    # Long-haul reinforcements only: trips > 16, round >= 60, forward rides.
    farAway = (enemyCore is not None
               and core_footprint_manhattan(myLoc, enemyCore) > 16
               and ct.get_current_round() >= 60)
    if farAway:
        bestL, bestD = None, None
        myD = core_footprint_manhattan(myLoc, enemyCore)
        for b in ct.get_nearby_buildings():
            ...
            if d > myD - 2:
                continue                    # not strictly ahead: no detours
        ...
        if bestL is not None:
            ...
            # WALLED TERRAIN GUARD: on corridor maps (drakkarfjord 1/10,
            # ragnarok 2/10 vs open midgard 6/10) throws have nowhere to
            # land and chain launchers wedge into dead ends. Only lay chain
            # where the terrain ahead is mostly open.
            wallCnt = 0
            checked = 0
            for tile in ct.get_nearby_tiles():
                if core_footprint_manhattan(tile, enemyCore) >= core_footprint_manhattan(myLoc, enemyCore):
                    continue
                checked += 1
                if ct.get_tile_env(tile) == Environment.WALL:
                    wallCnt += 1
            if not (checked >= 8 and wallCnt * 5 > checked):
                cands = []
                for d in CARDINALS:
                    n = myLoc.add(d)
