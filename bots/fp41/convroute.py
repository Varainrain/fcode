...
def _conveyor_tile_cost(ct: Controller, player, tile: Position, curEnd: Position):
    ...
    if tileId is not None:
        ...
        if tTeam != ct.get_team():
            if tType in (EntityType.BARRIER, EntityType.CONVEYOR):
                # Cheap to shoot out of the way, so routable rather than impassable.
                return 16, 'working'
            return None, 'stuck'
    ...
    if tileId is not None:
        ...
        if tTeam == ct.get_team() and tType in (EntityType.BARRIER, EntityType.GUNNER):
            if ct.can_destroy(curEnd):
                ct.destroy(curEnd)
                return
    # ANTI-BLOB. MEASURED on the v206 game ic3d screenshotted (0b8fb835 g2):
    # 68 conveyor tiles of which 54% had THREE OR MORE conveyor neighbours,
    # and the largest connected group was 33 tiles filling 61% of a 6x9 box
    # fifteen tiles from our core, laid between t139 and t293. A delivery
    # chain is a LINE - about two neighbours per tile - so that is roughly a
    # hundred titanium of conveyor that carries nothing, while the same games
    # showed 10-17 conveyors per harvester against O(1)'s 3.5-7.8.
    # A tile already touching three of our conveyors is inside saturated
    # ground: whatever route we think we are extending, it is already served.
    nbrs = 0
    for _d in CARDINALS:
        _n = curEnd.add(_d)
        if not (0 <= _n.x < mapPf.mapW and 0 <= _n.y < mapPf.mapH):
            continue
        if not ct.is_in_vision(_n):
            continue        # get_tile_building_id raises outside vision
        _id = ct.get_tile_building_id(_n)
        if (_id is not None and ct.get_team(_id) == ct.get_team()
                and ct.get_entity_type(_id) == EntityType.CONVEYOR):
            nbrs += 1
    if nbrs >= 3:
        return
