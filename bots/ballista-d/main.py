...
# DEFENSIVE LAUNCHER (the un-tried launcher concept after 14 tempo-opening
# failures): khaos's Cambridge trick under the 2.3.4 defense meta — a pad
# near OUR core that throws ENEMY builders away before they can seat a
# turret. Removing a sieger costs the launcher one action; the enemy paid
# 20+ turns of walking. Launcher docs (Aug 3) confirm: works on either
# team's bots. Self-contained, no mapPf (launchers have no map — landmine 1
# in LAUNCHER-HANDOFF.md).
# slot 9 = defensive pad built flag   slot 13 = pad pos, packed+1
SLOT_DPAD_FLAG = 9
SLOT_DPAD_POS = 13
DPAD_TRIGGER = 7      # enemy builder this close to our core -> build the pad
DPAD_MIN_TI = 45      # never starve the economy for it (pad costs 20)
...
class Player:
    ...
    def maybeBuildDefPad(self, ct: Controller) -> bool:
        """Defender builds ONE launcher beside our core once an enemy
        builder gets close. Returns True if the turn was used."""
        if ct.read_store(SLOT_DPAD_FLAG) != 0:
            return False
        if ct.get_global_resources() < DPAD_MIN_TI:
            return False
        ...
        for b in ct.get_nearby_entities():
            try:
                if ct.get_team(b) == ct.get_team():
                    continue
                if ct.get_entity_type(b) != EntityType.BUILDER_BOT:
                    continue
                bp = ct.get_position(b)
                if abs(bp.x - core.x) + abs(bp.y - core.y) <= DPAD_TRIGGER:
                    threat = True
                    break
            except Exception:
                continue
        if not threat:
            return False
        # pad on any free tile at manhattan 2 of the core footprint —
        # close enough that its pickup range (1-2) covers the seat zone
        for dx in range(-2, 4):
            for dy in range(-2, 4):
                pad = Position(core.x + dx, core.y + dy)
                if not (0 <= pad.x < self.mapW and 0 <= pad.y < self.mapH):
                    continue
                dcore = min(abs(pad.x - c[0]) + abs(pad.y - c[1])
                            for c in ((core.x, core.y), (core.x + 1, core.y),
                                      (core.x, core.y + 1), (core.x + 1, core.y + 1)))
                if dcore != 2:
                    continue
                dme = abs(myLoc.x - pad.x) + abs(myLoc.y - pad.y)
                if dme == 1 and ct.can_act() and ct.can_build_launcher(pad):
                    ct.build_launcher(pad)
                    ct.write_store(SLOT_DPAD_FLAG, 1)
                    ct.write_store(SLOT_DPAD_POS, packPos1(pad))
                    return True
        return False

    def runLauncher(self, ct: Controller) -> None:
        """Throw any adjacent ENEMY builder as far from our core as legal.
        No mapPf (landmine 1) — our core comes from slot 7."""
        me = ct.get_position()
        core = unpackPos1(ct.read_store(7) + 1) if ct.read_store(7) else None
        if core is None:
            return
            ...
            for dy in (-2, -1, 0, 1, 2):
                ...
                try:
                    if not ct.is_in_vision(spot):
                        continue
                    bid = ct.get_tile_builder_bot_id(spot)
                ...
                for tile in ct.get_nearby_tiles(26):
                    try:
                        if not ct.can_launch(spot, tile):
                            continue
                    except Exception:
                        continue
                    dc = abs(tile.x - core.x) + abs(tile.y - core.y)
                    if best is None or dc > best[0]:
                        best = (dc, tile)
                cur = abs(spot.x - core.x) + abs(spot.y - core.y)
                if best is not None and best[0] > cur + 2:
                    try:
                        ct.launch(spot, best[1])
                    except Exception:
                        pass
                    return

    def runDefend(self, ct: Controller):
        if self.maybeBuildDefPad(ct):
            return
        myLoc = ct.get_position()
        myTeam = ct.get_team()
