...
class Player:
    ...
    def runAttack(self, ct: Controller):
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            # A7: a gunner with an empty pool is 20+ Ti of scenery. A shot costs
            # 4 ammo and the core refills from titanium 1:1, so when the pool is
            # dry the same titanium is worth more left in the bank as ammo than
            # spent on a turret that cannot fire. Measured: 19.2% of attacker
            # turns run with the pool under one shot.
            if ct.get_global_resources() >= 96 and ct.get_global_ammo() >= 4:
