...
def choose_home_threat(threats, covered_positions, sequential: bool):
    """Keep one target until real damage justifies advancing the responder."""
    if not threats:
        return None
    if sequential:
        for threat in threats:
            if threat[4] not in covered_positions:
                return threat[4]
    return threats[0][4]
        ...
        if threats:
            ...
            if sequential:
                covered_positions = {
                    threat[4]
                    for threat in threats
                    if self._friendly_gunner_covers(ct, threat[4])
                }
            self._homeThreatPos = choose_home_threat(
                threats, covered_positions, sequential)
