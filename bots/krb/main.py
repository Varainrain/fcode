...
# KHAOS-REBORN synchronized opening: strikers (myNum >= 3) converge on the
# predicted enemy core, stage together, and push as ONE wave. act-xor-move
# favors pre-positioned attackers — a trickle siege gets killed piecemeal;
# a wave plants several guns before the defense can answer.
STRIKE_STAGE_D2 = 60     # hold at ~7-8 tiles out
STRIKE_GO = 3            # buddies within Chebyshev 4 to trigger the push
STRIKE_SOLO_ROUND = 90   # stop waiting for a full wave after this round
STRIKE_LAST_ROUND = 220  # opening expires; normal khaos life resumes
...
class Player:
    ...
    def run(self, ct: Controller) -> None:
        ...
        # turn). One bad tile query must never cost a unit — swallow and
        # live to act next round.
        try:
    ...
    def builder(self, ct: Controller) -> None:
        ...
        striking = (not getattr(self, "strike_done", False)
                    and self.myNum >= 3 and rnd <= STRIKE_LAST_ROUND
                    and self._strike(ct, mi))
        if not striking:
            states = (
        ...
        self._ringGuns(ct, mi)
        self._tryChokeBlock(ct, mi)
        self._denyHomeSiege(ct, mi)
    ...
    def _run_attack(self, ct, mi, data):
        ...
        if kind == 'plant':
            ...
            facing = {'E': Direction.EAST, 'W': Direction.WEST,
                      ...
                      'SE': Direction.SOUTHEAST, 'SW': Direction.SOUTHWEST}[fk]
            tb = mi.bit(px, py)
            if pos == tile:
    ...
    def _run_explore(self, ct, mi, _):
        ...
        if self.exploreTarget is None or self.stuck > 4:
            # home guard: patrol the undenied siege-ray tiles first so the
            ...
            if self.myNum == 1 and self.teamCore is not None:
                ...
                for (x, y) in self._denyTiles():
                    ...
                    if (mi.walls | mi.blocked | mi.own_barriers) & b:
                        continue
                    if ct.is_in_vision(p) and (
                            ct.get_tile_env(p) == Environment.ORE_TITANIUM
                            or ct.get_tile_building_id(p) is not None):
                        continue
                    todo.append((x, y))
                if todo:
    ...
    def _siegeRayTiles(self, core):
        """The tiles a gunner could shoot the given 2x2 core from: cardinal
        rays off each footprint row/col at distance 2-3 (range^2 13 caps the
        ray at 3). Distance-1 ring is left free — ours spawns there, theirs
        is where our own ring guns go."""
        mi = self.mi
        tiles = []
        for o in (0, 1):
            for d in (2, 3):
                tiles.append((core.x - d, core.y + o))       # west ray
    ...
    def _strike(self, ct, mi) -> bool:
        """Khaos-reborn opening. Returns True while it owns this builder's
        turn: sprint to the predicted enemy core, stage at ~8 tiles, push
        as a wave once buddies gather; _ringGuns (passives) does the
        planting once adjacent. Hands back to normal states when guns are
        standing or the opening expires."""
        eg = self._enemyCoreGuess()
        if eg is None:
            return False
        if getattr(self, "strike_guns", 0) >= 2:
            self.strike_done = True   # my wave contribution is planted
            return False
        pos = ct.get_position()
        d2 = pos.distance_squared(eg)
        if d2 <= 8:
            return True   # in planting range — passives build, hold here
        if d2 > STRIKE_STAGE_D2:
            self.target = eg
            self._moveToward(ct, mi.expand(mi.bit(eg.x, eg.y)) & mi.passable())
            return True
        buddies = 0
        for lsb in mi.iter_bits(mi.own_bots):
            x, y = mi.xy(lsb)
            if max(abs(x - pos.x), abs(y - pos.y)) <= 4:
                buddies += 1
        rnd = ct.get_current_round()
        if buddies >= STRIKE_GO or rnd >= STRIKE_SOLO_ROUND:
            self.target = eg
            self._moveToward(ct, mi.expand(mi.bit(eg.x, eg.y)) & mi.passable())
        return True   # staged and waiting is also owning the turn
    ...
    def _denyHomeSiege(self, ct, mi):
        ...
        has nowhere to stand. Denial beats reaction (kdef gates proved
        reacting after the siege stands is too late)."""
        if ct.get_action_cooldown() != 0 or self.teamCore is None:
            return
        ...
        for (x, y) in self._denyTiles():
            ...
            if not ct.is_in_vision(p):
                continue
            if ct.get_tile_env(p) in (Environment.WALL,
    ...
    def _ringGuns(self, ct, mi):
        """v85's proven kill move made deliberate: keep up to 3 gunners ON
        the enemy core's adjacent ring. A ring gunner one-clips every spawn
        (reload 1) and chips the core — their economy stops either way."""
        if ct.get_action_cooldown() != 0 or self.enemyCore is None:
            return
        ec = self.enemyCore
        pos = ct.get_position()
        if pos.distance_squared(ec) > 18:
            return
        if ct.get_global_resources() < ct.get_gunner_cost() + 40:
            return
        near = sum(1 for (x, y) in mi.own_turrets
                   if abs(x - ec.x) + abs(y - ec.y) <= 3)
        cap = 5 if (self.myNum >= 3
                    and not getattr(self, "strike_done", False)) else 3
        if near >= cap:
            return
        foot = {(ec.x + a, ec.y + b) for a in (0, 1) for b in (0, 1)}
        ring = [(x, y) for x in range(ec.x - 1, ec.x + 3)
                for y in range(ec.y - 1, ec.y + 3)
                if (x, y) not in foot
                and 0 <= x < mi.w and 0 <= y < mi.h]
        for (x, y) in sorted(ring, key=lambda t: (pos.x - t[0]) ** 2
            ...
            if not ct.is_in_vision(p):
                continue
            facing = self._cardinalTo(p, Position(ec.x, ec.y))
            if facing == Direction.CENTRE:
                continue
            if ct.can_build_gunner(p, facing):
                ct.build_gunner(p, facing)
                mi.own_turrets[(x, y)] = 'G'
                self.strike_guns = getattr(self, "strike_guns", 0) + 1
                return
