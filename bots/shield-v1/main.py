...
class Player:
    ...
    def passiveWall(self, ct: Controller, myLoc: Position):
        ...
        # SHIELD PIECES (both Cambridge top-2 teams do this; we had only ever
        # ringed the CORE, never harvesters): deny the tiles orthogonally
        # adjacent to our own harvesters so an enemy cannot plant a turret
        # point-blank on the mine. Same passive contract as the core wall —
        # spare action only, zero movement, zero state priority — because the
        # measured lesson is that the ~11pt cost is the RESTRICTION, not the
        # walling. Reuses the never-a-dead-end rule below unchanged, so a
        # shield tile only gets built where it can chain into the harvester's
        # existing line (i.e. once the mine is actually worth protecting).
        shield = set()
        for b in ct.get_nearby_buildings():
            try:
                if (ct.get_team(b) == ct.get_team()
                    ...
                    for d2 in CARDINALS:
                        q = hp.add(d2)
                        if 0 <= q.x < self.mapW and 0 <= q.y < self.mapH:
                            shield.add((q.x, q.y))
            except Exception:
                continue
        for d in CARDINALS:
            wp = myLoc.add(d)
            if (wp.x, wp.y) not in self._wallSet and (wp.x, wp.y) not in shield:
                continue
