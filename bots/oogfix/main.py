...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if RECALL:
            # SEVERITY, not a flag: 2 = critical (all hands), 1 = hurt
            # (split duty). Binary recall forced a false choice -- all-heal
            # halts the economy (measured: 172 arbiter turns in 246 rounds),
            # split-duty alone loses cores (kills against 58 vs 46).
            _hp = ct.get_hp()
            hurt = 2 if _hp < 250 else (1 if _hp < RECALL_HP else 0)
    ...
    def builderBot(self, ct: Controller):
        ...
        if RECALL:
            try:
                # SPLIT-DUTY RECALL (our own measured law: all-heal lock
                # gated 42%, one-healer-per-round + rest keep fighting gated
                # 4-1). Instrumented on eider: runBestState was reached only
                # 172 times in 246 rounds because EVERY eco builder on our
                # half answers recall and returns -- the economy stops
                # entirely while the core is hurt (100 Ti mined vs 600).
                # Alternate by round so healing continues but eco never
                # halts: half the crew nurses, half keeps working.
                _rc = ct.read_store(RECALL_SLOT)
                if (_rc == 2 or (_rc == 1
                                 and (ct.get_current_round() + self.mapPf.myNum) % 2 == 0)):
    ...
    def scoreProtect(self, ct: Controller, myLoc, uncoveredTurrets):
        ...
        if not uncoveredTurrets or ct.get_global_resources() < ct.get_gunner_cost():
            return 0, None
        # SCOPE (added with the protectTask typo fix). The docstring above
        # says distance-gating caused a 0-8 drumlin/eider regression -- but
        # that was measured while protectTask raised NameError on every call
        # ('tatrget' param vs 'target' body), i.e. while this state did
        # NOTHING. With the state actually working, ungated response chases
        # every turret on the map: eider mined 120 Ti vs 2480 (0/10).
        # A turret is our problem when it threatens our core or is close
        # enough to reach without crossing the map.
        home = self.mapPf.teamCore
        near = []
        for g in uncoveredTurrets:
            if g.distance_squared(myLoc) <= 50:
                near.append(g)
            elif home is not None and coreFootprintManhattan(g, home) <= 8:
                near.append(g)
        if not near:
            return 0, None
        target = min(near, key=lambda g: g.distance_squared(myLoc))
