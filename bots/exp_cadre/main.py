...
# Every allocation test so far held the builder POPULATION fixed and moved roles
# around, which always robbed income: 2 siegers -> 4.6 guns + 4.0 harvesters,
# 3 -> 0 and 0, all -> 4.5 and 0. Opportunistic delivery failed too, because
# economy builders never travel far enough to get the chance.
# So grow the population instead. A fixed economy CADRE (the waller plus the
# first two builders) keeps farming forever; every builder spawned after them is
# a sieger. Delivery then scales with income instead of competing with it.
# Builders cost 30 Ti and we sit on ~470 unspent at t45, so the spawn threshold
# is the thing actually capping our gun count.
CADRE_SIZE = 3
SPAWN_BASE = 40      # was 80 + 60*n; builders are the cheapest gun delivery we have
SPAWN_STEP = 30
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if self.fiveDirections and len(self.fiveDirections) > 0: # only the first 5 bots spawned should be there
            ...
            if ct.can_spawn(spawnPos):
                ...
                if self.numSpawned + 1 == CADRE_SIZE:
                    ...
                    # without needing a store slot per unit.
        ...
        if globalTitanium > SPAWN_BASE + SPAWN_STEP * self.numSpawned:
    ...
    def _is_sieger(self, ct: Controller) -> bool:
        ...
        return cutoff != 0 and encoded_id > cutoff
