...
SIMPLE_CORES = {
    ...
    (20, 20): frozenset({(1, 16), (17, 2),      # icefloe
                         (9, 1), (9, 17)}),     # auroraveil (14/14 vs gungnir)
    # royale (9,2)/(9,16) and antler (6,4)/(6,12) were TESTED here and REJECTED:
    # royale 0/14 and antler 3/14 when added. royale in particular reads 14/14
    # for a rush carrying the rotation features and 0/14 without them - an
    # unresolved n=14 contradiction, so it stays out until that is understood.
    (30, 30): frozenset({(14, 2), (14, 26),     # glacierkeep
                         (2, 24), (26, 4)}),    # drakkarfjord
    # drumlin + drakkarfjord added 08-17: 50% vs own eco (PERFECTLY
    # side-deterministic, 3 seeds each way - drumlin wins going first t47,
    # drakkarfjord wins going second t61). Below the beat-what-it-replaces
    # bar in the MIRROR, but in the field these two maps were near-auto-loss
    # to mid-table waves (Bisons kill t57/t73) while a closing rush outraces
    # waves (nordkap t29 / glacierkeep t44 vs the same team). Coinflip floor,
    # race ceiling. Banded scrimbot is the referee - pull them if the band
    # says otherwise.
...
class Player:
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        if self.numSpawned < 5 or (globalTitanium > 360 and curRound - self.lastSpawn >= 8):
            self.spawnBot(ct)
        ...
        # GRIND ALARM (slot 11): consecutive rounds with a turret threat in
        # core vision. A wave passes in a few rounds; a GRIND (jav1/farming/
        # HTTP 418 class - gunner mass parked at our core) keeps this hot for
        # dozens. Builders read it to convert a walker into a defender while
        # the pressure lasts.
        if enemyTurretSeen:
            self._threatRounds = getattr(self, "_threatRounds", 0) + 1
        else:
            self._threatRounds = 0
        ct.write_store(11, min(self._threatRounds, 30))
    ...
    def builder_bot(self, ct: Controller):
        ...
        # GRIND RESPONSE: while the core's threat has been hot 8+ consecutive
        # rounds (slot 11), walker #3 fights at home instead of marching -
        # the fixed 1-defender budget is what loses the 2-3s to gunner-mass
        # grinds (jav1 referee 42%). Reverts the moment pressure lifts.
        _foe = self.mapPf.enemyCorePos
        if (ct.read_store(11) >= 8 and self.mapPf.myNum == 3
                and self.mapPf.teamCore is not None
                and (_foe is None
                     or myLoc.distance_squared(self.mapPf.teamCore)
                        < myLoc.distance_squared(_foe))):
            # only while still on OUR half - a committed siege stays committed
            self.mapPf.setMoveZone(self.mapPf.teamCore, self.far_core_corner())
            eco.run(ct, self)
        elif self.mapPf.myNum % ATTACK_MOD == 0:
            attack.run(ct, self)
        else:
            eco.run(ct, self)
