...
# TAXI LAUNCHER (HTTP 418 pattern): builder #1 drops ONE launcher beside the
# core by ~r12, from the opening bank. Walkers board it (attack.run) and get
# thrown toward the enemy instead of walking. 20 Ti for 20-40 turns of
# arrival-race time on every walker for the rest of the game.
# PARKED at default-off (2026-08-18): the taxi is mechanically sound
# (midgard mirror core-kill t119 vs the usual t300+) and clean vs waves
# (70% wavebot), but REGRESSED the siege referee 42->30-33% across three
# gates - thrown walkers arrive ALONE into jav1's midfield gunner mass and
# die solo, exactly like un-supported defensive seats did. HTTP 418's
# version works because their deliveries land beside an ESTABLISHED
# forward gunner line. Next iteration: throw toward OUR forwardmost
# turret cluster (not the enemy core), or hold boarding until 2 walkers
# queue so deliveries arrive in pairs.
TAXI_ENABLE = _fi("TAXI_ENABLE", 1)   # v2: pair delivery - regate before ship
TAXI_ROUND_MAX = _fi("TAXI_ROUND_MAX", 14)
TAXI_SCORE = 8.5   # above economy (<=8), below picket (9) - picket lands first
...
def taxi_task(ct, player, myLoc):
    ...
    if foe is None:
        return
    # any own launcher in sight = job done (also catches a teammate's build)
    for b in ct.get_nearby_buildings():
