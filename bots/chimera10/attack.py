...
# TAXI (HTTP 418 pattern, match 8fffdc5d - they 4-1'd v133 with full eco plus
# ONE t2 launcher throwing builders toward the action): a walker rides the
# home launcher instead of walking 10-20 tiles. Throws are free and land
# dist-sq<=26 out, so the arrival race shortens by 20-40 turns while the
# economy keeps every conveyor. Boarding protocol needs no store traffic:
# the pickup tile is the launcher's enemy-ward neighbor - eco builders never
# idle exactly there, walkers path to it deliberately.
def taxi_pickup_tile(launcherPos, foe):
    dx = (foe.x > launcherPos.x) - (foe.x < launcherPos.x)
    dy = (foe.y > launcherPos.y) - (foe.y < launcherPos.y)
    return Position(launcherPos.x + dx, launcherPos.y + dy)


def run_taxi(ct: Controller, player):
    """The launcher entity: throw any own builder on the boarding tile as far
    toward the enemy core as a legal landing allows."""
    myLoc = ct.get_position()
    player.mapPf.setupMap(ct)
    foe = player.mapPf.enemyCorePos
    if foe is None and player.mapW:
        compact = ct.read_store(7)
        foe = Position(player.mapW - ((compact >> 5) & 0x1F) - 2,
                       player.mapH - (compact & 0x1F) - 2)
    if foe is None:
        return
    pick = taxi_pickup_tile(myLoc, foe)
    bbId = ct.get_tile_builder_bot_id(pick)
    if bbId is None or ct.get_team(bbId) != ct.get_team():
        player._taxi_hold = 0
        return
    # PAIR DELIVERY (v2 - solo throws gated 30-33% vs jav1: lone arrivals die
    # to gunner mass): hold the throw until a SECOND walker is adjacent, so
    # deliveries land together. Give up holding after 6 rounds - a slow pair
    # beats no delivery, but a stale solo beats an eternal wait.
    myTeam = ct.get_team()
    nAdjacent = 0
    for u in ct.get_nearby_units():
        if (ct.get_team(u) == myTeam
                and ct.get_entity_type(u) == EntityType.BUILDER_BOT
                and ct.get_position(u).distance_squared(myLoc) <= 2):
            nAdjacent += 1
    hold = getattr(player, "_taxi_hold", 0)
    if nAdjacent < 2 and hold < 6:
        player._taxi_hold = hold + 1
        return
    player._taxi_hold = 0
    cands = []
    for dx in range(-5, 6):
        for dy in range(-5, 6):
            ...
            if 0 <= t.x < player.mapW and 0 <= t.y < player.mapH:
                cands.append(t)
    cands.sort(key=lambda t: t.distance_squared(foe))
    for t in cands[:15]:   # TLE budget: try the 15 best landings, not all 120
        if ct.can_launch(pick, t):
            ct.launch(pick, t)
            return
...
def run(ct: Controller, player):
    ...
    # TAXI RIDE: a walker still on its own half boards the home launcher.
    _foe = player.mapPf.enemyCorePos
    _tc = player.mapPf.teamCore
    # round-gated: early walkers walk (their slow transit through our half is
    # incidental home defense - smokes lost royale/valkyrie without it);
    # mid/late walkers ride. NO BOARDING UNDER SIEGE: slot 7's upper bits are
    # the core's live turret-threat broadcast - if anything is shelling home,
    # walkers stay grounded (taxi gated 33% vs jav1 without this, 42% before
    # the taxi existed: it was shipping defenders out mid-siege).
    if (_foe is not None and _tc is not None
            and ct.get_current_round() > 20
            and (ct.read_store(7) >> 10) == 0
            and myLoc.distance_squared(_foe) > myLoc.distance_squared(_tc)
            and getattr(player, "_taxi_wait", 0) < 12):   # pair-wait patience
        for _b in ct.get_nearby_buildings():
            if (ct.get_team(_b) == ct.get_team()
                    and ct.get_entity_type(_b) == EntityType.LAUNCHER):
                _lp = ct.get_position(_b)
                if myLoc.distance_squared(_lp) <= 20:
                    _pick = taxi_pickup_tile(_lp, _foe)
                    player._taxi_wait = getattr(player, "_taxi_wait", 0) + 1
                    if myLoc.x == _pick.x and myLoc.y == _pick.y:
                        return          # hold still - the launcher throws us
                    player.mapPf.moveTo(ct, _pick)
                    player.draw_state(ct, C_MARCH_ATTACK, _pick)
                    return
                break
...
def run_gunner(ct: Controller, player):
    ...
    for directionIndex, d in enumerate(DIRECTIONS):
        ...
        selfDefHits = gunnerThreatHits = 0
        ...
        score = (gunnerThreatHits, coreThreatHits, gunnerHits, coreHits,
    ...
    if bestDir != myDir:
        _, _, gunnerHits, _, builderThreatHits, _, _ = bestScore
        ...
        # second way through.
        gunnerThreatHits = bestScore[0]
        coreThreatTop = bestScore[1]
        coreHitsTop = bestScore[3]
