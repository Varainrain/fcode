...
def run(ct: Controller, player):
    ...
    # FRONT BOOTSTRAP (rebuild-forever panel: jav1 unmoved at 44 because the
    # front waited for a forward GUNNER BUILD - exactly what grinders
    # prevent, so routing/trickle/wartime-spawns stayed inert vs their
    # designed target). The front now establishes on CROSSING: the first
    # walker into the enemy half writes a provisional front at its own
    # position; forward gun builds refine it later.
    _foeB = player.mapPf.enemyCorePos
    _tcB = player.mapPf.teamCore
    if (_foeB is not None and _tcB is not None
            and ct.read_store(SLOT_FRONT) == 0
            and myLoc.distance_squared(_foeB) < myLoc.distance_squared(_tcB)):
        ct.write_store(SLOT_FRONT, ((myLoc.x << 5) | myLoc.y) + 1)
