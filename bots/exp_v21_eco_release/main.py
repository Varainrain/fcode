...
class Player:
    ...
    def runEco(self, ct: Controller):
        ...
        if uncovered:
            ...
            self.mapPf.moveTo(ct, target)
            return
        # Covered threats belong to the fixed defender roles. The inherited
        # unconditional heal/return recalled every economy builder even when
        # the core was full or healing was unavailable, permanently deleting
        # the economy under harmless sustained pressure. Economy still counters
        # every uncovered gun above; once coverage exists it resumes its job.
