...
class Player:
    ...
    def _wouldEntomb(self, ct: Controller, spot: Position, myTeam) -> bool:
        # Would an IMPASSABLE building at `spot` help seal the core pocket?
        # Match 0788a40d g1: conveyors + defensive gunners sealed every
        # exit around our core; all 14 builders spawned into the tomb and
        # parked for 450 turns (2 harvesters all game, tiebreak loss -
        # ic3d watched it live). Rule: near the core, never place an
        # impassable building that leaves fewer than 3 OPEN exit tiles
        # (empty, or own barrier - passable to us) around the footprint.
        tc = self.mapPf.teamCore
        if tc is None:
            return False
        dmin = min(abs(spot.x - c[0]) + abs(spot.y - c[1])
                   for c in ((tc.x, tc.y), (tc.x + 1, tc.y),
                             (tc.x, tc.y + 1), (tc.x + 1, tc.y + 1)))
        if dmin > 3:
            return False
        openExits = 0
        for dx in (-1, 0, 1, 2):
    ...
    def buildGunnerFor(self, ct: Controller, target: Position) -> bool:
        ...
        for d in CARDINALS:
            ...
            if dist < 10 and dist != 5 and 0 <= spot.x < self.mapW and 0 <= spot.y < self.mapH:
                if self._wouldEntomb(ct, spot, myTeam):
                    continue
