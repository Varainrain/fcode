...
class MapPathfinder:
    ...
    def conveyorTileCostSafe(self, ct: Controller, tile: Position, enemyThreatened, curEnd: Position):
        ...
        # churn death-pricing removed (v84b experiment): Oogway's memoryless
        # router deleted it and dominates the tempo maps -- testing whether
        # the +45 detours are the drag that kept the merge from acquiring
        # his expansion speed. (Receipt was lighthouse churn vs Pantheon --
        # possibly meta-stale now that cut-and-cap exists on both sides.)
        return [cost, status]
