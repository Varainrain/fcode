...
try:                                   # env knobs are LAB-ONLY: if the
    import os                          # server sandbox blocks os, fall
    _env = os.environ.get              # back to shipped defaults
except Exception:
    _env = lambda k: None
...
S_PROTECT    = _fp("S_PROTECT", 10.0)
S_HEAL_CORE  = _fp("S_HEAL_CORE", 12.0)
S_HB_FLOOR   = _fp("S_HB_FLOOR", 8.5)
S_HB_MISSING = int(_fp("S_HB_MISSING", 1))
S_RC_FLOOR   = _fp("S_RC_FLOOR", 3.2)
S_RC_CAP     = _fp("S_RC_CAP", 4.0)
S_RH_FLOOR   = _fp("S_RH_FLOOR", 2.2)
S_RH_CAP     = _fp("S_RH_CAP", 3.2)
S_HV_FLOOR   = _fp("S_HV_FLOOR", 1.2)
S_HV_CAP     = _fp("S_HV_CAP", 2.2)
S_CB         = _fp("S_CB", 9.8)        # counter-battery: kill the artillery source
S_JAM        = _fp("S_JAM", 9.6)       # muzzle plug
S_SEAL       = _fp("S_SEAL", 9.4)      # firing-line barriers
    ...
    def runEco(self, ct: Controller):
        ...
        for g in self.broadcastGunners(ct):
            if (g.x, g.y) not in enemyTurretKeys and (g.x, g.y) not in covered:
                if myLoc.distance_squared(g) <= 32:
        ...
        # MEMORYLESS ARBITER (Oogway's layer) with the defense stack in its
        # score band: heal(12) > protect(10) > CB(9.8) > jam(9.6) > seal(9.4)
        # > healBuild(<=8.5+) > routing > harvest. Defense tasks keep their
        # own claims/bounds; the band only decides WHEN they may bid.
        scores = {
        ...
        # v84c ISOLATE: defense band silenced (matches oogstates' accidental
        # dead seal) -- measures whether CB/jam/seal eco-diversion is the
        # residual drag vs the pure arbiter
