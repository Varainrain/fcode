    ...
    def runGunner(self, ct: Controller):
        ...
        for directionIndex, d in enumerate(DIRECTIONS):
            ...
            for tile in ct.get_attackable_tiles_from(myPos, d, EntityType.GUNNER):
                # A9: the shot stops at the first thing on the ray, so score
                # only that tile. Summing the whole ray made a facing that
                # hits an enemy conveyor at range 1 and their core at range 2
                # score a core hit it can never land.
