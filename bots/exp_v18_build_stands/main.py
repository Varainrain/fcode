...
class Player:
        ...
        if self.mapPf.enemyCorePos is None:
            ...
            if ct.get_global_resources() >= 80:
                ...
                if gunnerStuff:
                    ...
                    if myDist < 1:
                        # Production 2.3.3 only permits orthogonally adjacent
                        # builds. Step behind the planned seat instead of
                        # abandoning the siege and walking all the way home.
                        self.stepOffBuildTile(
                            ct, self.mapPf.enemyCorePos, preferFarther=True)
                    elif myDist > 1:
                        self.mapPf.moveTo(ct, gunnerSpot)
                    return
            self.mapPf.moveTo(ct, self.mapPf.enemyCorePos)

    def stepOffBuildTile(self, ct: Controller, reference: Position,
                         preferFarther: bool) -> bool:
        """Leave an intended build tile while staying cardinally adjacent."""
        myLoc = ct.get_position()
        dirs = list(CARDINALS)
        dirs.sort(
            key=lambda d: myLoc.add(d).distance_squared(reference),
            reverse=preferFarther,
        )
        for d in dirs:
            if ct.can_move(d):
                ct.move(d)
    ...
    def harvestTask(self, ct: Controller, myLoc, myTeam) -> bool:
        ...
        if bestTile.distance_squared(myLoc) > 1:
            ...
            # As with gun seats, an ore tile is not a legal same-tile build.
            # Step toward the core so the harvester can be built next turn.
            self.stepOffBuildTile(ct, teamCore, preferFarther=False)
        return True
