...
class Player:
    ...
    def healCore(self, ct: Controller, home):
        ...
        if teamCore is None:
            return
        # ECON BOOTSTRAP OUTRANKS HEALS (72d9266b poverty trap: with h1,
        # continuous 1-Ti heals ate every coin, we never saved 20 for a
        # 2nd harvester and built NOTHING t15-t103. team lazy - who 5-0d
        # us doing this - bootstraps h2 FIRST, then heals 536/game. A flat
        # Ti floor gated 35% because always-healers win mirror races; this
        # gates on INCOME instead: until 2 harvesters ever built, heals
        # only for a critical core. slot 15 = harvesters ever built.)
        try:
            # v53 (income-gate alone) gated 43%: it held heals off through
            # too much of the race window. Bound it to the OPENING only -
            # the bootstrap either happens by t30 or the game is a race
            # anyway, and past t30 heals are unconditionally free.
            if (ct.get_current_round() <= 30
                    and ct.read_store(15) < 2):
                _cid = ct.get_tile_building_id(teamCore)
                if _cid is not None and ct.get_hp(_cid) > 150:
                    return
        except Exception:
            pass
    ...
    def harvestTask(self, ct: Controller, myLoc, myTeam) -> bool:
            ...
            for y in range(self.mapH):
                if self.mapPf.fullMap[x][y] == 1:
                    ...
                    if self.mapPf.inMoveZone(tile) and (x, y) not in enemyThreatened:
                        ...
                        # ORE BLINDNESS FIX (Coreflood 0-5, 49b90714: all 5
                        # were long-game starve-outs. heart: we built 3
                        # harvesters in 409 turns to their 10+ because this
                        # scorer ZEROED any ore beyond dist^2 160 (~12
                        # tiles) from our core - the far ore they expanded
                        # to sits at ~365 and was mathematically invisible.
                        # ic3d called it: "doesn't pathfind titanium".)
                        # Distance is now a PENALTY, never a cutoff: close
                        # ore still dominates 100x, far ore scores >0 so
                        # the economy expands when near patches are taken.
        ...
        if bestTile is not None:
            ...
            if ct.can_build_harvester(bestTile):
                ...
                ct.write_store(15, ct.read_store(15) + 1)  # econ bootstrap count
            return True
        return False
