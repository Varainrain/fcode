...
def run(ct: Controller, player):
    ...
    scores = {
        ...
        # TEST FIXTURE: this dummy exists to PARK SENTINEL WAVES, so the
        # eco keeps its hands off the war bank - the wave feeds on the
        # opening 500 (real wave teams bankroll this with real eco).
        'routeConv': [0, None] if ct.get_global_resources() < 100 else score_route_conv(ct, player, myLoc, myTeam),
        'routeHarv': [0, None] if ct.get_global_resources() < 100 else score_route_harv(ct, player, myLoc, myTeam),
        'harvest': [0, None] if ct.get_global_resources() < 100 else score_harvest(ct, player, myLoc, myTeam),
