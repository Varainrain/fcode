...
class Player:
    ...
    def runBestState(self, ct: Controller, myLoc: Position):
        ...
        if 78 <= ct.get_current_round() <= 90:
            print(
                "CE_TRACE",
                ct.get_current_round(), ct.get_id(), myLoc,
                "waller", self._is_waller(ct),
                "sieger", self._sieger_role(ct),
                "ti", ct.get_global_resources(),
                "attack", attackScore,
                "route", routeScore, routePos, routeLinks,
                "known", len(self.mapPf.knownTeamHarvesters),
                len(self.mapPf.knownTeamConveyors),
                len(self.mapPf.connectedConveyors()),
            )
