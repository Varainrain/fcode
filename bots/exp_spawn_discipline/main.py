...
def extra_spawn_allowed(nearby_friendly_builders: int) -> bool:
    return nearby_friendly_builders < CORE_BUILDER_CONGESTION_LIMIT


def count_nearby_friendly_builders(ct: Controller) -> int:
    my_team = ct.get_team()
    return sum(
        1 for unit_id in ct.get_nearby_units()
        if (ct.get_team(unit_id) == my_team
            and ct.get_entity_type(unit_id) == EntityType.BUILDER_BOT)
    )
...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        if (globalTitanium > 80 + 60 * self.numSpawned
                and extra_spawn_allowed(nearby_friendly_builders)):
