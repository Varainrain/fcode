...
class MapPathfinder:
        ...
        if tileId is not None:
            ...
            if tTeam == ct.get_team() and (tType == EntityType.BARRIER or tType == EntityType.GUNNER):
                if ct.can_destroy(curEnd):
                    ct.destroy(curEnd)
                    return
        # FIX 3: KEEP THE DOOR OPEN. Our own conveyors are impassable to our own
        # units, and 41% of this bot's conveyors (5.75 a game) land within two
        # tiles of the core->enemy axis - the corridor every attacker walks out
        # through. Blocking the corridor outright would starve the economy, since
        # that is often where the ore is, so this reserves only the single-tile
        # lane straight at the enemy, out to 6 tiles. Everything else is
        # unchanged.
        if self._blocksExitLane(curEnd):
            return False
    ...
    def _blocksExitLane(self, tile: Position) -> bool:
        """True if this tile sits in the one-wide lane from our core at theirs."""
        core = self.teamCore
        foe = self.enemyCorePos
        if core is None or foe is None:
            return False
        dx, dy = foe.x - core.x, foe.y - core.y
        length = (dx * dx + dy * dy) ** 0.5
        if length < 1:
            return False
        vx, vy = tile.x - core.x, tile.y - core.y
        along = (vx * dx + vy * dy) / length
        perp = abs(vx * dy - vy * dx) / length
        return 1 <= along <= 6 and perp < 1
