...
# FORWARD ARMOUR - measured from the 0-5 unrated sweep by Pantheon (#1) on
# 2026-08-03: 26 barriers across 5 games, ONE HUNDRED PERCENT of them within 6
# tiles of OUR core. They armour the siege position they have taken, not their
# own base. Result in the same games: they convert 10.2 of 13.2 guns onto our
# core (77%) while we convert 6.4 of 16.6 onto theirs (39%) - we build MORE guns
# and land fewer, because ours die on seats nobody is protecting.
# 3 Ti for 30 HP is three gunner shots absorbed, and per the team's own note our
# barriers are passable to our own units, so it costs no movement or heal access.
# This is NOT aegis-v1 (reactive armour at HOME, correctly rejected) and not
# Flotte's pre-armour (own-core seat denial). It is armour where our guns are.
FWD_ARMOR_TITANIUM_FLOOR = 100
FWD_ARMOR_BAND = 6
                ...
                if spotId is not None:
                    if ct.get_team(spotId) == myTeam and ct.get_entity_type(spotId) == EntityType.BARRIER:
                        if ct.can_destroy(spot):
                            ct.destroy(spot)
                            return True
        # No gun to place here: armour the position instead of idling. Only
        # inside the enemy firing band, and only where something is shooting.
        if self._forward_armor(ct, myLoc):
            return True
        return False

    def _forward_armor(self, ct: Controller, myLoc: Position) -> bool:
        enemyCore = self.mapPf.enemyCorePos
        if enemyCore is None or not ct.can_act():
            return False
        if ct.get_global_resources() <= FWD_ARMOR_TITANIUM_FLOOR:
            return False
        if abs(myLoc.x - enemyCore.x) + abs(myLoc.y - enemyCore.y) > FWD_ARMOR_BAND:
            return False
        myTeam = ct.get_team()
        threat = None
        for b in ct.get_nearby_buildings():
            ...
            if btype not in (EntityType.GUNNER, EntityType.SENTINEL):
                continue
            if myLoc in ct.get_attackable_tiles_from(
                    ct.get_position(b), ct.get_direction(b), btype):
                threat = ct.get_position(b)
                break
        if threat is None:
            return False
        best = None
        for d in CARDINALS:
            ...
            if ct.get_tile_building_id(spot) is not None:
                continue
            if not ct.can_build_barrier(spot):
                continue
            key = abs(spot.x - threat.x) + abs(spot.y - threat.y)
