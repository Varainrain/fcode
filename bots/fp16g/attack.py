...
class AttackBot:
    ...
    def atk_sent_siege(self, ct: Controller, myLoc, myTeam) -> bool:
        ...
        # OPEN THE GATE. Measured across the lingling scrim set (we lost 4
        # of 5): they fielded 1-5 sentinels in EVERY game and we fielded
        # 0-1, and sentinel fire is 100% of the core damage in all our
        # forensics. Waiting for proof the gunner path is dead means we
        # mostly never build the one unit that kills cores. Past the
        # opening, with money in hand, just build them.
        if (ct.get_current_round() >= 60
                and ct.get_global_resources() > 200):
            self.gunsDead = True
