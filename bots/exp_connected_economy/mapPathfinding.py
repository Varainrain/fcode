        ...
        for harvester in sorted(
            ...
            for end in sorted(ends, key=lambda p: (p.x, p.y)):
                ...
                if (occupyingBuilder is not None
                    ...
                    continue
                # A friendly builder standing on the missing link is ideal: it
                # can build a walkable conveyor on its own tile. Generic
                # passability reports occupied tiles as blocked, which made the
                # parent-style crowd erase route work exactly when it arrived.
