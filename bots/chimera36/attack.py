            ...
            if tileId is not None and ct.get_team(tileId) != myTeam:
                ...
                if tType == EntityType.CORE:
                    ...
                    if (tile.x, tile.y, ct.get_direction(tileId)) in threatSpots:
                        coreThreatHits += 1
                elif tType == EntityType.LAUNCHER:
                    # LOGISTICS KEYSTONE (Flotte doorstep v2): their camp
                    # regrows through launcher resupply - kill the taxi,
                    # strand the camp. Medic-tier when near our home.
...
def run_sentinel(ct: Controller, player):
    ...
    for tile in ct.get_attackable_tiles():
        ...
        if bType in [EntityType.GUNNER, EntityType.SENTINEL]:
            tileScore = 3
        if (bType == EntityType.LAUNCHER and player.mapPf.teamCore is not None
                and tile.distance_squared(player.mapPf.teamCore) <= 50):
            tileScore = 4   # logistics keystone: strand their camp
