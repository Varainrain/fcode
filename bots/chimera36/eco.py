...
PICKET_CORES = {
    ...
    (30, 30): frozenset({(2, 2), (26, 26),       # ragnarok+midgard
                         (14, 2), (14, 26),      # glacierkeep (Flotte campaign:
                         (2, 24), (26, 4),       # drakkarfjord - their line
                         (2, 14), (26, 14)}),    # valkyrie - kills unthinned
    # eco on every unpicketed 30x30 died t149-207 to Flotte's line;
    # ragnarok WITH picket wins t213-655. All big maps get the lane-thinner.
...
def run(ct: Controller, player):
    ...
    # DOORSTEP RUSH (Flotte's counter-iteration, 21c0223d g4: kicked builder
    # AT our core t3, gunners t8, walls t17 - the shell forms before protect
    # ever triggers because protect only knows turrets). An early enemy
    # BUILDER hugging our core is a protect-class threat: seat a gunner
    # facing it before its gunners exist.
    _tcD = player.mapPf.teamCore
    if _tcD is not None and ct.get_current_round() < 40:
        for _u in ct.get_nearby_units():
            if (ct.get_team(_u) != myTeam
                    and ct.get_entity_type(_u) == EntityType.BUILDER_BOT):
                _pu = ct.get_position(_u)
                if (_pu.distance_squared(_tcD) <= 13
                        and (_pu.x, _pu.y) not in covered):
                    uncoveredTurrets.append(_pu)
