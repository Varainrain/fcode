...
class Player:
    ...
    def runCore(self, ct: Controller) -> None:
        ...
        # STALL FIX ported from s_main v152 (Focalground 1c9231c9 g5: our
        # lkick built ONE gunner in 417 turns vs an EMPTY home - opening
        # builder died, nothing ever rebuilt). Builder heartbeats slot 8;
        # stale stamp -> respawn from the bank.
        if self.numSpawned >= 1:
            _hb = ct.read_store(8)
            _cr = ct.get_current_round()
            if _hb > 0 and _cr - (_hb - 1) > 6 and ct.get_global_resources() >= 90:
                for _t in ct.get_nearby_tiles():
                    if ct.can_spawn(_t):
                        ct.spawn_builder(_t)
                        self.numSpawned += 1
                        ct.write_store(0, self.numSpawned)
                        ct.write_store(8, _cr + 1)
                        break
    ...
    def run(self, ct: Controller) -> None:
        ...
        if etype == EntityType.CORE:
            ...
            # ABORT-AT-CONTACT: arrived at their ring and found defenders
            # waiting (defend-then-counter team) - do not feed the rush into
            # a prepared defense. Flip the whole team to eco; the war bank
            # becomes a head start.
            if (enemyCore is not None and ct.get_current_round() < 70
                    and self.mapW * self.mapH >= 300
                    and myLoc.distance_squared(enemyCore) <= 20):
                _defs = 0
                for _b in ct.get_nearby_buildings():
                    if (ct.get_team(_b) != ct.get_team()
                            ...
                            and ct.get_position(_b).distance_squared(enemyCore) <= 13):
                        _defs += 1
                if _defs >= 2:
                    ct.write_store(15, 1)
                    return
            ...
            # SENTINEL-AWARE SEATS (9cfe4060 g1, user-found: our lone builder
            # seated inside their sentinel's line and died - 18/hit, 40hp,
            # gone in 6 turns mid-build). The conservative default hid
            # sentinel lines from the seat filter.
            threatened = self.threatenedTiles(ct, myTeam, includeSentinels=True)
