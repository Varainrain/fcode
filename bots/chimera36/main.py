...
class Player:
    ...
    def run_core(self, ct: Controller) -> None:
        ...
        # THREAT-PERSISTENCE COUNTER (slot 11): consecutive rounds with an
        # enemy turret ABLE TO SHELL THE CORE (dist-sq <= 32 of footprint).
        # v158's first cut counted anything in VISION - on small maps enemy
        # home turrets are always visible, the counter never reset, and the
        # permanent recall crippled eco (antler fell t170 after 400-turn
        # holds). Same scoping lesson as war ammo: trigger on shellers only.
        _sheller = False
        for _b in ct.get_nearby_buildings():
            if (ct.get_team(_b) != myTeam
                    ...
                    and self.dist_to_core(ct, ct.get_position(_b)) <= 32):
                _sheller = True
                break
        if _sheller:
    ...
    def builder_bot(self, ct: Controller):
        ...
        if not eco.RECALL_STATE:
            ...
            if (coreHp < 120 or (coreHp < 350 and (self.mapPf.myNum) % 3 == 1)
                    or (ct.read_store(11) >= 10 and (self.mapPf.myNum) % 3 == 1)):
    ...
    def run(self, ct: Controller) -> None:
        ...
        # ABORT-AT-CONTACT (Flotte campaign: lk maps are 0-for-5 vs defend-
        # then-counter teams - their tell only shows when our rush ARRIVES).
        # A rush engine that finds defenders waiting writes slot 15 back to
        # eco; every unit re-reads and converts mid-game. The unspent war
        # bank becomes an eco head start instead of a corpse pile.
        if self.chMode in (3, 4, 5) and ct.read_store(SLOT_MODE) == 1:
            self.chMode = 1
