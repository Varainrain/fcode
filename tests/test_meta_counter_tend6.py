"""Tests for the bounded six-heal countergun maintenance budget."""
...
BOT_DIR = ROOT / "bots" / "exp_meta_counter_tend6"
sys.path.insert(0, str(BOT_DIR))
spec = spec_from_file_location("meta_counter_tend6", BOT_DIR / "main.py")
...
def test_maintenance_stops_after_six_lifetime_heals():
    player = BOT.Player()
    player._friendly_covering_gunner = lambda ct, threat: 7
    core_fallbacks = []
    player._heal_or_approach_core = lambda ct, pos: core_fallbacks.append(pos)
    ct = TendController()
    for _ in range(BOT.COUNTER_TEND_HEAL_LIMIT + 2):
        player._run_home_defense(ct, Position(3, 4), Position(3, 1))
    assert ct.heals == BOT.COUNTER_TEND_HEAL_LIMIT == 6
    assert len(core_fallbacks) == 2


def test_inherited_scope_and_no_fingerprints():
    ...
    assert "self._countertendheals += 1" in source
    for fingerprint in ("sweden.map", "string.map", "bridge.map", "ijti"):
        assert fingerprint not in source


if __name__ == "__main__":
    test_maintenance_stops_after_six_lifetime_heals()
    test_inherited_scope_and_no_fingerprints()
