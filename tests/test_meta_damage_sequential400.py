"""Scope tests for the 400-HP sequential home responder."""
...
BOT_DIR = ROOT / "bots" / "exp_meta_damage_sequential400"
PARENT = ROOT / "bots" / "meta-generalist-v1"
sys.path.insert(0, str(BOT_DIR))
spec = spec_from_file_location("meta_damage_sequential400", BOT_DIR / "main.py")
...
def test_threshold_matches_existing_recall_damage_floor():
    assert BOT.DAMAGE_SEQUENTIAL_HP == BOT.RECALL_CORE_HP == 400


def test_sequential_target_selection():
    threats = [threat(2, 3, 10), threat(4, 3, 11)]
    covered = {Position(2, 3)}
    assert BOT.choose_home_threat(threats, covered, False) == Position(2, 3)
    assert BOT.choose_home_threat(threats, covered, True) == Position(4, 3)
...
if __name__ == "__main__":
    test_threshold_matches_existing_recall_damage_floor()
    test_sequential_target_selection()
    test_exact_v9_scope()
