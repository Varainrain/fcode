"""Deterministic mechanism tests for exp_waller_route_light."""
...
BOT_DIR = ROOT / "bots" / "exp_waller_route_light"
...
def load_bot():
    ...
    try:
        spec = importlib.util.spec_from_file_location(
            "waller_route_light_main", BOT_DIR / "main.py")
...
def test_late_repair_is_bounded_and_below_direct_threat_priority():
    ...
    assert not allowed(BOT.LATE_ROUTE_REPAIR_START - 1, 500, 1)
    assert allowed(BOT.LATE_ROUTE_REPAIR_START,
                   BOT.LATE_ROUTE_RESOURCE_FLOOR,
                   BOT.LATE_ROUTE_MAX_LINKS)
    assert not allowed(BOT.LATE_ROUTE_REPAIR_START,
                       BOT.LATE_ROUTE_RESOURCE_FLOOR - 1, 1)
    assert not allowed(BOT.LATE_ROUTE_REPAIR_START, 500,
                       BOT.LATE_ROUTE_MAX_LINKS + 1)
...
def test_late_repair_is_waller_only_and_heal_gated():
    source = (BOT_DIR / "main.py").read_text(encoding="utf-8")
    assert "or not self._is_waller(ct)" in source
    assert "or healScore > 0" in source
    assert "routeScore = min(routeScore, LATE_ROUTE_SCORE_CAP)" in source


def test_economy_observation_is_not_in_every_builders_map_scan():
    source = (BOT_DIR / "mapPathfinding.py").read_text(encoding="utf-8")
    get_new = source[source.index("def getNewTiles"):source.index(
        "def observeEconomyTile")]
    assert "observeEconomyTile" not in get_new
    main_source = (BOT_DIR / "main.py").read_text(encoding="utf-8")
    assert "and self._is_waller(ct)" in main_source
    assert "self.mapPf.refreshEconomyGraph(ct)" in main_source
...
if __name__ == "__main__":
    ...
    test_late_repair_is_waller_only_and_heal_gated()
    test_economy_observation_is_not_in_every_builders_map_scan()
    print("lightweight waller route repair tests passed")
