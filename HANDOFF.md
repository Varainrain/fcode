...
## 🚨 STOP — ENGINE 2.3.6 VOIDS MOST NUMBERS BELOW (2026-08-06, oni's lane). Read `ENGINE-236-LAWS.md` first.
Measured this session with probe bots, not read off a changelog:
- **The map pool rotated to 15 maps and 12 were not on disk.** Only atoll/hive/jackpot survive from the old 21; everything gated locally before `fcode maps sync` was scored on maps the league no longer plays. Live pool is in `maps/`, the retired 18 are parked in `maps_stale_236/`.
- **The unit economy inverted again:** GUNNER is 20 Ti base / 7 dmg / 25 hp / 4 ammo per shot (docs below say 10 Ti / 10 dmg / 40 hp). New entity type SPLITTER (6 Ti) that no bot in `bots/` touches. A builder heals 4 hp/turn, so **one enemy healer cancels 57% of a gunner and two cancel it outright** — a lone seat cannot break a tended core, which is the mechanism under the guns-on-core invariant.
- **Costs now SCALE with what you own:** `scale% = 100 + 20*(living units - 1) + 1*(living buildings)`, cost = base * scale%/100 (verified both directions — destroying a building gives the 1% back). A gunner is 20 Ti at t0 and 58-72 Ti by t40+. **Every hardcoded titanium gate in every bot is now wrong**; `ct.get_gunner_cost()` and friends exist. Corollary: our own conveyor spam taxes our own offense, and killing enemy units makes their next purchase cheaper.
- **Ammo is a team-wide throttle:** the chassis converts `min(16 - ammo, Ti - 28)`, i.e. four gunner shots per turn for the whole team regardless of turret count. Instrumented: 19.2% of attacker turns run with the pool under one shot. v44 ran this line at 60; OogwayAttack shipped without it (PITFALL #17).
- **The gate is blinder than its own 55% bar:** a byte-identical copy of OogwayAttack read **61% / 49% / 50%** across three 120-game runs (pooled 53.3%, CI 48.2-58.4 — so no side bias, just width). Identical code lands anywhere in 49-61% at n=120, i.e. **nothing under ~10 points is resolvable**; a true +5 needs ~1500 games per arm. Pool a null control into every session and read candidates against it, never against 50%. `gate2.py` (from `claude/measurement-fixes`) prints Wilson CIs and per-seat splits; `gate.py` prints neither.
- Offense deliverables for Oogway: `modules/attack_advisor.py` + `modules/ATTACK-INTEGRATION.md` (decision functions + gate receipts). Instruments: `offense_audit.py` (per-game siege metrics from replays), `offense_gate_report.py` (pools gate runs, prints CIs vs the null).
